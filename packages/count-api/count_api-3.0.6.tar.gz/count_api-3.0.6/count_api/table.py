...
class Table:
        ...
        if isinstance(filters,dict):
            ...
            for key in filters:
                ...
                if not isinstance(cvs,list):
                    cvs = [cvs]
                new_filters = [(key.name,) + tuple(cv) if is_column_type(key) else (key,) + tuple(cv) for cv in cvs]
            filters = new_filters
    ...
    def upload_visual(self, x = None, y = None,  color = None,  size = None, aggregates = None, groupings = None, filters = None, chart_options = None):
        ...
        [x,y,size,color] = visual_helper.add_aggregates_to_axes(aggregates, x, y, size, color)
        [x,y,size,color] = visual_helper.add_groupings_to_axes(groupings, x, y, size, color)
        selects = visual_helper.get_select_from_columns([x,y,size,color])
        ...
        chart_axes = visual_helper.create_chart_axes(x,y,size,color)
        ...
        slack_token = self._get_slack_token(notebook_key)
    ...
    def _overwrite_concatenate(self, kwargs):
        ...
        if types:
            ...
            wrong_types = [t for t in types if not six.string_types(t)]
