...
class UserExamRecordUpdate(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if not old_status == new_status:
            send_message_signal.send(sender=UserExamRecord, user_exam_record=old_record)
            add_user_track_signal.send(sender=UserExamRecord, user_exam_record=old_record)
