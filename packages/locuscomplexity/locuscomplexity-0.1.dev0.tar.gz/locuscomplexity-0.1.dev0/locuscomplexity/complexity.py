...
def m_dn_un(m):
    ...
    """
    m_dn = m.div(diversity(m), axis='index')
    mT_un = (m.T).div(ubiquity(m), axis='index')
    m_tilde_community = pd.DataFrame(np.dot(m_dn, mT_un), index=m_dn.index, columns=m_dn.index)
    m_tilde_function = pd.DataFrame(np.dot(mT_un, m_dn), index=m_dn.columns, columns=m_dn.columns)
...
def complexity_indices(df_data, year):
    ...
    return pd.DataFrame(cci, index=Mc.index, columns=['cci']), \
           pd.DataFrame(fci, index=Mp.index, columns=['fci'])
...
def have_developed(df_data, y1, y2):
    """
    ...
    """
    m1 = build_m(df_data, y1)
    m2 = build_m(df_data, y2)
...
def distance_before_dev(df_data, y1, y2):
    """
    Get the distance of the community in each function it had before developing that RCA
    ...
    y = Y.sum(axis=0)
    n = N.sum(axis=0)
    H1 = pd.DataFrame(np.dot((D.transpose()), Y), index=Y.columns, columns=Y.columns).div(y, axis='index')
    H2 = pd.DataFrame(np.dot((D.transpose()), N), index=Y.columns, columns=Y.columns).div(n, axis='index')
...
def proba_to_develop(df_data, phi, y1, y2):
    """
    Generate the probability of developing a new RCA based on distance.
...
def plot_product_space(m, functional_prox, functional_comp, area=None, thresh=0.3,
    ...
    :param sim: (Dataframe) proximity matrix between all functions
    :param fci: (Series) Functional complexity series
    ...
    labels = {i: val for i, val in enumerate(list(functional_comp['Locus']))}
    colors = [cc.color(val) for key, val in enumerate(list(functional_comp['Locus']))]
    layout = nx.kamada_kawai_layout(mst)

    if (area is not None):
        presence = area_bipart.loc[area]
        for ind, code in enumerate(list(functional_comp['Locus'])):
    ...
    if (savepath is not None):
        savepath = os.path.join(dirname, '../results/fxnl_space/' + savepath)
