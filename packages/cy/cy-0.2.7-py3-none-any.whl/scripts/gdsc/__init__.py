...
def import_brass_bedpes(samples=None):
    samples = BRCA_SAMPLES if samples is None else samples
    return {s: cy.Utils.import_brass_bedpe(f'{DIR}/wgs/{s}.brass.annot.bedpe') for s in samples}
...
def get_copy_number_segments_wgs(as_dict=False, samples=None):
    samples = BRCA_SAMPLES if samples is None else samples
