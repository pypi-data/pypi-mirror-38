...
if __name__ == '__main__':
    ...
    for c in natsorted(set(cn_aucs_df['copy_number'])):
        if c not in ['0', '1']:
            ...
            pentaploid = cn_aucs_df.query(f"(copy_number == '{c}') & (ploidy_bin == '5+')")

            t, p = ttest_ind(diploid['auc'], pentaploid['auc'], equal_var=False)

            lof_tests.append(dict(t=t, p=p, c=c))

            print(f'{c}, {t}, {p:.1e}, {diploid.shape}, {pentaploid.shape}')
