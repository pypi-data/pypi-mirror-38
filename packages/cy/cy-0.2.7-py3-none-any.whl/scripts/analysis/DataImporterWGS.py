...
class DataImporterWGS(DataImporter):
    ...
    def __init__(self, copynumber_file='segmentation_data_9_lines_ascat.csv'):
        super().__init__(copynumber_file=copynumber_file)

    def import_brass_bedpes(self, fdir=None):
        fdir = f'{self.DIR}/wgs/' if fdir is None else fdir
        return {s: cy.Utils.import_brass_bedpe(f'{fdir}/{s}.brass.annot.bedpe') for s in self.samples}
...
def matching_svs(brass_bedpes, ascat_beds):
    sv_df, ratio_df = [], []
...
if __name__ == '__main__':
    # Define data-sets
    data = DataImporterWGS()
    data_process = DataProcessor(data=data, bed_filename_prefix='.wgs')

    # BRASS bedpe
    brass = data.import_brass_bedpes()

    # ASCAT segments
    ascat = data.get_copy_number(as_dict=True)
    ...
    crispr_beds = {
        c: BedTool(
            beds[c][['chr', 'start', 'end', 'fold_change', 'ratio']].to_string(index=False, header=False),
            from_string=True
        ).sort() for c in beds
    }

    # Matching SVs
    sv_df = matching_svs(brass, ascat)
    ...
    for s, chrm, xlim, ax3_ylim, ax2_ylim in svs:
        crispy_bed, brass_bedpe, ascat_bed = beds[s], brass[s], ascat[s].drop(['sample'], axis=1)
    ...
    for dist_offset in [1e3, 1e4, 1e5]:
        ...
        for y_var in ['fc_mean', 'ratio_mean']:
            ...
            plt.savefig(f'reports/analysis/svs_{y_var}_boxplot_{dist_offset:.1e}.pdf', bbox_inches='tight', transparent=True)
            plt.close('all')
