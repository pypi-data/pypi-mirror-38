...
class ShareAceTranslator(AceTranslator):
    def parse_trustee(self, trustee, ace):
        ...
        if len(trustee) == 1:
            ...
            if trustee == EVERYONE_NAME:
                ace['trustee'] = {'auth_id': EVERYONE_AUTH_ID}
                return
            elif trustee == GUEST_NAME:
                ace['trustee'] = {'auth_id': GUEST_AUTH_ID}
                return
            elif SID_REGEXP.match(trustee):
                ace['trustee'] = {'sid': trustee}
                return
            else:
    ...
    def pretty_trustee(self, ace):
        ...
        if ace['trustee']['auth_id'] == EVERYONE_AUTH_ID:
            return EVERYONE_NAME
        if ace['trustee']['auth_id'] == GUEST_AUTH_ID:
            return GUEST_NAME
        ...
        if ace['trustee']['domain'] == AD_DOMAIN:
    ...
    def ace_trustee_equal(self, ace, trustee):
        ...
        for attr in ('auth_id', 'sid', 'uid', 'gid'):
            if (attr not in arg_trustee) or (attr not in ace['trustee']):
                continue
            return arg_trustee[attr] == ace['trustee'][attr]
        return False
...
class SMBModShareAclCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        replace.add_argument("--grant-read-access", type=str_decode, nargs='+',
            metavar='TRUSTEE',
            help="Grant read access to these trustees.  e.g. Everyone, "
                 "uid:1000, gid:1001, sid:S-1-5-2-3-4, or auth_id:500")
        replace.add_argument("--grant-read-write-access",
            type=str_decode, nargs='+', metavar="TRUSTEE",
            help="Grant read-write access to these trustees.")
        replace.add_argument("--grant-all-access", type=str_decode, nargs='+',
            metavar="TRUSTEE",
            help="Grant all access to these trustees.")
        replace.add_argument("--deny-access", type=str_decode, nargs='+',
            metavar="TRUSTEE",
            help="Deny all access to these trustees.")
