...
class Molecule:
    ...
    def __init__(self, formula, adductIon = "H+", charge = 1, minRelAbundance = 0.01):
        ...
        self.isotopologues = IsoSpecPy.IsoSpec.IsoFromFormula(self.formula_ion, minRelAbundance, method='threshold_relative')
        ...
        # assemble the isotopic distribution as a list of (mass, 'normalised' abundance (= probability))
        # total m/z = mass + mass correction (amount of ionising species * correction per ionising specie) devided by charge
        self.isotopicDist = sorted(list(((mass + self.amountAdductIons * adductIon[2])/charge, exp(logProb)) for mass, logProb in zip(masses, logProbs)))
        # # leftover of previous version, kept to confirm calculations are still correct
