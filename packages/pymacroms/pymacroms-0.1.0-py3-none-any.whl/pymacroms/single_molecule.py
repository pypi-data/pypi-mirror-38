...
class Molecule:
    ...
    def __init__(self, formula, adductIon = "H+", charge = 1, minRelAbundance = 0.01):
        ...
        # The cutoff defines the fraction of the isotopes that should be accounted for (cumulative probability of the different isotopes)
        # No cutoff generates all the isotopologues, no matter how (un)likely they are to be encountered
        # Using the minRelAbundance to define this cutoff is somewhat arbitrary since you have no idea about the probablility of the most probable peak yet (this one will have a relative abundance of 1)
        # A value of minRelAbundance/2 should be sufficient though
        self.isotopologues = IsoSpecPy.IsoSpec.IsoFromFormula(self.formula_ion, 1 - (minRelAbundance/2))
        # IsoSpecPy returns the isotopic distribution as a list of lists
        masses, logProbs, confs = self.isotopologues.getConfs()
        # assemble the isotopic distribution as a list of (mass, relative abundance), filter out the peaks that have a too low relative abundance
        massMostAbundant = []
        isotopicDist = []
        for mass, logProb in zip(masses, logProbs):
            relAbund = exp(logProb) / exp(max(logProbs))
            if relAbund > minRelAbundance:
                isotopicDist.append(((mass + self.amountAdductIons * adductIon[2])/charge, relAbund)) # total m/z = mass + mass correction (amount of ionising species * correction per ionising specie) devided by charge
                if relAbund == 1:
                    massMostAbundant.append((mass + self.amountAdductIons * adductIon[2])/charge)
        # self.isotopicDist = sorted(isotopicDist)
        # then recalculate the normalised abundance (= probability) using the filtered isotopic pattern and write as an attribute
        self.isotopicDist = sorted(list((mass, relAbund/sum(list(zip(*isotopicDist))[1])) for mass, relAbund in isotopicDist))
        self.massMin = self.isotopicDist[0][0]
        self.massMax = self.isotopicDist[len(self.isotopicDist)-1][0]
        self.massMostAbundant = min(massMostAbundant)
