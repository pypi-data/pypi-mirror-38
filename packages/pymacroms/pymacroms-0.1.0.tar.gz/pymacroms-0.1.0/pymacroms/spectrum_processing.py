...
class Spectrum:
    ...
    def __init__(self, filename, mzRange, minRelAbundance = 0.01, ppmDev = 5, resolution = None, mergePeaksWithinResolution = False, filterByMonomer = None):
        """
    ...
    def matchPeaks(self, polymerSimulation):
        ...
        # Caution! for fitting, the normalised intensity is used rather than the relative intensity for the representation of the isotopic patterns!
        # This is fixed by now since everything should be transferred to norm abund internally
    ...
    def plotMatchPeaksOverview(self, mzRange: list = None, showMatched: bool = True, showNotMatched: bool = True):
        if showMatched:
            mz_axis = []
            relAbund_axis = []
    ...
    def printComposition(self):
        ...
        if self.composition is not None:
            ...
            for macromolecule, amount in self.composition.itertuples(name=None):
                report.append([list(self.polymerSimulation.macromolecules[macromolecule]["repUnitsCombination"]),
                               self.polymerSimulation.endgroupPairs[self.polymerSimulation.macromolecules[macromolecule]["endgroupPairIndex"]],
            ...
            for repUnitsCombination, endgroupPair, formula_ion, amount, massMostAbundant, matchedIsotopes in report:
    ...
    def plotComparisonSpectrum(self, resolution: float = None, color = "black", mzRange: list = None):
        fittedIsotopicDist = self.getFittedIsotopicDist(resolution)
        if fittedIsotopicDist is not None:
