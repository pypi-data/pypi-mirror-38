...
def combineIsotopes(peaklist: list, resolution: float):
    ...
    while index < len(peaklist_df):
        if not peaklist_df.iloc[index].isnull().all():  # checks that 'not all values in a row are NaN', alternative: .any()
            ...
            masses = list(peaklist_df.loc[(peaklist_df.mz >= mz - mz / resolution / 2) & (peaklist_df.mz <= mz + mz / resolution / 2)].mz)
            if len(masses) > 1:
                abundances = list(peaklist_df.loc[(peaklist_df.mz >= mz - mz / resolution / 2) & (peaklist_df.mz <= mz + mz / resolution / 2)].abundance)
                peaklist_df.loc[(peaklist_df.mz >= mz - mz / resolution / 2) & (peaklist_df.mz <= mz + mz / resolution / 2)] = None
