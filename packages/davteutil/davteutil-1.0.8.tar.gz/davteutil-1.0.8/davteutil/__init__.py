...
def make_button(text=None, callback_data='', prefix='', delimiter='|', data=[]):
    """Return a Telegram bot API-compliant button.
    callback_data can be either a ready-to-use string or a prefix + delimiter-joined data.
    If callback_data exceeds Telegram limits (currently 60 characters), it gets truncated at the last delimiter before that limit.
...
def mkbtn(x, y):
    ...
    Warning: this function will be deprecated sooner or later, stop using it and migrate to make_button.
    """
...
class CachedPage(Gettable):
    ...
    async def refresh(self):
        try:
            ...
            self._page = None
            logging.error(''.format(e), exc_info=False) # Set exc_info=True to debug
            return 1
        return 1
