...
class AXtrans:
    ...
    def act(self, acct1, acct2, rstatus, sendto, i):
        ...
        """
        if not self.db.get_user_token(self.memolist[i]['memofrom']):
            self.react.ignore(
            ...
            self.react.start(acct1,
                             acct2,
                             self.memolist[i]['memofrom'],
                             rstatus,
                             self.memolist[i]['memoid'])
        elif self.memolist[i]['action'] == "cancel":
            self.react.cancel(self.memolist[i]['memofrom'], self.memolist[i]['memoid'], rstatus, acct2)
        elif self.memolist[i]['action'] == "accept":
            self.react.accept(acct1, acct2, self.memolist[i]['memofrom'], rstatus, self.memolist[i]['memoid'])
        elif self.memolist[i]['action'] == "barter":
            self.react.barter(acct1, acct2, self.memolist[i]['memoid'], self.memolist[i]['memofrom'], rstatus,
        ...
        if self.react.reaction == "refund":
            self.sendto = self.memolist[i]['memofrom']
        else:
            self.sendto = sendto
    ...
    def fetch_history(self):
        ...
        # We go throught the transaction history of @steem-ax
        for a in self.steem.get_my_history():
            memofrom = a[1]['op'][1]['from']
            received_amount = a[1]['op'][1]['amount']
            trxid = a[1]['trx_id']
            ...
            if self.parse_history_record(a, rt):
                ...
                if self.parse_memo(memo=a[1]['op'][1]['memo']):
                    ...
                    if received_amount * 1000 >= len(self.memolist):
                        # We divide the amount sent by the number of messages
                        # that must be sent.
                        send_amount = int((received_amount * 1000) / len(self.memolist)) / 1000
                        # self.parse_memo creates self.memolist which we 
                        # then iterate through.
                        for i in range(0, len(self.memolist)):
                            if self.db.verify_memoid(memofrom, self.memolist[i]['memoid']):
                                ...
                                self.act(self.db.dbresults[0][0],
                                         ...
                                         self.db.sendto, i)
                            # If the memoid does not match the sender an error is
                            # sent along with a refund.
                            else:
                                self.react.ignore("Invalid Memo ID.")
                                self.sendto = memofrom
                            # Once the info has been parsed we send the message and update the
