...
class Pattern(object):
    ...
    def __repr__(self):
        ...
        return ("<Pattern(logdiam=%d, beszel_index=%d, ulqoma_index=%d) owned by %r>" % (logdiam, beszel_index, ulqoma_index, self.owner))
    ...
    def stream(self, data_or_filename, tempfile='tempfile'):

        tempfile = os.path.abspath(tempfile)

        if isinstance(data_or_filename, list):
            with open(tempfile, 'w') as f:
                for i in range(0, len(data_or_filename), 16):
                    data = data_or_filename[i:i+16]
                    f.write('%s\n' % str(data))
            newptr = self.lifelib('MakeSpaceshipStream', self.ptr, tempfile)
            try:
                os.remove(tempfile)
            except OSError:
                pass
