...
def get_data(url=None, file_name='output.json', source="file", key=None, ssl_verify=True,
    ...
    return GetData(options).get_data()
...
class GetData(dict):
    ...
    def __standardize(item):
        ...
        # new_item = {value:item.get(key, None) for (key, value) in key_map.items()}
        new_item = item.get('configuration', {})
