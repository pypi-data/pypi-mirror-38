...
class UserService:
    ...
    def _login(self, cred_type, identity, password=None, otp=None) -> Session:
        ...
        log(message='login', request_url=request_url, request_body=body, status_code=response.status_code, response=response.json())
    ...
    def update_profile(self, user_id: str, profile: dict) -> UserProfile:
        ...
        log(message='update_profile', request_url=request_url,
            status_code=response.status_code, response=response.json(), request_body=profile)
        if response.status_code == HTTPStatus.OK:
            return self._dict_to_user_profile(response.json().get('data'))
        if response.status_code == HTTPStatus.BAD_REQUEST:
    ...
    def create_user(
        ...
        log(message='create_user', request_url=request_url,
            status_code=response.status_code, response=response.json(), request_body=body)
