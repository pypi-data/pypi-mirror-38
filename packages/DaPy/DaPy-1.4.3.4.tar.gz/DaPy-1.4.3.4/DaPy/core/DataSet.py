...
class DataSet(object):
    ...
    def save(self, addr, encode='utf-8', decode='utf-8'):
        ...
        if ftype == 'csv' or ftype == 'txt':
            ...
            for data, sheet in zip(self._data, self._sheets):
                ...
                with open(addr, 'w') as f:
                    if hasattr(data, 'columns'):
                        writeline(f, ','.join(data.columns))

                    if isinstance(self._data, (Frame, Matrix, SeriesSet)) or \
                       (is_seq(data) and is_seq(data[0])):
                        for line in filter(is_seq, data):
                            writeline(f, ','.join(map(str, line)))

                    elif hasattr(data, 'items'):
                        if all(map(is_seq, data.values())):
                            writeline(f, ','.join(map(str, data.keys())))
                            temp = SeriesSet(data)
                            for line in temp:
                                writeline(f, ','.join(map(str, line)))
                            ...
                            for key, value in data.items():
                                writeline(f, ','.join([str(key), str(value)]))
                        ...
                        for record in data:
                            writeline(f, str(record))

                    else:
                        raise ValueError('DaPy can save a sequence object or dict-like object only.')
