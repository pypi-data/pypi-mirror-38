...
class WidgetListViewSet(ModelViewSet):
    ...
    authentication_classes = api_settings.WIDGET_FRAMEWORK_AUTHENTICATION_BACKENDS
...
class WidgetViewSet(ModelViewSet):
    ...
    def check_widget_list_edit_permissions(self, request):
        widget_list = get_object_or_404(WidgetList, pk=self.kwargs['widget_list_id'])
        if not self.request.user.has_perm(api_settings.WIDGET_LIST_EDIT_PERMISSION, widget_list):
            self.permission_denied(request, "This user does not have permission to edit that widget list")
    ...
    def create(self, request, *args, **kwargs):
        ...
        class
        """
        self.check_widget_list_edit_permissions(request)
        super().create(request, *args, **kwargs)
