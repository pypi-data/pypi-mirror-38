...
class WidgetBase(serializers.Serializer):
    ...
    def __init__(self, **kwargs):
        ...
        if widget_instance is not None and "data" not in kwargs:
    ...
    @property
    def data(self):
        return {key: self.fields[key].serialize(super(WidgetBase, self).data[key]) for key in self.fields}


class WidgetListSerializer(serializers.Serializer):
    widgets = serializers.ListField(child=WidgetBase)

    def __init__(self, **kwargs):
        widget_list_id = kwargs.pop("widget_list_id", [])
        try:
            widget_list = WidgetList.objects.get(widget_list_id)
            widgets = [widget.get_serialized_data() for widget in widget_list.get_widgets()]
        except ObjectDoesNotExist:
            widgets = []
        kwargs['data'] = {'widgets': widgets}
        super().__init__(**kwargs)

    @property
    def data(self):
        return [widget.get_serialized_data() for widget in self.fields['widgets']]
