...
@countRunTimeDecorater
def dropVarCorr(data,corThreshold=0.9999,varThreshold=0):
    '''
    在dataframe的view中修改，不进行copy
    去掉方差中小于等于varThreshold的列
    去掉相关系数绝对值大于corThreshold的列
    '''
    v = data.var()
    constant_feature = v[v<=varThreshold].index.values.tolist()
    for i in constant_feature:
        _ = data.pop(i)
    print('low var cols:',constant_feature)
    corr_col = ut.count_corr(data)
    corr_col = corr_col[corr_col.cor>corThreshold].col_2.values.tolist()
    for i in corr_col:
        _ = data.pop(i)
    print('corelation cols:',corr_col)    

@countRunTimeDecorater
def missing_columns_types(data):
    '''
    统计缺失值的类型及列名
    返回:
    cols: list(str,str))
    cols_dtype: dataframe
    '''
    ds = data.isnull().sum().sort_values()
    missing_cols = ds[ds!=0].index.values.tolist()
    print(f'missing cols: {missing_cols}')
    missing_cols_dtype =  data[missing_cols].dtypes.to_frame(name='dtype')
    print(f"missing types:\n{'-'*30}\n{missing_cols_dtype.astype(str).sort_values('dtype')}\n")
    print(f"missing types count:\n{'-'*30}\n{missing_cols_dtype.dtype.value_counts().to_frame(name='count')}")
    return missing_cols,missing_cols_dtype
...
def lgb_f1_sklearn_multi(y_true, y_pred):
