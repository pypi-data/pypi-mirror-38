...
class REINFORCE(Agent):
	def __init__(self, env_, network_specs, value_estimator_specs=None, gamma = 0.95, logdir = '.', inertia = 0.99):
		self.env_ 			= env_
	...
	def _add_placeholders(self):
		...
		self.value_est_learning_rate = tf.placeholder(dtype=tf.float32, name='value_est_lr')
	...
	def _add_loss(self):
		with tf.name_scope("loss_fn"):
			if self.use_baseline:
				...
				self.value_estimator_loss = tf.losses.mean_squared_error(self.returns_placeholder, self.state_values)
			else:
	...
	def _add_optim(self):
		...
		if self.use_baseline:
			self.optim_step_value_est = tf.train.AdamOptimizer(learning_rate = self.value_est_learning_rate).minimize(self.value_estimator_loss)
	
	def _start_session(self):
		self.sess = tf.Session()
		self.summary_op = tf.summary.merge_all()
		self.writer = tf.summary.FileWriter(os.path.join(self.logdir, "tensorboard/pg/"), self.sess.graph)
		self.writer.close()
		self.sess.run(tf.global_variables_initializer())

	def train(self, episodes = 10, lr = 0.01, value_est_lr = 0.01, gamma = 0.95, update_steps = 10):
		self.gamma = gamma
		self.lr = lr
		all_moving_rewards=[]
		...
		for episode in range(episodes):
			...
			self.update_net(lr, value_est_lr)
	...
	def update_net(self, lr, value_est_lr):
		...
		__, v__, loss_, loss_value_est = self.sess.run([self.optim_step, self.optim_step_value_est, self.loss, self.value_estimator_loss], feed_dict={self.state_placeholder: states, 
			self.returns_placeholder:returns, self.actions_placeholder:actions, self.learning_rate:lr, self.value_est_learning_rate:value_est_lr})
