...
class S3(botohelper.botohelpermain.Main):
    ...
    def create_copy_of_s3_bucket(self, src_bucket, dest_bucket, copy_only_mising_objects=False):
        """
        ...
        if copy_only_mising_objects:
            ...
            missing_objects_list = list(src_bucket_generator_set - dest_bucket_generator_set)
            for s3_object_key in missing_objects_list:
                response = self.client_s3.copy_object(Bucket=dest_bucket, Key=s3_object.key, CopySource={'Bucket': src_bucket, 'Key': s3_object_key})
        else:
            for s3_object in src_bucket_object.objects.all():
                response = self.client_s3.copy_object(Bucket=dest_bucket, Key=s3_object.key, CopySource={'Bucket': src_bucket, 'Key': s3_object.key})
