...
class Command(metaclass=abc.ABCMeta):
    ...
    def __init__(self, namespace, filename, timeout, askpass, password, disk_dir, brew_renew):
        ...
        text = 'Running update command for {}'.format(self.mode)
        ...
        if self._pkg_args(namespace):
            ...
            text = 'macdaily-{}: {}{}{}: no {}{}{} to {}'.format(self.cmd, yellow, self.mode, reset, bold, self.desc[1], reset, self.act[0])
    ...
    def _check_confirm(self):
        ...
        if self._yes or self._quiet:
            return True
        while True:
            ans = input('Would you like to {}? (y/N)'.format(self.act[0]))
    ...
    def _did_you_mean(self):
        for package in self._var__lost_pkgs:
            ...
            matches = '{}, {}'.format(reset, bold).join(
