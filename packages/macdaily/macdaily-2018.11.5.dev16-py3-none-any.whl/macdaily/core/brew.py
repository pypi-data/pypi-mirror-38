...
class BrewCommand(Command):
    ...
    def _proc_fixmissing(self, path):
        ...
        def _proc_confirm():
            ...
            text = 'macdaily-update: {}brew{}: found broken dependencies: {}{}{}'.format(yellow, reset, bold, pkgs, reset)
            print_term(text, self._file, redirect=self._qflag)
        ...
        if not _deps_pkgs:
            text = 'macdaily-update: {}brew{}: no broken dependencies'.format(green, reset)
            print_term(text, self._file, redirect=self._qflag)
            return
        ...
        if _proc_confirm():
            ...
            text = 'macdaily-update: {}brew{}: all broken dependencies fixed'.format(green, reset)
        else:
            text = 'macdaily-update: {}brew{}: all broken dependencies remain'.format(red, reset)
        print_term(text, self._file, redirect=self._qflag)
