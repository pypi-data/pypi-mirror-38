...
def get_track_info_from_file(fname):
    ...
    for idx, el in enumerate(mf):
        if len(el) > 0:
            tone_count = sum([1 for event in el[1:] if type(event) == midi.events.NoteOnEvent], 0)
        else:
            tone_count = 0
...
def three_track_midi(mf, dest, tr1, tr2, tr3):
    if isinstance(mf, str):
        mf = midi.read_midifile(mf)
    pat = midi.Pattern()
    pat.append(mf[tr1])
...
def get_timeline_from_track(track, tick_multiplier=2):
    ...
    notes = []
    for ev in track:
        # not sure why lots of midi files have multiple NoteOn events at the same time?
        if ev.tick == 0:
            continue
        timestamp += int(ev.tick * tick_multiplier)
...
def get_notes_from_timelines(timelines: list):
    ...
    # depending on the input tracks the beginning might be quiet, if so trim
    # it off
    if notes[0][0] == 0 and notes[0][1] == 0 and notes[0][2] == 0:
        notes = notes[1:]
