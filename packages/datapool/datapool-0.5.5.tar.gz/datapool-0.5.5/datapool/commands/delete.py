...
def delete_signals(do_delete, max_rows, filters, print_ok, print_err):
    ...
    with get_cmdline_logger(verbose=False):
        ...
        q = (
            session.query(SignalDbo.signal_id)
            .outerjoin(SignalDbo.site)  # site might be null !
            .join(SignalDbo.source)
            .join(SignalDbo.parameter)
            .filter(filter_expression)
        ...
        _delete_associations(session, q)
...
def _print_signals(q, max_rows, print_ok):
    ...
    if len(rows) > max_rows:
        ...
        rows = (
            rows[:n_start]
            + ["... skipped {} signals ...".format(n_skipped)]
            + rows[-n_end:]
        )
    for row in rows:
        print_ok("  " + row)
...
def _delete_source(
    ...
    _list_matching_signals(
        session, SourceDbo, SourceDbo.name == name, max_rows, print_ok
    )
    ...
    _delete_signals(
        session, SourceDbo, SourceDbo.source_id == source.source_id, print_ok
    )
...
def _delete_parameter(
    ...
    _delete_signals(
        session,
        ParameterDbo,
        ParameterDbo.parameter_id == parameter.parameter_id,
        print_ok,
    )
...
def _delete_signals(session, dbo, filter_, print_ok):
    q = session.query(SignalDbo.signal_id).join(dbo).filter(filter_)
    _delete_associations(session, q)
...
def _delete_associations(session, q):
    for association in (
        session.query(SignalCommentAssociation)
        .filter(SignalCommentAssociation.signal_id.in_(q.subquery()))
        .all()
    ):
    ...
    for association in (
        ...
        .filter(SignalSignalQualityAssociation.signal_id.in_(q.subquery()))
        .all()
    ):
        session.delete(association)
