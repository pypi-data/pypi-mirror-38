...
def _doc_generater(itemset, func):
    ...
    def defaulter(item):
        ...
        msg = getattr(
            item["method"], "msg", item.get("msg") or item.get("description") or "msg"
        )  # method maybe V
        if type(description) in {list, tuple}:
            description = list2mk(
                description, title=["value", "description"]
            )  # 支持list转化为markdown表格的形式
        elif isinstance(description, dict):
            description = parse_description(
                description, title={"value": "description"}
            )  # 支持dict转化为markdown表格的形式
        ...
        def wrapper(cls, request, *args, **kwargs):
            ...
            for item in itemset:
                name, v, required, msg, replace = [
                    item[x] for x in ["name", "method", "required", "msg", "replace"]
                ]
                value = None  # 与 '' 区别
                para = paramap.get(name)
                if required and para in (None, ""):  # 如果是post方法并且传参是json的话，para可能为0
                    result.error(name, "required")
                # GET 方法时不区分 '' 和不传这个parameter name的情况
                # POST方法时，区分'' 和  不传参数名的情况，因为'' 代表把这个参数名所对应字段置空
                if (request.method !='GET' and para is not None) or (request.method == 'GET' and para):
                    try:
                        value = v(para)
                    except Exception:
                        if settings.DEBUG:
                    ...
                    if value is None or value is False:
                        return result.error(name, msg)()
                    if value is True:  # 当v返回的value为True时，取request中的值
                        value = para
                    kwargs.update({replace or name: value})  # method 返回了非布尔值则更新kwargs
            if not result:
...
def join(data, length):  # 填充空格
    try:
        row = (
            "".join((x, (length[index] - len(x)) * " ")) for index, x in enumerate(data)
        )
        row = "|".join(row)
...
def list2mk(dataset, title=None):
    ...
    mk = "\n".join(_make(index, data, length) for index, data in enumerate(dataset))
    if txt:
        mk = "\n".join((txt, "", mk))
    return mk
...
def parse_description(dataset, title):
    dataset = {
        isinstance(x, int) and str(x) or x: y for x, y in dataset.items()
    }  # 吧数字转为字符串
