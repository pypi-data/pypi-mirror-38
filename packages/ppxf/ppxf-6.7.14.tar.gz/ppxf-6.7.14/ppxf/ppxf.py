...
class ppxf(object):
    ...
    .matrix:
        ...
        ``pp.matrix[nPixels, -nGas:]`` contains the ``nGas`` emission line
        templates if given. In the latter case the best fitting gas emission
        line spectrum is::

            gas_bestfit = pp.matrix[:, -nGas:] @ pp.weights[-nGas:]

        This is also returned in the ``.gas_bestfit`` attribute above.
