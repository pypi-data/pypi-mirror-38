...
class DNN:
    ...
    def maybe_save_checkpoint (self, acc):
        ...
        elif self.decay_plateau:
            self.plateau_continued += 1
            if self.plateau_continued >= self.decay_plateau:                
                self.sess.run (self.__increase_lr_reduce_step_op)
                self.plateau_continued = int (self.plateau_continued * 0.4) # cooldown        
    ...
    def set_learning_rate (self, initial_learning_rate = 0.001, decay_rate = 1.0, decay_step = 0, decay_plateau = 0):
        if decay_step and decay_plateus: 
            raise ValueError ("decay_step and decay_plateus should be mutaully exclusive")
        self.initial_learning_rate = initial_learning_rate
        self.decay_rate = decay_rate
        self.decay_step = decay_step
        self.decay_plateau = decay_plateau

    def trainable (self, improve_metric = "cost", overfit_threshold = 40, accuracy_threshold = 0.0):
