...
def aggQuery(depth1, depth2, cruise_name, cluster_level, size_frac_lower, domain, kingdom, phylum, class_tax, order, genus, species):
...
def organismsList(tax, depth1, depth2, cruise_name, cluster_level, size_frac_lower, table='tblESV'):
    query = "SELECT [%s], SUM(relative_abundance) AS total FROM %s WHERE " % (tax, table)
    query += "depth BETWEEN %f AND %f AND " % (depth1, depth2)
    query += "cruise_name='%s' AND " % cruise_name
    query += "cluster_level=%d AND " % cluster_level  
    query += "size_frac_lower=%f " % size_frac_lower  
...
def plotESVs(topN, tax, depth1, depth2, cruise_name, cluster_level, size_frac_lower):
    def spectrum(ind):
    ...
    orgs = organismsList(tax, depth1, depth2, cruise_name, cluster_level, size_frac_lower)
    if topN < len(orgs):
        orgs = orgs[:topN]
    ...
    for i in tqdm(range(len(orgs)), desc='overall'):
        ...
        dfOrg = aggQuery(depth1, depth2, cruise_name, cluster_level, size_frac_lower, domain, kingdom, phylum, class_tax, order, genus, species)
