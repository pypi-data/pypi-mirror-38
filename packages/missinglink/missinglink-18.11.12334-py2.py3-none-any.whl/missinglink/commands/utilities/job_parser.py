...
class JobParser(CloudCredentialsHandler):
    ...
    def _ensure_value(cls, input_data, key):
        if input_data.get(key) is None:
            raise exceptions.BadOptionUsage('Please provide `%s`' % key)
    ...
    def build_src_pointer(self, input_data):
        if not self._has_valid_git_pointer(input_data):
            ...
            if 'error' in sync_res:
                ...
                raise exceptions.BadOptionUsage('Failed to obtain git point to use fo the experiment. The error was %s' % sync_res['error'])
            else:
