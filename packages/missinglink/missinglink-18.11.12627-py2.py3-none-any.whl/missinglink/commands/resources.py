...
@click.option('--disable', 'state', flag_value=False, help='Disable existing queue')
@click.option('--enable', 'state', flag_value=True, help='Enable existing queue')
@click.pass_context
def manage_queue(ctx, org=None, state=None, display=None, create=None, name=None, **kwargs):
    url = '{}/queue/{}'.format(org, name)

    is_update = state is not None or display is not None
    disabled = None
    ...
    if create:
        result = ApiCaller.call(ctx.obj, ctx.obj.session, 'post', url, {'disabled': False, 'display': name})
    elif is_update:
