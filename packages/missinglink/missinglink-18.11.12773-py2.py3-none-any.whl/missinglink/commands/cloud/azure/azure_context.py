...
class AzureContext(BackendContext, CloudConnector):
    ...
    def _authorize_app(self, service_principal):
        ...
        role_definition_ids = ['/subscriptions/%s/providers/Microsoft.Authorization/roleDefinitions/%s' % (auth_client.config.subscription_id, role_id) for role_id in role_ids]
        for role_id in role_definition_ids:
            self._create_role_if_missing(auth_client, role_id, roles, service_principal)
        click.echo('Done')

    def _create_role_if_missing(self, auth_client, role_id, roles, service_principal):
        role = [r for r in roles if r.role_definition_id == role_id]
        if not role:
            auth_client.role_assignments.create(
                '/subscriptions/%s/resourceGroups/%s/' % (auth_client.config.subscription_id, self.rg_name),
                uuid.uuid4(),
