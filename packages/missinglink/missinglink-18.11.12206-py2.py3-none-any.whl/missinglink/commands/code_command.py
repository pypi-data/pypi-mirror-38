...
def _validate_code_params(source_repository, tracking_repository):
    ...
    if not code_tracking.validate_remote_urls_org(source_repository, tracking_repository):
        raise click.BadOptionUsage("The tracking repository must belong to the same organization as the source ({})".format(code_tracking.get_org_from_remote_url(source_repository)))
