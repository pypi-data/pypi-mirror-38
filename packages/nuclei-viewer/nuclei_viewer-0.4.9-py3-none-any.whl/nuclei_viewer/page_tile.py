...
layout = html.Div(className='row', children=[
    ...
    html.Div(className='two columns', children=[
        ...
        html.Div(className='card', children=[
            ...
            dashx.RadioItemRows(id='pt_tile', style={'display': 'grid'}),
            dashx.RadioItemRows(
                id='pt_color',
                style={'font-size': '6px', 'margin-top': '1px'},
                options=[[
                    {
                        'label': ' ',
                        'value': c,
                    ...
                    for c in ['Silver', 'Red', 'Orange', 'Gold', 'LimeGreen', 'RoyalBlue', 'BlueViolet']
                ]],
            ),
        ]),
        # Toolbar
...
@page.callback(
    ...
    [Input('pt_sample', 'value'), Input('pt_info', 'children'), Input('pt_color', 'value')],
    [State('pt_group', 'value'), State('pt_tile', 'value')]
)
def list_tile(sample, info, color, group, tile):
    ''' update tiles from selected sample '''
    if info and tile and color:
        info = json.loads(info)
        count = info['ctc'] if 'ctc' in info else 0
        count += info['immune'] if 'immune' in info else 0
        return common.list_tile(group, sample, tile, count, color)
