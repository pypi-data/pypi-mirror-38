...
def list_tile(group, sample, tile=None, count=None):
    ...
    for j in range(grid.shape[0]):
        ...
        for i in range(grid.shape[1]):
            ...
            if tid == tile:
                grid[j, i] = count
            uid = os.path.join(group, sample, tid)
            valid = helper.is_valid_sample(uid)
            option.append({
                'label': grid[j, i] if valid else '',
                'value': tid,
                'disabled': False if valid else True
            })
