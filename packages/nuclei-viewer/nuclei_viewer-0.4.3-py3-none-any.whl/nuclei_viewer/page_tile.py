...
layout = html.Div(className='row', children=[
    ...
    html.Div(className='two columns', children=[
        # hidden variables
        html.Div(id='pt_dummy', style={'display': 'none'}),
        # Dataset info
        ...
        dashx.TableInfo(id='pt_info', graph='pt_graph', cell='ctc', color=color),
    ]),
    # major content
    dcc.Graph(
        id='pt_graph',
...
@page.callback(
    ...
    [Input('pt_sample', 'value')],
    [State('pt_group', 'value')]
)
...
@page.callback(
    ...
    [Input('pt_tile', 'value'), Input('pt_channel', 'value'), 
...
def update_graph(tile, channel, nearby, enhance, group, sample, lock, relayout):
...
@page.callback(
    Output('pt_dummy', 'children'),
    [Input('pt_info', 'clickData')],
    [State('pt_group', 'value'), State('pt_sample', 'value'), 
     State('pt_tile', 'value')],
)
