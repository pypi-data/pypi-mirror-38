...
#    log.info(msg="save model file to mongodb")
    ...
    self.save_key=model_meta_primarykey
...
class Syncer(with_metaclass(Singleton, ModelDbSyncerBase.Syncer)):
    ...
    #     Syncer = cls
    #     return Syncer  experiment_run_config,
    def __init__(
            self, project_config, experiment_config, experiment_run_desc,
            thrift_config=None):
        ...
        self.thrift_config=thrift_config
        self.save_key=None
        self.experiment_run_desc=experiment_run_desc
    ...
    def sync(self,buffer_list=None,sql_cli=None):

        if self.save_key !=None:
            self.experiment_run_config=NewExperimentRun(self.experiment_run_desc,sha=self.save_key)
        else:
            self.experiment_run_config = NewExperimentRun(self.experiment_run_desc, sha="未保存模型文件")
        super(Syncer, self).__init__(self.project_config, self.experiment_config,
                                     self.experiment_run_config, self.thrift_config)
        ...
        #             if self.save_key!=None and sql_cli!=None: #"5bee664d87c5f627186b020b"
        #                 cursor = sql_cli.cursor()
