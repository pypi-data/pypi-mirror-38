...
class Configuration:
  ...
  def UpdateConf(self):		# Called second to update the configuration for the selected radio
    ...
    if radio_type in ("Hermes", "Red Pitaya", "Odyssey2"):
      if not radio_dict.has_key("Hermes_BandDict"):
        radio_dict["Hermes_BandDict"] = {}
      if not radio_dict.has_key("Hermes_BandDictTx"):
...
class QPowerMeterCalibration(wx.Frame):
  ...
  def OnBtnSave(self, event):
    ...
    if not name:
      ...
      dlg.ShowModal()
      dlg.Destroy()
    elif name in conf.power_meter_std_calibrations.keys():		# known calibration names from the config file
      dlg = wx.MessageDialog(self,
...
class RadioHardware(BaseWindow):		# The Hardware page in the second-level notebook for each radio
  ...
  def PowerMeterCalChoices(self):
    values = conf.power_meter_std_calibrations.keys()		# known calibration names from the config file
    radio_dict = local_conf.GetRadioDict(self.radio_name)
    values += radio_dict.get('power_meter_local_calibrations', {}).keys()		# local calibrations
    values.sort()
    values.append('New')
    return values
  def OnButtonChangePMcal(self, ctrl):
    ...
    if value == 'New':
      if not self.PMcalDialog:
        self.PMcalDialog = QPowerMeterCalibration(self, local_cal.keys())
    else:
  ...
  def ChangePMcalFinished(self, name, table):
    ...
    if name is None:        # Cancel
      ...
      if table is None:		# delete name
        del local_cal[name]
        name = conf.power_meter_std_calibrations.keys()[0]      # replacement name
      else:     # new entry
