...
class Trainer(object):
    def __init__(self, solver_file, weight_file, gpus, opts = None):
        ...
        if self.solver_file is None:
            if opts is not None and opts.model is not None:
                ...
                self.solver = net_info['props']['solver']
        else:
    ...
    def parse_solver(self):
        ...
        self.betas             = (float(solver.get('beta1', 0.5)), float(solver.get('beta2', 0.999)))
        ...
        self.warmup_mode       = solver.get('warmup_mode', 'linear')
        ...
        assert(self.lr_policy in ['fixed', 'step', 'exp', 'inv', 'multistep', 'poly', 'sigmoid', 'triangle', 'triangle2', 'trianglee', 'sgdr'])
        ...
        if self.lr_policy == 'multistep': assert('stepvalue' in solver)
    ...
    def create_optimizer(self):
        if self.solver_type == 'SGD':
            ...
            self.optimizer = optim.Adam(self.net.get_parameters(self.base_lr, self.weight_decay), betas=self.betas)
            ...
            #self.optimizer = optim.RMSprop(self.net.get_parameters(self.base_lr, self.weight_decay), alpha=0.9, eps=1.0, momentum=self.momentum, centered=False)
    ...
    def run_eval(self, cur_batch=None):
         ...
         self.net.broadcast_apply('reset_metric')
