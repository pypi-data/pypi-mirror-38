...
class XnatUploadTool:
    ...
    def renew_session(self):
        ...
        if self.lastrenew is None or ((self.lastrenew + self.sessiontimeout) < datetime.datetime.now()):
            self.logger.info('[SESSION] Renewing http session from %s with timeout %d' % (self.host, self.timeout))
            ...
            self.logger.debug('[SESSION] Reusing existing session until %s' % (self.lastrenew + self.sessiontimeout))
    ...
    def close_session(self):
        ...
        self.logger.debug('[SESSION] Deleting session')
        return True
