...
def createMetadata(workbook, metadataList=default_metadata, title="MarineGEO Data Template", imgpath=None, metadataValues=None):
    """
    ...
    mtb_version = Field("Metadata", "WorkbookBuildInfo",
                        "Information about the tool used to create the excel template")
    now = datetime.datetime.now()

    mtb_version_str = "Created using MarinegeoTemplateBuilder v{} on {}".format(__version__, now.strftime("%Y-%m-%d %H:%M"))
...
def main(output, fields, vocab=None, title="MarineGEO Data Entry Template", branding=None, protect=False, metadataValues=None, dryrun=False):
