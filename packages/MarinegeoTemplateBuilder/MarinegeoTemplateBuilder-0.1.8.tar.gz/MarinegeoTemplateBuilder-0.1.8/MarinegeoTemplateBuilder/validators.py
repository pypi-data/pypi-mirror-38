...
def validateList(field):
    ...
    :return: validator
    """
    validator = {'validate': 'list', 'error_type': field.warnLevel, 'source': '={}'.format(field.fieldName)}
    return validator
...
def validateFkey(field, allFields=Field.all):
    ...
    validator = {'validate': 'list', 'error_type': field.warnLevel, 'source': '{}'.format(eq)}
    return validator
...
def validateNumber(field):
    ...
    validator = {'validate': field.fieldType,
                 'error_type': field.warnLevel,
                 'error_message': '{} must be a number.'.format(field.fieldType.capitalize())}
    return validator
...
def selectValidator(field):
    ...
    if field.fieldType == "string":
        ...
        raise ValueError("Unknown validator for {} .... field type is {}".format(field.fieldName, field.fieldType))
    return
