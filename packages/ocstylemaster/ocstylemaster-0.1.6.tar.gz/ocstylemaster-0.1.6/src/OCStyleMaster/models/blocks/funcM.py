...
class FuncM(BlockBase):
    ...
    def line_count(self):
        ...
        bodyIndex = self.__get_func_body_start()
        bodyContent = self.content()[bodyIndex:]
        regex = "\n"
        finds = re.findall(regex,bodyContent,re.M)
        self._lineCount = len(finds)
        if self._lineCount is None or self._lineCount < 0:
            assert False
        return self._lineCount

    def __get_func_body_start(self):
