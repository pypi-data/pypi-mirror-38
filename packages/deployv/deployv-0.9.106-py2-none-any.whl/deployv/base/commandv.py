...
class CommandV(Extensible):
    ...
    def get_containers(self):
        ...
        for container_info in containers:
            for name in container_info.get('Names'):
                ...
                domain = re.search(r'^/([tiu][0-9]+(?:\w*)_[a-z]+[0-9]+)_odoo', name)
                if not domain:
                    continue
    ...
    def update_production(self):
        ...
        self.__instance_manager.exec_cmd('su {user} -c "branchesv save -p {home} -f {p}"'.format(
            user=odoo_user, home=odoo_home, p=path_branches))
