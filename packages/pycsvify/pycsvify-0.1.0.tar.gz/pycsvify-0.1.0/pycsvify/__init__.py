...
def parseCSV(csvString, itemSeperator=",", lineSeperator="\n"):
    """
    ...
    """
    try: 
        csvContent = csvString.split(str(lineSeperator))
        csvDict = {"_content" : {}}
        keyIndex = []
        keyCount = 0
        columnCount = 0
        ...
        for line in csvContent:
            if csvContent.index(line) == 0:
                for element in line.split(str(itemSeperator)):
                    keyIndex += [element]
                    csvDict["_content"][element] = []
                    keyCount += 1
                csvDict["_keyCount"] = keyCount
...
def exportCSV(dictionary, itemSeperator=",", lineSeperator="\n"):
    ...
    try:
        ...
        raise Exception("Generic dictionary error")
