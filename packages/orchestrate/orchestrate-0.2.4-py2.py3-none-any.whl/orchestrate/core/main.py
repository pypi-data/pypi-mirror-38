...
class OrchestrateClusterController(ArgparseController):
  ...
  @expose(
    ...
    arguments=[
      ...
      (['--cloud-provider'], dict(help='The cloud provider used to create this cluster', default='aws')),
    ],
  )
  def destroy(self):
    ...
    provider_string = provider_string or cloud_provider_string
  ...
  def test(self):
    ...
    (cluster_name, provider_strings) = self.app.services.cluster_service.test()
    if not provider_strings:
      print('This cluster was not located in any providers, assuming that you have created it.')
    elif len(provider_strings) == 1:
      provider_string = provider_strings[0]
      print(safe_format('Located a cluster named: {}, hosted on: {}', cluster_name, provider_string))
    else:
      print(safe_format('Located clusters named: {}, hosted on multiple providers:', cluster_name))
      for provider_string in provider_strings:
        print(safe_format('\t{}', provider_string))
    print(safe_format('Successfully connected to kubernetes cluster: {}', cluster_name))
