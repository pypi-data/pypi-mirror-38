...
class MLP(Mesh):
    ...
    def check_arr(self, arr):
        if self.layer_size is None or self.layer_size == []:
            ...
            for key in self.tensors:
                self.layer_size[key] = numpy.prod(self.tensors[key]['Shape'])
        ...
        for epoch in range(self.epoch_train):
            ...
            for idx in range(SIZE):
                ...
                loss_ = self.train_one(
                    in_arr[idx_], target_arr[idx_], self.rate, self.importance_out)
                if self.loss is None:
                    self.loss = loss_
                else:
                    self.loss += (loss_ - self.loss) * self.loss_mul

                if self.verbose > 1:
                    if (idx + 1) % self.iteration_log == 0:
                        print("Iteration %d/%d Epoch %d/%d\n    rate: %g loss: %g" %
                              (idx+1, SIZE, epoch, self.epoch_train, self.rate, self.loss))
                        sys.stdout.flush()
            ...
            if self.verbose > 0:
                print("Epoch %d/%d\n    rate: %g loss: %g" %
                      (epoch+1, self.epoch_train, self.rate, self.loss))
                sys.stdout.flush()
                if math.isnan(self.loss):
                    raise ValueError("loss is nan")
    ...
    def predict(self, in_arr, out_arr=None, verbose=0):
