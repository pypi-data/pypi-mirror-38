...
class NumericalizeTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    """
    Transforme categorical columns with a label encoding.
    """
    ...
    def transform(self, x):
        ...
        result = x.copy()
        features_to_convert = result.select_dtypes(
            ['object', 'category', 'bool']).columns
        for feat in features_to_convert:
            result[feat] = pd.Categorical(result[feat]).codes
        return result
...
class TimeSeriesTransformer(BaseEstimator, TransformerMixin, TransformerLogger):
    ...
    def __init__(self, param_dict, logger=None):
        ...
        self.target_column_name = str(uuid.uuid4())
        # Used to make data compatible with timeseries dataframe
        self.dummy_grain_column = str(uuid.uuid4())
        self.dummy_group_column = str(uuid.uuid4())
        self.original_order_column = str(uuid.uuid4())

        self.engineered_feature_name = None
    ...
    def _construct_pre_processing_pipeline(self, tsdf, columns_to_drop):
        ...
        default_pipeline = AzureMLForecastPipeline([
            ('drop_irrelevant_columns', DropColumns(columns_to_drop)),
    ...
    def fit(self, df, y=None):
        ...
        columns_with_only_nans = [x for x in tsdf.columns if
                                  np.sum(tsdf[x].notnull()) == 0]
        if isinstance(self.columns_to_drop, str):
            all_columns_to_drop = columns_with_only_nans + [self.columns_to_drop]
        elif self.columns_to_drop is not None:
            all_columns_to_drop = columns_with_only_nans + self.columns_to_drop
        else:
            all_columns_to_drop = [self.time_column_name]
    ...
    def transform(self, df, y=None):
        ...
        if self.engineered_feature_name is None:
            self.engineered_feature_name = transformed_data.columns.values
    ...
    def get_engineered_feature_names(self):
        ...
        """
        return self.engineered_feature_name
