...
def compute_metrics_classification(y_pred_probs, y_test, metrics,
                                   ...
                                   class_labels=None, y_transformer=None):
    ...
    if constants.Metric.ConfusionMatrices in metrics:
        ...
        try:
            if class_labels is not None:
                y_pred_labels = class_labels[y_pred_bin]
                bac = try_calculate_metric(
    ...
    if constants.Metric.Predictions in metrics:
        ...
        try:
            if bin_info is not None:
                bac = _predvstrue_compute(y_test, y_pred, bin_info)
            else:
                bac = np.nan
        except ValueError as e:
            bac = np.nan
...
class AccuracyTableMetric(ClassificationMetric):
    SCHEMA_TYPE = constants.Metric.AccuracyTable
    SCHEMA_VERSION = 'v1'
    NUM_POINTS = 100
...
class ConfusionMatrixMetric(ClassificationMetric):
    ...
    SCHEMA_TYPE = constants.Metric.ConfusionMatrix
    SCHEMA_VERSION = 'v1'
    ...
    def _compute_matrix(self, class_labels, sample_weights=None):
        ...
        return sklearn.metrics.confusion_matrix(y_true=self._y,
...
class ResidualsMetric(RegressionMetric):
    ...
    SCHEMA_TYPE = constants.Metric.Residuals
    SCHEMA_VERSION = 'v1'
    ...
    def compute(self, sample_weights=None, bin_info=None,
        ...
        if bin_info is None:
            raise ValueError("bin_info required to compute ResidualsMetric")
        if y_std is None:
...
class PredictedTrueMetric(RegressionMetric):
    ...
    SCHEMA_TYPE = constants.Metric.PredictedTrue
    SCHEMA_VERSION = 'v1'
