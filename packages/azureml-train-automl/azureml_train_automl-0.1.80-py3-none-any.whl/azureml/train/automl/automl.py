...
def _get_problem_info(X, y, task_type, y_transformer=None):
    ...
    dataset.parse_data("parse", X, y, task_type, init_all_stats=False, y_transformer=y_transformer)
...
def set_problem_info(X, y, task_type, current_run=None, workspace=None,
                     ...
                     enable_cache=False, subsampling=False,
                     timeseries=False, timeseries_param_dict=None, **kwargs):
    """
    ...
    if current_run is None:
        ...
        current_run = Run(experiment, run_id, _batch_upload_metrics=False)
...
def fit_pipeline(pipeline_script,
    ...
    if automl_settings.experiment_timeout_minutes is not None and elapsed_time is not None:
        experiment_max_time_sec = int(automl_settings.experiment_timeout_minutes) - elapsed_time
        if iteration_timeout_min is None or experiment_max_time_sec < iteration_timeout_min:
            iteration_timeout_min = experiment_max_time_sec
    ...
    try:
        ...
        try:
            ...
            dataset, pipeline_spec, training_type = \
            ...
            if enforce_time_on_win_required:
                ...
                runner = ClientRunner(dataset, metrics=metrics,
                                      task_type=automl_settings.task_type, runtime_constraints=runtime_constraints)

                results, status = runner.run_type(dataset,
                                                  pipeline_spec,
                                                  training_type=training_type,
                                                  is_ensemble_iteration=is_ensemble_iteration,
                                                  training_percent=train_frac * 100,
                                                  subsample_seed=automl_settings.subsample_seed)
        ...
        if (transformer is not None or lag_transformer is not None or ts_transformer is not None) and \
            ...
            fitted_pipeline = _add_transformer_x(transformer, lag_transformer, ts_transformer, fitted_pipeline)
        ...
        if y_transformer is not None:
            ...
            fitted_pipeline = PipelineWithYTransformations(fitted_pipeline, "LabelEncoder", y_transformer)
        ...
        if automl_settings.model_explainability:
            ...
            _explain_transformer = None
            if transformer is not None:
                _explain_transformer = transformer
            elif ts_transformer is not None:
                _explain_transformer = ts_transformer

            _explain_model_in_fit(child_run_metrics, fitted_pipeline, _explain_transformer, x_raw_column_names,
...
def _explain_model_in_fit(child_run, pipeline, transformer, columns, transformed_data_context, class_labels, logger):
    """
    ...
    :param transformer: the transformer used to get engineered features
    ...
    try:
        ...
        # Set the engineered columns information for model explanation
        if transformer is not None:
            columns = transformer.get_engineered_feature_names()
...
def _get_training_args(metrics,
    ...
    return dataset, pipeline_spec, training_type
    ...
    for name, score in scores.items():
        try:
            if isinstance(score, float):
                ...
                child_run.log_list(name, score.flatten())
            ...
            # These metrics will be deprecated
            elif name in [constants.Metric.ConfusionMatrices,
                          constants.Metric.Predictions,
                          constants.Metric.ROC,
                          constants.Metric.PrecisionRecall]:
                if name == constants.Metric.ConfusionMatrices:
                    for key in score.keys():
                        score[key] = (score[key].flatten()).tolist()
                uniform_arr_len = np.max([len(score[x]) for x in score])
                for key in score:
                    while len(score[key]) != uniform_arr_len:
                        score[key] = np.append(score[key], np.nan)
                child_run.log_table(name, score)
            else:
                logger.warning("Did not recognize metric: {}. Will not log.".format(name))
        except Exception as e:
            logger.warning("Failed to log the metric {} with value {}".format(name, score))
            _log_traceback(e, logger)
