...
class AutoMLRun(Run):
    ...
    def continue_experiment(
        ...
        :param exit_time_sec: How many additional seconds to run for this experiment for.
        :type exit_time_sec: int
        :param exit_score: Terminates the experiment when this score is reached.
        :type exit_score: int
        ...
        if not isinstance(exit_time_sec, int) and exit_time_sec is not None:
            raise TypeError("exit_time_sec expected to be 'int' or 'None'. received '{0}'".format(type(exit_time_sec)))
        if not isinstance(exit_score, int) and not isinstance(exit_score, float) and exit_score is not None:
            raise TypeError("exit_time_sec expected to be 'int' or 'None'. received '{0}'".format(type(exit_score)))
        ...
        automl_settings.exit_time_sec = exit_time_sec
        automl_settings.exit_score = exit_score
        ...
        self.tag('exit_time_sec', str(automl_settings.exit_time_sec))
        self.tag('exit_score', str(automl_settings.exit_score))
        ...
        automl = _azureautomlclient.AzureAutoMLClient(
            experiment=self.experiment, **automl_settings._as_serailizable_dict())
        ...
        if (automl_settings.compute_target is not None and automl_settings.compute_target != 'local') or\
           spark_context:
            try:
                if exit_time_sec is None and exit_score is None:
                    ...
                    self._jasmine_client.continue_remote_run(
                        self.id, automl_settings.iterations, automl_settings.exit_time_sec, automl_settings.exit_score)
