...
from .utilities import (_auto_blacklist, _format_training_data,
                        ...
                        friendly_http_exception, _get_ts_params_dict)
...
class AzureAutoMLClient(object):
    ...
    def _init_adb_driver_run(self,
        ...
        aml_token = self.current_run._client._client. \
            _client.run.get_token(experiment_name=self.automl_settings.name,
        ...
        datapre_json = self._get_dataprep(X=X, y=y, sample_weight=sample_weight,
                                          X_valid=X_valid, y_valid=y_valid, sample_weight_valid=sample_weight_valid)
        ...
        if not existing_run:
            ...
            parent_run_dto = CreateParentRunDto(target='local',
                                                ...
                                                raw_aml_settings_string=str(
                                                    self.automl_settings._as_serailizable_dict()),
    ...
    def _get_transformed_context(self):
        ...
        raw_data_context = RawDataContext(task_type=self.automl_settings.task_type,
                                          ...
                                          cv_splits_indices=self.input_data.get(
                                              "cv_splits_indices"),
                                          validation_size=self.automl_settings.validation_size,
                                          num_cv_folds=self.automl_settings.n_cross_validations,
                                          timeseries=self.automl_settings.is_timeseries,
                                          timeseries_param_dict=_get_ts_params_dict(self.automl_settings),
        ...
        try:
            ...
            if len(validation_scores['errors']) > 0:
                ...
                if 'TimeoutError' in inner_exception['traceback']:
                    ...
                    raise RuntimeError("Run '{0}' failed. Check {1} for more details.".format(
                        run_id, self.automl_settings.debug_log)) from inner_type
    ...
    def _get_dataprep(self, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None,
    ...
    def _fit_remote_core(self, run_configuration, X=None, y=None, sample_weight=None, X_valid=None, y_valid=None,
        ...
        dataprep_json = self._get_dataprep(X=X, y=y, sample_weight=sample_weight,
                                           X_valid=X_valid, y_valid=y_valid, sample_weight_valid=sample_weight_valid)
    ...
    def _log_data_stat(self, data, data_name, prefix=None):
        ...
        if type(data) is not np.ndarray and type(data) is not np.array and type(data) is not pd.DataFrame:
            try:
                ...
                self.logger.warning(
                    "The data type is not supported for logging.")
                return
        self.logger.info(
