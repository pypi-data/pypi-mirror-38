...
class EnsembleSelector(object):
    ...
    def _compute_models_predictions(self):
        ...
        if self.training_type == constants.TrainingType.TrainAndValidation:
            ...
            for index, tuple in enumerate(self.fitted_models):
                # the tuple is (iterationIndex, pipeline, algo_name)
                pipeline = tuple[pipeline_tuple_index]
                if pipeline is None:
                    continue
                predictions[:, :, index] = self._get_model_predictions(pipeline, X_valid)

        elif self.training_type == constants.TrainingType.MeanCrossValidation:
            X, _, __ = self.dataset.get_full_set()
            predictions = np.zeros((X.shape[0], self.num_classes, self.model_count))

            # we'll first arrange the X_valid, y_valid and the weights to follow the same order as CV folds
            sample_weight_valid = None
            split_index = 0
            for X_train_fold, y_train_fold, sample_wt_train_fold, X_valid_fold, y_valid_fold, sample_wt_valid_fold \
                    in self.dataset.get_CV_splits():
                if split_index == 0:
                    ...
                    X_valid = np.concatenate((X_valid, X_valid_fold))
            ...
            for index, tuple in enumerate(self.fitted_models):
                split_index = 0
                temp = None
                for X_train_fold, y_train_fold, sample_wt_train_fold, X_valid_fold, y_valid_fold,\
                        sample_wt_valid_fold in self.dataset.get_CV_splits():
                    pipeline = tuple[pipeline_tuple_index][split_index]
                    model_predictions = self._get_model_predictions(pipeline, X_valid_fold)
                    if split_index == 0:
