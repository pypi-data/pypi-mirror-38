...
def _get_border(vol):
    tmp = np.ones((vol.shape[0]+2, vol.shape[1]+2, vol.shape[2]+2), dtype=vol.dtype)
    tmp[1:-1,1:-1,1:-1] = vol
...
def distanceMetrics(vol1, vol2, voxelsize_mm):
    ...
    b1dst = scipy.ndimage.morphology.distance_transform_edt(border1 == 0, sampling=voxelsize_mm)
    b2dst = scipy.ndimage.morphology.distance_transform_edt(border2 == 0, sampling=voxelsize_mm)
