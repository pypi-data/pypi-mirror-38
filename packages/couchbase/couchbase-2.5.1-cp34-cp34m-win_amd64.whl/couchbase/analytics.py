...
#
from couchbase._libcouchbase import FMT_JSON

import couchbase.n1ql as N
import couchbase.exceptions
import couchbase._libcouchbase as LCB
import time
from couchbase.exceptions import CouchbaseInternalError
try:
    import urlparse
except:
...
class DeferredAnalyticsRequest(AnalyticsRequest):
    def __init__(self, params, host, parent, timeout = None, interval = None):
        # type: (DeferredAnalyticsQuery, str, couchbase.bucket.Bucket, Optional[float], Optional[float]) -> None
        """
    ...
    def _is_ready(self):
        ...
        raise couchbase.exceptions.TimeoutError("Deferred query timed out")
    ...
    def _poll_deferred(self):
        ...
        try:
            ...
            pass
        if status == 'failed':
            raise couchbase.exceptions.InternalError("Deferred Query Failed")
        if status == 'success':
            response_handle = response_value.get('handle')
            if not response_handle:
                raise couchbase.exceptions.InternalError("Got success but no handle from deferred query response")
            try:
                parsed_response_handle = urlparse.urlparse(response_handle)
            except Exception as e:
                raise couchbase.exceptions.InternalError("Got invalid url: {}".format(e))
