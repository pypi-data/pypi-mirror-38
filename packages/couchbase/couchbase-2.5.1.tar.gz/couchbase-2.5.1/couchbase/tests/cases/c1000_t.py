...
from couchbase.tests.base import ViewTestCase, SkipTest, RealServerTestCase, ConnectionTestCase
...
MAP="""
function (doc, meta) {
    if (meta.type == 'json' && doc && doc.cows && doc.cows.length > 0 && doc.rabbit &&
       typeof(doc.mouse) == "number" && doc.fish) {

      if (doc.fish) {
        if (doc.dog) {
          emit([doc.rabbit, -1, doc.mouse - 1, doc.fish], [1, 1]);
        } else {
          emit([doc.rabbit, -1, doc.mouse - 1, doc.fish], [0, 1]);
        }
        for(var cow_index in doc.cows){
          var cow = doc.cows[cow_index];
          for(var fly_index in cow.fly){
            var dori = doc.cows[cow_index].fly[fly_index].dori;
            if (doc.dog) {
              emit([doc.rabbit, dori, doc.mouse - 1, doc.fish], [1, 1]);
            } else {
              emit([doc.rabbit, dori, doc.mouse - 1, doc.fish], [0, 1]);
            }
          }
        }
      }
...
function (key, values, rereduce) {
    ...
    for(i=0; i < values.length; i++){
      sum[0] = sum[0] + values[i][0];
      sum[1] = sum[1] + values[i][1];
    }
    return(sum);
}
"""
...
class ViewIteratorTest(ConnectionTestCase):
    ...
    def test_long_uri(self):
        qobj = Query()
        qobj.mapkey_multi = [ str(x) for x in xrange(MAX_URI_LENGTH+100) ]
        ...
        keys = [r.key for r in self.cb.query("beer", "brewery_beers", limit=7000)]
        self.assertEqual(7000, len(keys))

        kslice = keys[90:]
        self.assertEqual(6910, len(kslice))
        rows = [x for x in self.cb.query("beer", "brewery_beers", mapkey_multi=kslice, limit=6910)]
        self.assertEqual(6910, len(rows))
        for row in rows:
    ...
    def test_c1000(self):
        ...
        #design=mgr.design_get(DNAME)
        #if not design:
        #mgr.design_create("rabbit", DESIGN_JSON, use_devmode=True)


        qobj = Query()
        qobj.mapkey_multi = [ str(x) for x in xrange(MAX_URI_LENGTH) ]
        ret = self.cb.query(DNAME, DQUERY, query=qobj)
        # No assertions, just make sure it didn't break
        for row in ret:
            raise Exception("...")
        ...
        # Apparently only the "keys" parameter is supposed to be in POST.
        # Let's fetch 100 items now
        keys = [r.key for r in self.cb.query(DNAME,  DQUERY, limit=7000)]
        self.assertEqual(7000, len(keys))

        kslice = keys[90:]
        self.assertEqual(6910, len(kslice))
        rows = [x for x in self.cb.query(DNAME, DQUERY, mapkey_multi=kslice, limit=6910)]
        self.assertEqual(6910, len(rows))
        for row in rows:
            self.assertTrue(row.key in kslice)
            print(row)
