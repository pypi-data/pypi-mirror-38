...
def assert_non_pip(dist, attr, value):
    '''Check and install non-pip packages if the value of attribute ``check_non_pip`` is True.

       Some dependencies of qmesh packages can not be installed via pip. The keyword
       ``check_non_pip``, in calls to ``setuptools.setup``, directs the installation
       procedure to check for pyrdm and GFD_basisChangeTools, then fetch and install the
       packages via other means, not using pip. This function first asserts the value of
       the keywords ``check_non_pip`` is a Boolean and then checks, and perhaps installs,
       the packages.
       ...
       Args:
           ...
           value (str): The value of the attribute (keyword), which should be a gmsh path.
       Raises:
           DistutilsSetupError: If the keyword value is not a Boolean.
    '''
    from setuptools import distutils
    from setuptools_qmesh.command import install_non_pip
    if bool(value) != value:
        raise distutils.errors.DistutilsSetupError(
            "%r must be a boolean value (got %r)" % (attr, value)
        )
    have_non_pip = install_non_pip.InstallNonPip(dist)
    have_non_pip.run()
