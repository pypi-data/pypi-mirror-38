...
angular.module('ajenti.filesystem').service('filesystem', function ($rootScope, $http, $q) {
    ...
    this.createFile = function (path, mode) {
        return $http.post('/api/filesystem/create-file/' + path, { mode: mode }).then(function (response) {
            return response.data;
        });
    };

    this.createDirectory = function (path, mode) {
        return $http.post('/api/filesystem/create-directory/' + path, { mode: mode }).then(function (response) {
            return response.data;
        });
    };
