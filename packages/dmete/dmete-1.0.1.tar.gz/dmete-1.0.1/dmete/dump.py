...
def read_station_ids(fileName=HOME+os.sep+'NationalAutomaticStations.txt'):
    ''' 从默认的站点列表中，读取各个站点号 '''
    with open(fileName, "r") as file:
        station_ids = [line.split()[0] for line in file.readlines()[1:]]
    return station_ids
...
def create_time_series(begTime, endTime, outName):
    ''' 输出时间序列 '''
    timeSeries = []
    while begTime <= endTime:
        thisTime = begTime.strftime("%Y%m%d%H")
        if not os.path.exists(outName.format(time=thisTime)):
            timeSeries.append(thisTime)
        begTime += timedelta(hours=1)
    return timeSeries

def main(maxThreads=10):
    ''' main function '''
    begTime, endTime, outPath = get_dynamic_parameters()
    if not os.path.exists(outPath):
        os.makedirs(outPath)
    outName = outPath+os.sep+"obs_mete_{time}.txt"
    ids = read_station_ids(fileName=HOME+os.sep+'NationalAutomaticStations.txt')
    timeSeries = create_time_series(begTime, endTime, outName)
