...
class TestBaseVolumeWeightAveragePriceFunction(unittest.TestCase):
    def setUp(self):
        ...
        self.pricing_data = pd.DataFrame({
             ...
             'TRADING_TOTALCLEARED': [20, math.nan, math.nan, 80, math.nan, math.nan],
    ...
    def setUp(self):
        ...
        self.cap_and_output_nans = pd.DataFrame(
            {'SETTLEMENTDATE': ['2015/01/01 00:00:00', '2015/01/01 00:05:00', '2015/01/01 00:10:00',
             ...
             'TOTALDEMAND': [1000, 1100, 13000, 900, 800, 1000]})
        self.cap_and_output_nans['SETTLEMENTDATE'] = pd.to_datetime(self.cap_and_output['SETTLEMENTDATE'])
...
class TestCapacityFactor90(unittest.TestCase):
    def setUp(self):
        ...
        self.cap_and_output2_nans = pd.DataFrame(
            {'SETTLEMENTDATE': ['2015/01/01 00:00:00', '2015/01/01 00:05:00', '2015/01/01 00:10:00',
             ...
             'TOTALDEMAND': [10000, 11000, 13000, 900, 800, 1000, 1000, 11000, 13000, 900, 800, 18000]})
        self.cap_and_output2_nans['SETTLEMENTDATE'] = pd.to_datetime(self.cap_and_output2['SETTLEMENTDATE'])
        self.cap_and_output2_nans2 = pd.DataFrame(
            {'SETTLEMENTDATE': ['2015/01/01 00:00:00', '2015/01/01 00:05:00', '2015/01/01 00:10:00',
                                ...
                                '2015/01/02 12:00:00', '2015/01/02 12:10:00', '2015/01/02 22:10:00'],
             'DUID': ['A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A'],
             'MAXCAPACITY': [math.nan, math.nan, math.nan, 68, 68, 68, 68, 68, 68, 68, 68, 68],
             'SCADAVALUE': [math.nan, math.nan, math.nan, 11, 10, 0, 8, 9, 50, 11, 10, 90],
             'TOTALDEMAND': [10000, 11000, 13000, 900, 800, 1000, 1000, 11000, 13000, 900, 800, 18000]})
        self.cap_and_output2_nans2['SETTLEMENTDATE'] = pd.to_datetime(self.cap_and_output2['SETTLEMENTDATE'])
        pass
    ...
    def test_capacity_factor_2_intervals_one_nan(self):
        peak = custom_tables.capacity_factor_over_90th_percentile_of_nodal_demand(self.cap_and_output2_nans)
    ...
    def setUp(self):
        self.cap_and_output = pd.DataFrame(
            {'SETTLEMENTDATE': ['2015/01/01 00:00:00', '2015/01/01 00:05:00', '2015/01/01 00:10:00',
             ...
             'MAXCAPACITY': [68, 68, 68, 68, 68, 68],
             'SCADAVALUE': [8, 9, 50, 11, 10, 0],
             'TRADING_TOTALCLEARED': [111, math.nan, math.nan, 250, math.nan, math.nan],
...
class TestMergeTables(unittest.TestCase):
    ...
    def test_merge_tables(self):
        merged_table = custom_tables.merge_tables_for_plant_stats(self.timeseries_df, self.gen_info, self.gen_info2,
                                                                  self.scada, self.trading_load,  self.dispatch_price,
...
class TestSelectHighestVersionNumber(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_result['EFFECTIVEDATE'] = pd.to_datetime(self.expected_result['EFFECTIVEDATE'])
...
class TestSelectInterventionIfPresent(unittest.TestCase):
    def setUp(self):
        self.dispatch_price = pd.DataFrame({
            ...
            'SETTLEMENTDATE': ['2015/01/01 00:00:00', '2015/01/01 00:05:00',  '2015/01/01 00:05:00',
                               '2015/01/01 00:00:00', '2015/01/01 00:05:00'],
        ...
        self.expected_result['SETTLEMENTDATE'] = pd.to_datetime(self.expected_result['SETTLEMENTDATE'])

    def test_select_highest_version_no(self):
        result = custom_tables.select_intervention_if_present(self.dispatch_price,
                                                              defaults.table_primary_keys['DISPATCHPRICE'])
        np.array_equal(result, self.expected_result)
