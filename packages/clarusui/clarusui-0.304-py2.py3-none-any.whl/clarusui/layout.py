...
class Element(object):
    ...
    def toHTML(self, event=None):
        if event is not None:
            outputType = clarus.get_output_type(event)
            if outputType is not None:
                if outputType.lower() == 'email' or outputType.lower() == 'mail':
                    return self.toInlinedHTML()
                
        return self.toStandardHTML()
    ...
    def toInlinedHTML(self):
        ...
        html = self.toStandardHTML()
        html = html.replace('<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">', '')
        converter = Premailer(html, disable_leftover_css=True, keep_style_tags=False,preserve_inline_attachments=False)
        return converter.transform(pretty_print=False)
