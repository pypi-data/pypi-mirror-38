...
class ObjectProxy(object):
    ...
    def include(self, *items):
        include = self.query.get('include', '').split(',')
        include.extend(items)
        self.query['include'] = ','.join(
            sorted(
                set(
    ...
    def order_by(self, *fields):
        sort = self.query.get('sort', '').split(',')
        sort.extend(fields)
        self.query['sort'] = ','.join(
            sorted(
                set(
    ...
    def filter(self, id=None, **kwargs):
        if id:
            ...
            return Model(self.bundle, path)

        query = dict(**self.query)
        for key, value in kwargs.items():
            query['filter[%s]' % key] = value

        return self.bundle.client.get(self.path, **query)
    ...
    def __populate__(self):
        ...
        data = self.bundle.client.load(self.path, **self.query)
