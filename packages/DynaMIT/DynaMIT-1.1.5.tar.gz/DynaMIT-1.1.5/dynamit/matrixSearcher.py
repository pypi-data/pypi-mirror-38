...
class MatrixSearcher(dynamit.motifSearcher.MotifSearcher):
	...
	def runSearch(self, sequencesFilename):
		...
		try:
			...
			matrixMotif.background = dict.fromkeys(["A", "C", "G", "T"], 0.25)
			# add a little pseudocounts to avoid -inf weights
			matrixMotif.pseudocounts = 0.005
			...
			if self.matrixType == "pfm":
				...
				matrix = PositionWeightMatrix(matrixMotif.alphabet,
									dict([(key, [v+0.0001 for v in lst]) \
												for key, lst in list(matrixMotif.counts.items())]))
			...
			try:
				scoreThreshold = ScoreDistribution(pssm=matrix,
													background=dict.fromkeys(["A", "C", "G", "T"], 0.25)).\
			...
			for sequence in sequences:
				# significantHits is a list of matching (startPosition, score) tuples
				significantHits = matrix.search(sequence.seq, threshold=scoreThreshold,
																				both=self.bothStrands)
