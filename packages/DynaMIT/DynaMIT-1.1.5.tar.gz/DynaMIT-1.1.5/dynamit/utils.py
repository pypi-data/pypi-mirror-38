...
def getFullSequenceID(sequences, partialSequenceID, minLength):
	...
	fullSeqIDList = [seqID for seqID in list(sequences.keys()) \
	...
	for seqRecord in sequences:
		...
		for motif in motifs:
			...
			for match in currentMotifMatches:
				...
				matchPositions.extend(list(range(startPosition + int(matchInfo[4]) -1,
...
def getBySequenceMotifsInstancesLists(sequences, motifsMatches):
	...
	for seqRecord in sequences:
		...
		for motif in motifs:
			...
			if seqRecord.description not in vectors[motif]:
...
def getBySequenceMotifsInstancesVectors(sequences, motifsMatches, binary=False):
	...
	for seqRecord in sequences:
		...
		for motif in motifs:
			...
			for match in currentMotifMatches:
				matchInfo = match.split('\t')
				matchPositions.extend(list(range(int(matchInfo[4]) - 1, int(matchInfo[5]) - 1)))
...
def computeClustersSilhouette(affinities, clustering, precomputed=True):
	...
	try:
		...
		# unable to compute silhouette, so return all zeroes.
		print("[ERROR] Unable to compute clusters silhouette values.")
		return None
