...
import sys, time
from dynamit import runner, __version__
...
def main():
	...
	if (len(sys.argv) < 3) or ("-h" in sys.argv):
		print("\n-----------------------------------------\nDynaMIT v" + \
					__version__ + " command line reference\n" + \
					"-----------------------------------------")
		...
		print("\nOPTIONS -----------------------------------\n" \
					...
					"\n\t-nc \n\t   avoid copying configuration and input files in the output folder.")
	...
	# check for the presence and store the parameter telling to avoid
	# copying the input and configuration files in the output folder.
	noCopy = 0
	if "-nc" in sys.argv:
		noCopy = 1

	# print DynaMIT version currently in use, and the
	# commandline with which it was invoked, useful for
	# analysis reproducibility and bug tracking.
	print("\n--------------------------------\nThis is DynaMIT v" + \
				__version__ + ".\n--------------------------------")
	...
	if myRunner.run(outputDirectory, sequencesFilename, nproc, polisherDist, noCopy) != 0:
