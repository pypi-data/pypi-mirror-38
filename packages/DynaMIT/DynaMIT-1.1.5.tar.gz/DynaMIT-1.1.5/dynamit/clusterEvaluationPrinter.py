...
class ClusterEvaluationPrinter(dynamit.resultsPrinter.ResultsPrinter):
	...
	def printResults(self, sequences, searchResults,
		...
		if "clusteringSilhouettes" in integrationResults:
			if integrationResults["clusteringSilhouettes"] != None:
			...
			for motif in clusterMotifs:
				...
				if len(motifMatches[0]) > 6:
					try:
						...
						scores.append(10.0 * mean(motifScores))
					except ValueError:
						continue
		...
		for cluster in clusters:
			...
			clustersGC[cluster] = 100.0 * mean([(float(m.upper().count("G")) + \
																				...
																				max(1.0, float(len(m))) for m in clusterMotifs])
		...
		with open("results_ClusterEvaluation.txt", "w") as outHandle:
			for cluster in clusters:
