...
##  This example calls on the NonlinearLeastSquares class to use the
##  Levenberg-Marquardt algorithm for estimating the parameters of a model surface
##  that best fits the synthetically generated noisy height data, but with the
##  proviso that the Jacobian must be calculated through the partial-derivatives of
##  the model functional as supplied by the user through the constructor option
##  'partials_for_jacobian' for the OptimizeSurfaceFit class.
##
...
##
##      4) The functionals for the partial derivatives to be used in the Jacobian
##         matrix, assuming that the option 'partials_for_jacobian' was set in the
##         instance of OptimizeSurfaceFit that was created.
##  
##      5) The display function to be used for plotting the partial and the final
...
##
##  Call syntax:  leven_marq_with_partial_derivatives.py
...
result = surface_fitter.calculate_best_fitting_surface('lm')                        #(E)
...
print("\n\n\nRESULTS RETURNED BY leven_marq_with_partial_derivatives.py:")          #(J)
...
for i,param_val in enumerate(sorted(initial_param_values.items())):
    print("%s  =>  %s" % (param_val[0], final_param_values[i]))                     #(N)
