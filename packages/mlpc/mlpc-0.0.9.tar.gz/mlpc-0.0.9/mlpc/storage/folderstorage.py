...
class FolderStorage:
    ...
    def get_filepath_for_writing(self, entity_type, file_name, file_type):
        ...
        file_path_with_number = self._append_number_suffix(file_path)
        if file_type is not None:
            file_type = "." + file_type
        else:
            file_type = ""
        return file_path_with_number + file_type
    ...
    def get_folderpath_for_writing(self, entity_type, resource_name):
        ...
        folder_path_with_number = self._append_number_suffix(folder_path)
    ...
    def _append_number_suffix(self, path, suffix_number=1):
        if os.path.exists(path):
            return self._append_number_suffix(path, suffix_number + 1)
        return path + "." + str(suffix_number)
