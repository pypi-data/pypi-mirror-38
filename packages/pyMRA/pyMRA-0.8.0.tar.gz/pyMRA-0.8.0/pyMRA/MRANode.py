...
class Node(object):
    ...
    def __init__(self, parent, ID, locs, notKnots, levelsFromLeaves,
        ...
        if self.res==self.critDepth:
            ...
            pt = ThreadPool(nodes=NCh)
            self.children = pt.map(Node, [self]*NCh, data['chID'], data['chLocs'], data['chNotKnots'], data['lev'], data['J'], data['r'], data['critDepth'], data['covCh'], data['chObs'], data['chR'])
