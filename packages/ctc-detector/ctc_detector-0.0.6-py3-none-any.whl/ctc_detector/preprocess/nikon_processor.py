...
└── group_1
     ├── sample_1
     │   ├── sample_1_(labels).csv
     │   ├── sample_1_(3 points).csv
     │   └── images
     │       ├──sample_1_(R).tif
     │       ├──sample_1_(G).tif
     │       └──sample_1_(B).tif
...
def get_tiff_images(image_dir):
    ...
    for i, f in enumerate(files):
        ...
        match = re.search(r'[A-Za-z0-9 -_]*\(([RrGgBb])\)', fbase)
...
def process_sample(sample_dir, grids, output_dir):
    ...
    centroid_file = os.path.join(sample_dir, f'{sample_id}_(labels).csv')
    roi_file = os.path.join(sample_dir, f'{sample_id}_(3 points).csv')
