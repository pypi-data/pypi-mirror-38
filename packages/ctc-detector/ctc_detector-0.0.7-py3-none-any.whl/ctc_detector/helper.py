...
def get_ctc_nuclei_array(csv_path, json_path, shape):
    ...
    try:
        df = pd.read_csv(csv_path, index_col=0, usecols=['ImageId', 'EncodedPixels'], delimiter=',')
...
def get_match_nuclei(path, prob_array):
    ...
    if not os.path.isfile(mask_csv):
        if gen_nuclei_rle:
            generate_target_nuclei(path, prob_array)
        else:
            raise FileNotFoundError(f'{mask_csv} not found!')
...
def write_ctc_prob(src_path, tgt_path, data):
    ...
    for id_, prob in data:
        df.at[id_, 'TumorProb'] = prob
    df.to_csv(tgt_csv)
