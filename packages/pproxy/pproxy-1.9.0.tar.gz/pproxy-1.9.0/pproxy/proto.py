...
class WS(BaseProtocol):
    def correct_header(self, header, **kw):
        return header and header.isalpha()
    def patch_ws_stream(self, reader, writer):
        data_len, _buffer = None, bytearray()
        def feed_data(s, o=reader.feed_data):
            ...
            while 1:
                if data_len is None:
                    if len(_buffer) < 2:
                        break
                    required = 2 + (2 if _buffer[1]&128 else 0)
                    p = _buffer[1] & 127
                    if p == 126:
                        required += 2
                    elif p == 127:
                    ...
                    if len(_buffer) < required:
                        break
                    if p == 126:
                        data_len = int.from_bytes(_buffer[2:4], 'big')
                    elif p == 127:
                        data_len = int.from_bytes(_buffer[2:6], 'big')
                    else:
                        data_len = p
        ...
        def write(data, o=writer.write):
            if not data: return
            length = len(data)
            return o(b'\x02' + (length.to_bytes(1, 'big') if length < 126 else b'\x7e'+length.to_bytes(2, 'big') if length < 65536 else b'\x7f'+length.to_bytes(4, 'big')) + data)
        writer.write = write
    async def parse(self, header, reader, writer, auth, authtable, httpget, **kw):
        lines = header + await reader.read_until(b'\r\n\r\n')
        headers = lines[:-4].decode().split('\r\n')
