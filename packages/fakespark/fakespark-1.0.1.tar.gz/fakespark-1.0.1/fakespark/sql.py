...
class FakeFrame(object):
    def __init__(self):
        self._content = [Row(fields='_1', row=[''])]
        self.name = None

    def __str__(self):
        return '<fakespark.sql.FakeFrame>,it has {} lines, \nits first row fields are {}' \
            .format(self.length, self.dtypes)

    def __repr__(self):
        return '<fakespark.sql.FakeFrame>,it has {} lines'.format(self.length)
    ...
    def count_key(self, key):
        ...
        ck = []
        for v in values_set:
            count = self.filter(condition=lambda row: row['city'] == '{}'.format(v), new=True).count()
            ck.append((v, count))
        return ck
    ...
    def to_csv(self, file, head=False, encoding='utf8', quoting=csv.QUOTE_ALL):
        """
        将对象内容写入CSV文件
        :param file: 要写入的文件名
        :param head: 是否要将列名写入
        ...
        :return:
        """
        with open(file, mode='a+', encoding=encoding, newline='') as f:
