...
## TODO: Filetype management on dialouges
## TODO: Create exceptions 
## TODO: Determine filetypes by using file dialogues 
## TODO: Convert UI file to python file 
## TODO: Convert images and icons to a QT resource file
## TODO: Create status messages and direct them to the console
## TODO: Cancel deletes the existing path. correct it
...
description_CL = """Creates a language file from an exported treatment file (txt). zBrac grabs the text that are wrapped in double brackets. Generated files can later be used for translation/text replacement.
"""

description_IL = """
Generates zTree treatment file from an exported zTree file(txt) and a language file (xlsx) in the target language.
"""

description_SB = """
Test description
Removes double brackets from the file. You can use this option if your zTree file is already in the language you need. This option is useful if you are coding your zTree files with brackets by default and you would like to see how it looks without them.
"""
...
class MyWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # self.button_treatment_CL.clicked.connect(self.browse_file("line_treatment_CL"))

        ### Create Language File (CL) Items
        # This is not the best implementation but since we have to roll out soon, we can just handle it after the release.
...
"""
    ...
    def text_format_desc(self,text):
        prefix = "<html><head/><body><p><span style=\" font-size:7pt; font-weight:600;color:#00007f\">"
    ...
    def generate_language(self):
        ...
        if not matched_entries:
            ...
            print('')
            return (False)
        # print('promting for a savepath')
        filedialog = QFileDialog()
        filedialog.setDefaultSuffix(
            '.xlsx')  # this somehow doesn't do the trick. it is better to determine the filetype by using the file dialog.
        # just a note here. if we have to force filenames in the function, we should use os.path.splittext()
    ...
    def generate_treatment(self):
        ...
        if (not (os.path.isfile(path_treatment_in))):
            self.pop_error('snap, you dont have a proper treatment file')

        elif (not (os.path.isfile(path_language_in))):
            self.pop_error('snap, you dont have a proper language file')
            ...
            if (not (os.access(os.path.dirname(save_path), os.W_OK))):
                self.pop_error('snap, you dont have a proper save path')
