...
class XnatWorkflowGather:
        ...
        try:
            ...
            if self.dbpass:
                ...
                conn = psycopg2.connect(host=self.dbhost,
                                        ...
                                        user=self.dbuser)
            cur = conn.cursor()
            mywindow = datetime.now() - timedelta(minutes=self.lastmin)
            ...
            cur.execute(
                ...
                "COALESCE(last_modified,launch_time) > %s GROUP BY pipeline_name, wrk.status;", (mywindow,)
            )
    ...
    def push_datadog(self):
        for pipeline, state, count in self.workflow:
            self.logger.info("Pushing %d workflow events to datadog" % len(self.workflow))
            mytags = ["pipeline:%s" % pipeline, "state:%s" % state]
            datadog.statsd.histogram('xnat.workflow', count, tags=mytags)
        return True
