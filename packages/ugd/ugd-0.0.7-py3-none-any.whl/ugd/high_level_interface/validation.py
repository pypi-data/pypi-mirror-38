...
def parse_input(stat_f, test_variable, controlls, var_dict, adj_m):
    ...
    nodesetpartition = constr_parition(controls=controlls, var_dict=var_dict, adj_m=adj_m)
...
def validate_var_dict(var_dict, n):
    ...
    for i in range(n):
        if not (i in var_dict):
            raise ValueError(
                'var_dict must be a dictionary with primary key the integer 1..n, where n is the nodenumbe')
        if not (isinstance(var_dict[i], dict)):
            raise ValueError(
                "Values of vardict must be a dictionary, with the variable name as key and the varibale value as value")
...
def validate_controlls(controlls, var_dict):
    ...
    for controll in controlls:
        if not (controll in var_dict[0]):
            raise ValueError(
                'controlls must be a list of variable names, the varible names key in the values of var_dict \n'
