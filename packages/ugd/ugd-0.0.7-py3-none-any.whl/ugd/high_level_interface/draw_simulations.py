...
def hyp_test(adj_m, var_dict, stat_f, test_variable, controlls, mixing_time, anz_sim, show_polt, is_directed):
    ...
    graph = generate_graph(adj_m, nodesetpartition, is_directed)  # fixme: adapt this

    '3) determine mixing time, and mixing "amount" '
    mixing_time = evaluate_mixing_time(graph, mixing_time, anz_sim)
    ...
    postprocess(adj_m_original=adj_m, stats_list=stats_list, var_dict=var_dict, stat_f=stat_f, show_polt=show_polt, test_variable=test_variable)
