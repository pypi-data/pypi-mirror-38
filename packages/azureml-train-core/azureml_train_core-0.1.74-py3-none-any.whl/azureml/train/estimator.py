...
class MMLBaseEstimator(ABC):
    ...
    def _get_telemetry_values(self, func):
        ...
        telemetry_values['nodeCount'] = self._estimator_config.batchai.node_count
...
class MMLBaseEstimatorRunConfig(RunConfiguration):
    ...
    :param vm_location: The location of the compute target that will be created for the training. If not
    specified, it will be defaulted to the workspace's location.This takes effect only when the vm_size
    param is specified in the input.
    ...
    def __init__(self, compute_target, vm_size=None, vm_priority=None, vm_location=None,
        ...
        if node_count > 1 and compute_target.type.lower() != BatchAITarget._BATCH_AI_TYPE.lower():
        ...
        if framework is None:
            framework = MMLBaseEstimatorRunConfig._get_framework(
        ...
        super(MMLBaseEstimatorRunConfig, self).__init__(
            script=entry_script,
            arguments=arguments)
        self.batchai.node_count = node_count
        self.amlcompute.vm_size = vm_size
        self.amlcompute.vm_priority = vm_priority
        self.amlcompute.location = vm_location
    ...
    @staticmethod
    def _get_framework(backend, node_count, process_count_per_node):
        framework = "Python" if not backend else "PythonMpi" if backend.lower() == "mpi" \
            else "TensorFlowParameterServer" if backend.lower() == "ps" else None
        if not framework:
