...
class HyperDriveRun(ScriptRun):
    ...
    def __init__(self, experiment, run_id, run_config, hyperdrive_run_config):
        ...
        if hyperdrive_run_config is not None:
            self._hyperdrive_run_config = hyperdrive_run_config
        else:
            # Get HyperDrive runconfig from experiment in the cloud
            hyperdrive_run_config = RunConfiguration._get_runconfig_using_runid(experiment, run_id)
            self._hyperdrive_run_config = hyperdrive_run_config
    ...
    def _get_best_run_id_by_primary_metric(self):
        ...
        child_run_ids = [run.id for run in self.get_children() if run.get_status() not in ["Failed", "Canceled"]]
