...
from azureml.train.hyperdrive._search import MAX_DURATION_MINUTES, search
...
from azureml._base_sdk_common.utils import convert_dict_to_list, list_remove_empty_items
...
class HyperDriveRunConfig(object):
    ...
    def _get_platform_config(self, workspace, run_name):
        ...
        if run_config.target == "amlcompute":
            self._set_amlcompute_runconfig_properties()
    ...
    def _remove_duplicate_estimator_arguments(self):
        ...
        estimator_args_dict = self.estimator._estimator_config._script_params
        ...
        if warning:
            ...
            run_config_copy.arguments = list_remove_empty_items(convert_dict_to_list(estimator_args_dict))

        return run_config_copy

    def _set_amlcompute_runconfig_properties(self):
        run_config = self._estimator.run_config
        # A new batchai cluster with this name will be created for this HyperDrive run.
        ...
        warnings.formatwarning = _simple_warning
        warnings.warn("A BatchAI cluster with {} node count will be created for this HyperDriveRun. "
                      "Please consider modifying max_concurrent_runs if you think this will exceed the"
