...
do
	...
	function TBMenu:showEvents(viewElement)
		...
		eventDisplayTime:addCustomDisplay(true, function()
				set_color(1,1,1,1)
				draw_quad(eventItems[1].button.pos.x, eventItems[1].button.pos.y - 5, (os.clock() - rotateClock.start) % rotateTime / rotateTime * eventItems[1].button.size.w, 5)
			end)
		viewElement:addCustomDisplay(false, function()
				if ((math.floor(os.clock()) - rotateClock.start) % rotateTime == 0 and math.floor(os.clock()) ~= rotateClock.last) then
	...
	function TBMenu:showHome()
		...
		local tbMenuSaleIcon = UIElement:new({
			...
			bgImage = "torishop/icons/" .. TB_MENU_TORISHOP_INFO.sale.name:lower() .. ".tga"
		})
		...
		tbMenuSaleName:addCustomDisplay(true, function()
				tbMenuSaleName:uiText(TB_MENU_TORISHOP_INFO.sale.name, nil, nil, nil, LEFTMID, 0.8, nil, 1)
			end)
		...
		tbMenuHide:addCustomDisplay(false, function()
				if (tbMenuHide.state == -1) then
					...
					if (tbMenuMain.pos.y >= WIN_H) then
						tbMenuHide.child[1]:updateImage("/system/arrowtop.tga")
				...
				elseif (tbMenuHide.state == 1) then
					...
					if (tbMenuMain.pos.y <= 0) then
						tbMenuHide.child[1]:updateImage("/system/arrowbot.tga")
