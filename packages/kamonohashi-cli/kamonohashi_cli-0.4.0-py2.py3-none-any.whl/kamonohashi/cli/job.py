...
def list_job(count, id, name, started_at, data_set, memo, status):
    """List jobs filtered by condition"""
    api = rest.JobApi(configuration.get_api_client())
    per_page = 1000
    command_args = {
        'id': id,
        'name': name,
...
def create(name, registry_image, registry_tag, data_set_id, entry_point,
    ...
    """Submit a new job"""
    api = rest.JobApi(configuration.get_api_client())
    container_image = rest.ComponentsContainerImageInputModel(image=registry_image, tag=registry_tag)
    git_model = rest.ComponentsGitCommitInputModel(branch=git_branch, commit_id=git_commit, owner=git_owner, repository=git_repository)
    option_dict = None
    if options is not None:
        option_dict = {}
        for option in options:
            option_dict[option[0]] = option[1]
    model = rest.JobApiModelsCreateInputModel(container_image=container_image, cpu=cpu, data_set_id=data_set_id, entry_point=entry_point,
...
def update(id, memo):
    """Update a job"""
    api = rest.JobApi(configuration.get_api_client())
    model = rest.JobApiModelsEditInputModel(memo=memo)
    result = api.update_job(id, model=model)
    print('updated', result.id)


@job.command('upload-file')
@click.argument('id', type=int)
...
@job.command('download-container-files')
@click.argument('id', type=int)
@click.option('-d', '--destination', type=click.Path(exists=True, file_okay=False), required=True,
              help='A path to the output files')
