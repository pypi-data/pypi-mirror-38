    ...
    def update(self):
        with self.__lock:
            ...
            if self.__first_update:
                ...
                _LOGGER.info("First update so getting data no matter what")
            elif (self.state == 'online' and
                self.drive.shift_state != "P"):
                should_update = self.__check_driving_interval()
                if should_update:
                    _LOGGER.info("Tesla is driving getting updates at {}".format(self.__drive_interval))
            elif self.state == 'online':
                should_update = self.__check_update_interval()
                if should_update:
                    _LOGGER.info("Tesla online getting updates at interval {}".format(self.__update_interval))
            else:
                should_update = self.__check_wake_interval()
                if should_update:
                    _LOGGER.info("Tesla not awake but waking up. Wake interval is  {}".format(self.__wake_interval))

            if should_update:
                _LOGGER.info("Getting updated data on tesla")
                self._update()
