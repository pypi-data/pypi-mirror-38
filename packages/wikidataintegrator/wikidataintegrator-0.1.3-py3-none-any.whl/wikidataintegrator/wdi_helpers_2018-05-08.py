...
def _execute_sparql_query_iter(query, endpoint, batch_size, total=None):
    """
    Execute sparql query iterating through results.
    Note, this expects a query from id_mapper, as it simply appends the offset, limit and order by commands.
    So it wont work with arbitary queries as these may already be present, and order by needs something to order by,
    ...
    results = []
    if total:
        num_pages = (int(total) // batch_size) + 1
        print("Query: {}/{}".format(page_count, num_pages))
    query = query + \
            """
            ORDER BY ?id
            OFFSET **offset**
            ...
            """
    while True:
        this_query = query.replace("**offset**", str(page_count * batch_size)). \
            replace("**page_size**", str(batch_size))
        print(this_query)
        this_results = WDItemEngine.execute_sparql_query(this_query, endpoint=endpoint)['results']['bindings']
...
def _id_mapper_count(prop, endpoint, filters=None):
    # same as id_mapper but returns the number of results
    ...
    count = int(results[0]['c']['value'])
    return count


def id_mapper(prop, filters=None, raise_on_duplicate=False, return_as_set=False, prefer_exact_match=False,
              endpoint='https://query.wikidata.org/sparql', batch_size=None, show_progress=False):
    """
    ...
    """
    if show_progress:
        total = _id_mapper_count(prop, filters=filters, endpoint=endpoint)
    ...
    if batch_size:
        results = _execute_sparql_query_iter(query, endpoint, batch_size, total=total)
    else:
        results = WDItemEngine.execute_sparql_query(query, endpoint=endpoint)['results']['bindings']
