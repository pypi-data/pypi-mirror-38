...
@click.option('--yes', '-y', default=False, is_flag=True, required=False, help='Force yes')
@click.pass_obj
def vault_config(backend, kitchen, inherit, service, url, token, prefix, private, inheritable, yes):
    """
    ...
    try:
        ...
        if not kitchen:
            if not yes:
                message = 'You are configuring default vault!. Are you sure you want to proceed? [yes/No]'
                confirm = raw_input(message)
                if confirm.lower() != 'yes':
                    return
...
def secret_write(backend, entry, yes, kitchen):
    ...
    try:
        ...
        if kitchen and not DKCloudCommandRunner.is_vault_config_writable(backend.dki, kitchen):
            raise click.ClickException('Vault is read only for kitchen %s.' % kitchen)
