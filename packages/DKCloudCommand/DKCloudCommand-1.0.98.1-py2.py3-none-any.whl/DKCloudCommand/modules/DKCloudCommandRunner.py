...
class DKCloudCommandRunner(object):
        ...
        if kitchen:
            ...
            if kitchen_output_message == '':
                ...
                if kitchen in secret_list_backend_response and secret_list_backend_response[kitchen]['error']:
                    kitchen_output_message = os.linesep + secret_list_backend_response[kitchen]['error-message'] + os.linesep
                else:
        ...
        for item in items:
            if item in payload:
                ...
                if 'error' in payload[item] and payload[item]['error']:
                    ...
                    return ' %s%s' % (error_message, os.linesep)
                ...
                if 'list' in payload[item]:
                    ...
                    for list_item in sorted_list:
                        override_message = ''
                        if opposite_item and 'list' in payload[opposite_item] and list_item in payload[opposite_item]['list']:
    ...
    def secret_exists(dk_api, path, kitchen_name):
        ...
        if 'list' in payload[item]:
            ...
            if complete_path in payload[item]['list']:
                return True, 'True (found in %s vault) %s' % (item_name, os.linesep)
    ...
    def is_vault_config_writable(dk_api, kitchen_name):
        ...
        if backend_response and 'config' in backend_response and kitchen_name in backend_response['config']:
            ...
            if 'default' == service: return False # is default
    ...
    def orderrun_detail(dk_api, kitchen, pd):
        ...
        if serving and DKCloudCommandRunner.LOGS in serving:
            s += '\n\nLOG\n'
            s += DKCloudCommandRunner._get_logs_from_serving(serving)

        rc.set_message(s)
        return rc

    @staticmethod
    def _get_logs_from_serving(serving):
        ret = ''
        if serving and DKCloudCommandRunner.LOGS in serving:
