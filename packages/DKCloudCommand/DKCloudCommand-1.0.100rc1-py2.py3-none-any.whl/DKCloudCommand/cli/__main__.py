...
# @click.option('--recipe', '-r', type=str, help='recipe name')
@click.pass_obj
def orderrun_detail(backend, kitchen, summary, nodestatus, runstatus, log, full_log, timing, test, all_things,
    ...
    try:
        ...
        check_and_print(DKCloudCommandRunner.orderrun_detail(backend.dki, use_kitchen, pd, full_log=full_log))
    except Exception as e:
        if not runstatus and not disp_chronos_id and not disp_mesos_id:
            raise click.ClickException(str(e))
        else:
