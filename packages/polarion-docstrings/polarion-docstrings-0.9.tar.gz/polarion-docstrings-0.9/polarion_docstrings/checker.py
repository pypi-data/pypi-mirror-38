...
ValidatedDocstring = namedtuple("ValidatedDocstring", "unknown invalid missing markers")
...
class DocstringsChecker(object):
    ...
    def get_invalid_fields(self, docstring_dict, valid_values):
        results = {
            key: self.validate_value(docstring_dict, key, valid_values) for key in valid_values
        }
        invalid = [
            FieldRecord(docstring_dict[key].lineno, docstring_dict[key].column, key)
    ...
    def validate_docstring(self, docstring_dict):
        ...
        unknown = self.get_unknown_fields(docstring_dict, cfg_docstrings.get("known_fields"))
        ...
        return ValidatedDocstring(unknown, invalid, missing, markers)

    # pylint:disable=too-many-arguments,too-many-locals
    def get_fields_errors(
        self, validated_docstring, docstring_dict, marker_fields, lineno=0, column=0
    ):
            ...
            if record.value:
                ...
                errors.extend(
                    self.get_fields_errors(
                        valdoc,
                        record.value,
                        self.config["docstrings"].get("marker_fields") or {},
                        record.lineno,
                        record.column,
                    )
                )
