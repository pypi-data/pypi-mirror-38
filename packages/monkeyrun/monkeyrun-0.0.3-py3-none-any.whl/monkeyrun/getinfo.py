...
import os
from .monkeyLib import get_aapt_command, get_adb_command

# from lib.mod_db import DataBase
...
class GetAndroidInfo(object):
    ...
    def get_error(self, path, pname):
        comm = "%s -s %s logcat -d \*:E | grep %s " % (get_adb_command(),
            self.devices, pname)  # .split(".")[2])
        p = self.run_comm(comm)
    ...
    def get_anr(self, path):
        comm = "%s -s %s logcat -v time -d | grep ANR" % (get_adb_command(),self.devices)
        p = self.run_comm(comm)
        result = p.stdout.read()
        result = self.transfer_content(str(result, "utf-8"))
        if len(result) != 0:
