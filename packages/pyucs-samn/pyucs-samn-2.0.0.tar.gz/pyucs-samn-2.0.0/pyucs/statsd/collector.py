...
class StatsCollector:
    ...
    def _query_stats(thread_args):
        ...
        if StatsCollector._get_device_type(device_chunk) == 'VnicEther':
            data = ucs.get_vnic_stats(vnic=device_chunk, ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'VnicFc':
            data = ucs.get_vhba_stats(vhba=device_chunk, ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'FcPortChannel':
            data = ucs.get_fabric_fcportchannel_stats(portchannel=device_chunk, type='Fc', ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'EtherPortChannel':
            data = ucs.get_fabric_etherportchannel_stats(portchannel=device_chunk, type='Ether', ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'FabricEtherPort':
            data = ucs.get_fabric_etherport_stats(port=device_chunk, ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'FabricFcPort':
            data = ucs.get_fabric_fcport_stats(port=device_chunk, ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'FabricStorage':
            data = ucs.get_system_storage_stats(storageitem=device_chunk, ignore_error=True)
        elif StatsCollector._get_device_type(device_chunk) == 'FabricKernel':
    ...
    def _get_device_type(data):
        ...
        if isinstance(data, VnicEther):
            return 'VnicEther'
        if isinstance(data, mometa.sw.SwSystemStatsHist.SwSystemStatsHist):
            return 'FabricKernel'
