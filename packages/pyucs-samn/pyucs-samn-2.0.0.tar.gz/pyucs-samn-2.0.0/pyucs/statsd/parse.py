...
class Parser:
    ...
    def _parse_data(self, data):
        ...
        try:
            if isinstance(data, FcPortChannelStat):
                json_series.append(self._prep_fc_port(data, 'FcPc'))
            if isinstance(data, EthPortChannelStat):
                json_series.append(self._prep_ether_port(data, 'EtherPc'))
            if isinstance(data, EthPortStat):
                json_series.append(self._prep_ether_port(data, 'EtherPort'))
            if isinstance(data, FcPortStat):
                json_series.append(self._prep_fc_port(data, 'FcPort'))
            if isinstance(data, mometa.storage.StorageItem.StorageItem):
                json_series.append(self._prep_system_storage(data))
            if isinstance(data, mometa.vnic.VnicFc.VnicFc):
                json_series.append(self._prep_vhba(data))
            if isinstance(data, mometa.vnic.VnicEther.VnicEther):
    ...
    def _get_storage_value(metric, data):
        ...
        if metric == 'storage.used.mb':
            return float(data.size) * (float(data.used) / 100)
        elif metric == 'storage.used.percent':
            return float(data.used)
        elif metric == 'storage.free.mb':
            return float(data.size) - (float(data.size) * (float(data.used) / 100))
        elif metric == 'storage.free.percent':
            return 100 - float(data.used)
