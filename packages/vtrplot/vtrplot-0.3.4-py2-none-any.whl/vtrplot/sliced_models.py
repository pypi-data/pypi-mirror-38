...
class SlicedModel_SEGY(object):
    ...
    def __init__(self, file_name, use_memmap=False):
        ...
        # 300ms base delay + 100ms randomness
        retry_delay_sec = 0.300 + (random.randint(1, 101) / 1000.0)

        f = None

        got_initial_headers = False
        ...
        while True:  # outer loop retry
            while (retry_count <= max_retries):
                ...
                if (last_sourceX > 0 or last_sourceY > 0) and not tried_source:
                    try:
                        # read segy file using these headers for inline/xline sorting and see if trace count is consistent
                        f = segyio.open(file_name, mode="r", strict=False, iline=TraceField.SourceX, xline=TraceField.SourceY)
                ...
                if not got_correct_headers and (last_inline3d > 0 or last_xline3d > 0) and not tried_inline:
                    try:
                        f = segyio.open(file_name, mode="r", strict=False, iline=TraceField.INLINE_3D, xline=TraceField.CROSSLINE_3D)
                        if len(f.ilines) * len(f.xlines) == trace_count:
                            got_correct_headers = True
                ...
                if not got_correct_headers and (last_recieverX > 0 or last_recieverY > 0) and not tried_group:
                    try:
                        f = segyio.open(file_name, mode="r", strict=False, iline=TraceField.GroupX, xline=TraceField.GroupY)
                        if len(f.ilines) * len(f.xlines) == trace_count:
                            got_correct_headers = True
                ...
                if not got_correct_headers and (last_cdpX > 0 or last_cdpY > 0) and not tried_cdp:
                    try:
                        f = segyio.open(file_name, mode="r", strict=False, iline=TraceField.CDP_X, xline=TraceField.CDP_Y)
                        if len(f.ilines) * len(f.xlines) == trace_count:
                            got_correct_headers = True
                ...
                if not got_correct_headers:
                    # deal with file not being available due to S3 errors
                    time.sleep(retry_delay_sec)
                    retry_count += 1
                    continue
                else:
