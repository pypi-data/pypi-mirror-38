...
class VTRImage(object):
    ...
    # @profile
    def _normalize_to_unit(self, input_array, ignore_min_range=False):
        """
        ...
        if self.max_val is None:
            ...
            if (not ignore_min_range) and \
               (self.min_range is not None) and \
    ...
    def _quantize_to_N(self, xa, N=256):
        ...
        # use more efficient representaiton if possible
        if (N <= 256):
    ...
    # @profile
    def to_image(self, model_propID=0, slice_dim=2, slice=-1, scale_factor=-1, quant_bits=8,
                 ignore_min_range=False, reset_minmax=True):
        """
        ...
        if reset_minmax:
            self.min_val = None
            self.max_val = None
        resampled_array = self._normalize_to_unit(resampled_array, ignore_min_range)

        # discretize to uint8 for image
        # rgba_array = self.to_rgba(resampled_array, bytes=True)
        discretized_array = self._quantize_to_N(resampled_array, N=2**quant_bits)
