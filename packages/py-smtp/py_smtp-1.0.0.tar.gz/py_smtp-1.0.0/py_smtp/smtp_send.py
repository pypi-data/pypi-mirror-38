...
def make_file_msg(file_names: []):
    ...
    for file_name in file_names:
        ...
        myemail.add_header('Content-Disposition', 'attachment; filename="{}"'.format(basename))  # 添加邮件头
        msg.attach(myemail)
    return msg


def smtp_send(host, sender, pwd: str, to: [], cc: [], subject: str, content: str, attach_files: [])->[]:
    """
    邮件发送
...
if __name__ == '__main__':
    ...
    # print('发送文件消息体')
    # msg = make_file_msg(['d:\Documents\_PY\daily_mail_settlement\daily\config.json'])
    # msg['From'] = SENDER
    # msg['To'] = ', '.join(RECIPS)
    # msg['Subject'] = '文件消息邮件'
    # msg.attach(make_mpa_msg('wujh test 测试'))
    # sendMsg(SENDER, RECIPS, msg.as_string())
