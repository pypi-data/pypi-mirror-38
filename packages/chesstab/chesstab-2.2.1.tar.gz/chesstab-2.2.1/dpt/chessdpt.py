...
class ChessDatabase(DPTapi):
    ...
    def adjust_database_for_retry_import(self, files):
        ...
        for dbn in self._broken_sizes.keys():
            self.get_dbsets()[dbn].increase_size_of_full_file(
                self.get_dbserv(),
                self.get_dbsets()[dbn].get_file_parameters(self.get_dbserv()),
    ...
    def open_context(self):
        ...
        for dbo in self.get_dbsets().values():
            fistat[dbo] = dbo.get_file_parameters(self.get_dbserv())['FISTAT']
        for dbo in self.get_dbsets().values():
            if fistat[dbo][0] != 0:
                break
        else:
    ...
    def delete_database(self):
        ...
        for k, v in self.get_dbsets().items():
            names.add(v._file)
    ...
    def open_after_import_without_backups(self, files=()):
        ...
        for dbn, dbo in self.get_dbsets().items():
            gfp = dbo.get_file_parameters(self.get_dbserv())
