...
from solentware_base.api.constants import (
    FILEDESC, BRECPPG, BSIZE, DSIZE, TABLE_B_SIZE)
    ...
    for f in file_records:
        if (FISTAT_DEFERRED_UPDATES !=
            cdb.get_database_instance(
                f, None).get_file_parameters(cdb.get_dbserv())['FISTAT'][0]):
            status = False
    cdb.close_context()
    return status
...
class ChessDatabaseDeferred(object):
    ...
    Subclasses must include a subclass of DPTbase as a superclass.
    ...
    def open_context(self, files=None):
        ...
        if oc:
            vr = self.get_dbserv().Core().GetViewerResetter()
            for dbo in self.get_dbsets().values():
                if vr.ViewAsInt(
                    'FISTAT',
    ...
    def get_archive_names(self, files=()):
        ...
        specs = {f for f in files if f in self.get_dbsets()}
        names = [v._file for k, v in self.get_dbsets().items() if k in specs]
    ...
    def get_pages_for_record_counts(self, counts=(0, 0)):
        ...
        brecppg = self.get_dbsets()[GAMES_FILE_DEF]._filedesc[BRECPPG]
        return (
            counts[0] // brecppg,
            (counts[1] * self.get_dbsets()[GAMES_FILE_DEF]._btod_factor
             ) // brecppg,
            )
        ...
        for k, v in self.get_database_parameters(
            ...
            message = ''.join(
                ('The free data size of the ',
                 ...
                 ' pages.'))
            if len(self.get_dbsets()[k].get_extents()) % 2 == 0:
                 message = ''.join(
                     (message,
            ...
            if dialogues.askyesno(
                ...
                increase_done = True
                self.get_dbsets()[k].get_database().Increase(bincrease, False)
        if increase_done:
    ...
    def _increase_index(self, event=None):
        ...
        for k, v in self.get_database_parameters(
            ...
            if dialogues.askyesno(
                ...
                increase_done = True
                self.get_dbsets()[k].get_database().Increase(dincrease, True)
        if increase_done:
            self._reporter.append_text(
                ' '.join(
                    ('Recalculation of planned database size increases',
