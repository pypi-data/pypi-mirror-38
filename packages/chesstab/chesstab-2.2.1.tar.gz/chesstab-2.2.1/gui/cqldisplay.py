    ...
    def on_partial_change(self, instance):
        ...
        if self.sourceobject is not None:
            ...
            if self is self.ui.partial_items.active_item:
                if key == self.sourceobject.key:
                    pds = self.ui.partial_games.datasource

                    # Do these two as separate transactions so the second can
                    # fail leaving recordset absent so one is calculated anyway
                    # by a later get_cql_statement_games() call even if the
                    # calculate_cql_statement_games() call fails.
                    pds.forget_cql_statement_games(self.sourceobject)
                    pds.calculate_cql_statement_games(value, self.sourceobject)

                    self._get_cql_statement_games_to_grid(value)

                    # value is an instance of a subclass of CQLStatement which
                    # adds no attributes, but does add database access related
                    # methods.
