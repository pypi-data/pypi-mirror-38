...
from solentware_misc.api import modulequery
...
class Chess(ChessException):
        ...
        **kargs - passed through to database object
        ...
        try:
            ...
            menu10101.add_command(
                label='Archive PGN',
                underline=0,
                command=self.try_command(self.archive_games_as_pgn, menu10101))
            menu10101.add_command(
                label='RAV PGN',
                underline=0,
    ...
    def __del__(self):
        ...
        if self.opendatabase:
            self.opendatabase.close_context()
            self.opendatabase = None
        ...
        if len(ed) == 0:
            dialogues.showinfo(
                message=''.join((
                    'Folder ',
                    os.path.basename(chessfolder),
                    ' does not contain a chess database')),
                title='Open',
                )
            return
        elif len(ed) > 1:
        ...
        if cs.is_ok():
            if self.opendatabase:
                options.save_options(
                    self.opendatabase.get_database_folder(),
                    cs.get_options())
    ...
    def database_delete(self):
        ...
        if dlg == tkinter.messagebox.YES:
            if self.opendatabase.delete_database():
                message = ''.join(
    ...
    def archive_games_as_pgn(self):
        """Export all games as a PGN file in reduced export format."""
        exporters.archive_games_as_pgn(
            self.opendatabase,
    ...
    def _database_import(self, pgnfiles):
        ...
        self.ui.set_import_subprocess(
            subprocess_id=self.opendatabase.do_deferred_updates(
                os.path.join(
    ...
    def _database_open(self, chessfolder):
        ...
        self.opendatabase = self._database_class(
            chessfolder,
            **self._chessdbkargs)
        if not self.opendatabase.open_context():
            self.opendatabase = None
            return
            ...
            if len(archives) == 0:
                ...
                if returncode != 0:
                    ...
                    self._tidy_up_after_import(
                        ...
                        names)
                    return
                action = self.opendatabase.open_after_import_without_backups(
                    files=(GAMES_FILE_DEF,))
                if action is None:
                    #Database full
    ...
    def _database_close(self):
        ...
        # Order of these two calls should not matter, and does not seem to, at
        # time of writing.
        #self.ui.set_open_database_and_engine_classes()
    ...
    def _retry_import(self, files):
        ...
        """
        self.opendatabase.open_context_allocated(files=files)
