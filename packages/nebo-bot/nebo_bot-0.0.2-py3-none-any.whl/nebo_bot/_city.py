...
class City:
    ...
    def go_to_city(self, city_name):
        ...
        if re.search(city_name, self.current_city_name, re.IGNORECASE):
            self.bot.handler_send_message.bot_exists_in_city()
            return True
        else:
            self.exit_with_city()
        ...
        while True:
            if self.bot.get_bs.find('span', text=re.compile(city_name, re.IGNORECASE)):
                ...
                self.bot.handler_send_message.bot_in_city()
                return True
            else:
                ...
                if not _to_notify_invite:
                    self.bot.handler_send_message.bot_wait_your_invite()
                time.sleep(2)
...
class Work:
    ...
    def run_invite(self, count_invite=50, from_lvl=10, to_lvl=80, from_day=0, to_day=-1):
        ...
        while True:
            if not self.is_run:
                self.bot.handler_send_message.bot_canceled()
