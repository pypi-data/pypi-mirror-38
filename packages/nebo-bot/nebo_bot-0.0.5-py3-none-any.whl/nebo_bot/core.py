...
class BotCore(object):
    ...
    def __init__(self, handler_send_message=None, debug=False, stop_signal_handler=lambda _: False, **data):
        self.logging = logging.basicConfig(level=logging.DEBUG if debug is True else logging.INFO)
    ...
    def stop(self):
        if self._count_iter_stop_handler >= self.iter_check_stop_handler:
            self._count_iter_stop_handler = 0
            if self.stop_signal_handler(self.data):
                return True
