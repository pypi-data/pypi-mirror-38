    ...
    def get_cost(self):
        ...
        while True:
            if not self.bot.press('Собрать выручку'):
                self.bot.handler_send_message.finished_get_cost(self)
    ...
    def get_delivery(self):
        ...
        while True:
            if not self.bot.press('Выложить товар'):
                self.bot.handler_send_message.finished_get_delivery(self)
        ...
        while True:
            if self.bot.press(text='Закупить товар'):
                if self.bot.press(href='product[ABC]:emptyState:action:link::ILinkListener::'):
                    ...
                    continue
            self.bot.handler_send_message.finished_get_buy_products(self)
    ...
    def lift(self):
        ...
        while True:
            ...
            if self.bot.press(href='lift/wicket:interface/:\d+:liftState:tipsLink:link::ILinkListener::'):
                ...
                self.bot.handler_send_message.lift_visitor_up(self)
                continue
            try:
            ...
            self.bot.handler_send_message.lift_finished(self)
            return FINISHED_LIFT_WORK

    def open_floors(self):
        self.bot.get('home')
        while True:
            if not self.bot.press(href=':majorFloorsBlock:majorFloorsPanel', text='Открыть этаж!'):
                return OPEN_ALL_NEW_FLOORS
            self.bot.handler_send_message.open_new_floor(self)
