...
class City:
    ...
    def go_to_city(self, city_name):
        ...
        if re.search(city_name, self.current_city_name, re.IGNORECASE):
            self.bot.handler_send_message.bot_exists_in_city(self)
            return True
        else:
            self.exit_with_city()
        ...
        while True:
            if self.bot.get_bs.find('span', text=re.compile(city_name, re.IGNORECASE)):
                ...
                self.bot.handler_send_message.bot_in_city(self)
                return True
            else:
                ...
                if not _to_notify_invite:
                    self.bot.handler_send_message.bot_wait_your_invite(self)
                time.sleep(2)
...
class Work:
    ...
    def run_invite(self, count_invite=50, from_lvl=10, to_lvl=80, from_day=0, to_day=-1):
        ...
        if not self.accept_to_invite:
            self.bot.handler_send_message.request_give_post(self)
            if self._wait_give_post() == DOES_NOT_GAVE_POST:
                self.bot.handler_send_message.time_expired_wait_post(self)
                return DOES_NOT_GAVE_POST
        ...
        while True:
            if not self.is_run:
                self.bot.handler_send_message.bot_canceled(self)
                return CANCEL_INVITING
            if self.count_invite >= count_invite:
                self.bot.handler_send_message.finish_inivte(self)
