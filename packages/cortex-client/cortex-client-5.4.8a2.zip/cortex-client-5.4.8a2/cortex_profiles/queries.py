...
from . import utils
from .datamodels import ProfileAttribute, FindQuery
from .datamodels import ProfileCommit
...
def derive_operations_to_unmark_changed_attributes_as_latest(oldCommit: ProfileCommit, newCommit: ProfileCommit) -> list:
    # We also need to deprectate the old modified values
    return [
        UpdateMany(
            query_to_find_attributes_associated_with_commit_by_key(
                oldCommit, [attr.attributeKey for attr in newCommit.attributesModified]
            ),
...
def derive_operations_to_save_modified_attributes_and_associate_them_with_latest_commit(attributes: List[ProfileAttribute], newCommit: ProfileCommit):
    ...
    new_modified_attrs = [
        utils.modify_named_tuple(
            attributes_map[attr.attributeId],  # Get the new attribute that we are modifying ..
            {
        ...
        for attr in newCommit.attributesModified
    ]

    # Operatins to insert all the newly modified values ...
    return [
        ReplaceOne(
            {"id": new_mod_attr.id},
            utils.namedtuple_asdict(new_mod_attr),
            upsert=True
        )
...
def derive_operations_to_save_new_attributes_and_associate_them_with_latest_commit(
    ...
    return [
        ReplaceOne(
            ...
            utils.namedtuple_asdict(
                utils.modify_named_tuple(
                    attributes_map[attr.attributeId],
                    {
                        "onLatestProfile": True,
                        "commits": [newCommit.id],
                        "tenantId": newCommit.tenantId,
        ...
        for attr in newCommit.attributesAdded
    ]
...
def query_to_find_stale_attributes_from_old_commit_that_need_to_bo_associated_with_new_commit(oldCommit:ProfileCommit, newCommit: ProfileCommit) -> FindQuery:
    ...
    added_attribute_ids = [attr.attributeId for attr in (newCommit.attributesAdded + newCommit.attributesModified)]
    # Attributes that are being removed ... thus they dont need to be carried over ...
    modified_attribute_keys = [attr.attributeKey for attr in (newCommit.attributesAdded + newCommit.attributesModified)]
    return {
        "$and": [
            ...
            # Attributes are not being removed ...
            {
                "attributeKey": {
                    "$nin": modified_attribute_keys
                }
            }
        ]
    }
...
def extract_profile_commit_from_mongo_payload(query_result: dict) -> ProfileCommit:
    returnval = ProfileCommit(**{
        f: query_result.get(f) for f in ProfileCommit._fields
    })
    return returnval