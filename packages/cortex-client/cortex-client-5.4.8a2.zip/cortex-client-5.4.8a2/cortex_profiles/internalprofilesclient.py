...
from cortex_profiles import datamodels
from cortex_profiles import profile_commit_utils
from cortex_profiles import queries
from cortex_profiles import schemas
from cortex_profiles import utils
from cortex_profiles.datamodels import AggregateQuery, LatestCommitPointer, ProfileSnapshot
from cortex_profiles.datamodels import ProfileCommit, Profile, ProfileAttribute
...
class InternalProfilesClient(_Client):
    ...
    def __init__(self, url:str, version:str, token:str, environmentId:str="cortex/default"):
        ...
        # databases here ...
        self.mongoclient = pymongo.MongoClient(self.secretsclient.get_secret(base64.b64decode(b'ZnBp').decode("utf-8"))["mongo_uri"])
    ...
    def append_attributes_to_profile(self, attributes: List[ProfileAttribute], profileId: str) -> Tuple[
        ...
        # helpers.print_json_with_header("Default Commit: ", helpers.namedtuple_asdict(default_commit))

        latest_commit = self.find_latest_commit_for_profile_or_create(profileId, default_commit)
        # helpers.print_json_with_header("Latest Commit: ", helpers.namedtuple_asdict(latest_commit))

        # Get Profile from latest commit
        latest_profile = self.get_profile_based_off_commitId(latest_commit.id)
        # helpers.print_json_with_header("Latest Profile: ", helpers.namedtuple_asdict(latest_profile))
        ...
        # helpers.print_named_tuple_with_header("attributes_added: ", attributes_added)
        # helpers.print_named_tuple_with_header("attributes_modified: ", attributes_modified)
        # helpers.print_named_tuple_with_header("attributes_removed: ", attributes_removed)
        ...
        # helpers.print_named_tuple_with_header("new_commit: ", new_commit)

        # Save newest commit
        saved_commit = self.save_profile_commit(new_commit)
        # helpers.print_named_tuple_with_header("saved_commit: ", saved_commit)
        ...
        # helpers.print_named_tuple_with_header("latest_commit_pointer: ", latest_commit_pointer)

        # Rebuild the latest profile from the latest commits ...
        latest_profile = self.get_latest_profile(profileId)
        # helpers.print_named_tuple_with_header("latest_profile: ", latest_profile)
        ...
        # helpers.print_named_tuple_with_header("expanded_profile: ", expanded_profile)
    ...
    def get_latest_profile(self, profileId:str) -> Optional[Profile]:
        ...
        # helpers.print_named_tuple_with_header("Latest profile commit: ", commit)
        if not commit:
            return None
    ...
    def expand_profile(self, profile:Profile) -> ProfileSnapshot:
        return ProfileSnapshot(**utils.merge_dicts(
            utils.namedtuple_asdict(profile),
            {
    ...
    def build_profile_from_commits(self, commits:OrderedList[ProfileCommit], commitId:str) -> Optional[Profile]:
        if not commits:
            return None
        # helpers.print_named_tuple_with_header("Applying commits", commits)
    ...
    def find_latest_attributes_for_profile(self, profileId: str, attributeKeys: Optional[str]) -> List[ProfileAttribute]:
        ...
        return list(map(
            lambda x: datamodels.load_profile_attribute_from_dict(utils.drop_from_dict(x, ["_id"])),
    ...
    def find_latest_commit_for_profile_or_create(self, profileId:str, default_commit:ProfileCommit) -> ProfileCommit :
        ...
        pointer = LatestCommitPointer(
            **utils.drop_from_dict(
                queries.find_or_create_atomically(
                    ...
                    utils.namedtuple_asdict(self.dervive_latest_commit_pointer_from_commit(default_commit)),
                    {
        ...
        commit = queries.find_or_create_atomically(
            ...
            utils.namedtuple_asdict(default_commit),
            {
                "sort": [('createdAt', pymongo.DESCENDING)],
                "upsert": True,
                "return_document": pymongo.ReturnDocument.AFTER
    ...
    def save_profile_commit(self, commit: ProfileCommit) -> ProfileCommit:
        safe_commit = utils.modify_named_tuple(commit, self.append_secure_variables_to_query({}))
        saved_commit = self.commits_collection.find_one_and_replace(
            {"id": safe_commit.id},
            utils.namedtuple_asdict(safe_commit),
            **{
                "upsert": True,
                "return_document": pymongo.ReturnDocument.AFTER
            }
    ...
    def replace_latest_snapshot_for_profile(self, snapshot:ProfileSnapshot) -> ProfileSnapshot:
        return utils.drop_from_dict(
            self.snapshots_collection.find_one_and_replace(
                ...
                utils.namedtuple_asdict(snapshot),
                **{
                    "upsert": True,
                    "return_document": pymongo.ReturnDocument.AFTER
                }
    ...
    def get_attributes_associated_with_profile(self, profile: Profile) -> List[ProfileAttribute]:
        return list(map(
            ...
            self.attributes_collection.find(
                self.append_secure_variables_to_query({
                    "id": {"$in": [attr.attributeId for attr in profile.attributes]}
                })
            )
        ))
    ...
    def update_latest_commit_pointer(self, fromCommit:ProfileCommit, toCommit:ProfileCommit) -> LatestCommitPointer:
        ...
        saved_pointer = self.latest_commit_pointers_collection.find_one_and_replace(
            ...
            utils.namedtuple_asdict(pointer),
            **{
                "upsert": True,
                "return_document": pymongo.ReturnDocument.AFTER
            }
