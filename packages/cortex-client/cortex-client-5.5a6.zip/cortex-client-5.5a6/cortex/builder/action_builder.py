...
class ActionBuilder:
    ...
    def _build_function(self, dry_run=False):
        if self._code_s:
            ...
            if not dry_run:
                ...
                self._deploy_function_image(self._code_s, self._fn_name or 'main', '')
    ...
    def _build_daemon(self, dry_run=False):
        if self._code_s:
            if not dry_run:
                ...
                self._deploy_daemon_image(self._code_s, self._fn_name or 'main', '')
        elif self._model:
            # Deploy from ML model object
            self._deploy_model(dry_run)
        else:
    ...
    def _build_job(self, dry_run=False, **kwargs):
        if self._code_s:
            if not dry_run:
                # Build new docker image with the given action source code
                self._deploy_job_image(self._code_s, self._fn_name or 'main', '', **kwargs)
        elif self._model:
            # Deploy from ML model object
