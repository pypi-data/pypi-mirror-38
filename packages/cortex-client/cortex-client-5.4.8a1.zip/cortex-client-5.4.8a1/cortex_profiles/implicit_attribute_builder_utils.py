...
from .schemas import COUNT_OF_TAG_SPECIFIC_INTERACTIONS_COL, TIMES_SPENT_ON_TAG_SPECIFIC_INTERACTIONS_COL, \
    LOGIN_COUNTS_COL, LOGIN_DURATIONS_COL, INSIGHT_COLS, INTERACTIONS_COLS, INTERACTIONS, SESSIONS_COLS, \
    DAILY_LOGIN_COUNTS_COL, DAILY_LOGIN_DURATIONS_COL
from .utils import derive_day_from_date, derive_hour_from_date
...
def derive_count_of_insights_per_interactionType_per_relatedConcepts_per_profile(
    ...
    filtered_interactions_with_tags = expanded_insight_interactions_df[
        expanded_insight_interactions_df[COUNT_OF_TAG_SPECIFIC_INTERACTIONS_COL.TAGGEDCONCEPTRELATIONSHIP] == schemas.CONTEXTS.INSIGHT_TAG_RELATED_TO_RELATIONSHIP
    ]
...
def prepare_interactions_per_tag_with_times(insight_interactions_df:pd.DataFrame, insights_df:pd.DataFrame) -> pd.DataFrame :
    ...
    interactions_about_related_concepts_mask = expanded_insight_interactions_df[TIMES_SPENT_ON_TAG_SPECIFIC_INTERACTIONS_COL.TAGGEDCONCEPTRELATIONSHIP] == schemas.CONTEXTS.INSIGHT_TAG_RELATED_TO_RELATIONSHIP
...
def append_interaction_time_to_df_from_properties(df:pd.DataFrame) -> pd.DataFrame:
    return df.assign(**{
        INTERACTIONS.STARTED_INTERACTION: df["properties"].map(
            lambda x: x.get(INTERACTIONS.STARTED_INTERACTION)),
        INTERACTIONS.STOPPED_INTERACTION: df["properties"].map(
            lambda x: x.get(INTERACTIONS.STOPPED_INTERACTION)),
    })
