...
def derive_dimensional_observed_attributes_from_df(
        ...
        contextOfDimension:str, contextOfDimensionValue:str,
        isColumnForDimensionContext:bool=True,
        profileType:str=schemas.CONTEXTS.END_USER_PROFILE
    ) -> List[datamodels.ObservedProfileAttribute]:
    return [
        derive_observed_dimensional_attributes_from_grouped_df(
            ...
            contextOfDimension, contextOfDimensionValue, isColumnForDimensionContext, profileType)
        for gid, gdf in df.groupby(attribute_identifiers, as_index=False)
    ]


def derive_observed_dimensional_attributes_from_grouped_df(grouped_df:pd.DataFrame, group_id_keys:List, group_id_values:List, attributeKeyPattern:str,
        dimensionIDColumn:str, dimensionValueColumn:str, contextOfDimension:str, contextOfDimensionValue:str,
        isColumnForDimensionContext:bool=False, profileType:str=schemas.CONTEXTS.END_USER_PROFILE) -> datamodels.ObservedProfileAttribute:
    if grouped_df.empty:
        return None
    assert "profileId" in grouped_df.columns, "ProfileId must be in dataframe ..."
    identifier = dict(zip(group_id_keys, group_id_values))
    return datamodels.ObservedProfileAttribute(
        id=str(uuid.uuid4()),
        ...
        attributeValue=datamodels.DimensionalAttributeContent(
            ...
            value=sorted(
                [
                    datamodels.Dimension(dimensionId=x, dimensionValue=y)
...
def derive_simple_observed_attributes_from_grouped_df(
        ...
        profileType:str=schemas.CONTEXTS.END_USER_PROFILE) -> datamodels.ObservedProfileAttribute:
    if grouped_df.empty:
        return []
...
def derive_declared_attributes_from_key_value_df(
        ...
        keyColumn:str="key", valueColumn:str="value", attributeValueClass:type=datamodels.ObjectValue,
        profileType:str=schemas.CONTEXTS.END_USER_PROFILE
    ) -> List[datamodels.DeclaredProfileAttribute]:
    return [
        datamodels.DeclaredProfileAttribute(
            id=utils.unique_id(),
...
def derive_declared_attributes_from_value_only_df(
        ...
        attributeValueClass:type=datamodels.ObjectValue,
        profileType:str=schemas.CONTEXTS.END_USER_PROFILE
    ) -> List[datamodels.DeclaredProfileAttribute]:
    return [
        datamodels.DeclaredProfileAttribute(
