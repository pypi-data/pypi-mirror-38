...
class UniS3TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.storage = unis3.S3Storage(self.bucket)
    ...
    def test_as_path(self):
        scheme = 'https'
        netloc = 's3.example.com'
        path = 'some/path.ext'
        params = ''
        query = 'AWSAccessKeyId=MyHands4re%WritingWords%3D&Expires=71370537557'
        fragment = ''
        self.assertEqual(path, self.storage.as_path(path))
        url = '%s://%s/%s/%s?%s' % (scheme, netloc, self.bucket, path, query)
        self.assertEqual(path, self.storage.as_path(url))
        url = '%s://%s/%s/%s' % (scheme, netloc, self.bucket, path)
        self.assertEqual(path, self.storage.as_path(url))
    ...
    def test_reupload(self):
        ...
        self.storage.upload(self.src_path)
        self.storage.upload(self.tst_path)
        self.storage.upload(self.ext_path)
        self.assertEqual(len(self.storage.listdir()[1]), 3)
    ...
    def test_read(self):
        with open(self.src_path) as f:
            before = f.read()
        with self.storage.open(self.src_url, 'r') as f:
            after = f.read()
        self.assertEqual(after, before)

    def test_read_with_S3FileOpener(self):
        with open(self.src_path) as f:
            before = f.read()
        url = self.storage.client.generate_url(self.src_url)
    ...
    def test_overwrite(self):
        ...
        with self.storage.open(url, 'r') as f:
            data = f.read()
