...
#
# @return updated cmd with mono and full path to exe
####
def MonoWrap(cmd):
    cmdlist = cmd.split(' ')
    file = cmdlist[0]
    for a in os.getenv("PATH").split(os.pathsep):
        ...
        if os.path.isfile(os.path.join(a, file)):
            cmdlist[0] = os.path.join(a, file)
            logging.debug("File was found on the path: %s" % cmdlist[0])
            break
    cmdlist = ["mono"] + cmdlist
...
def RunCmd(cmd, capture=True, workingdir=None, outfile=None, outstream=None):
    if (cmd.split(' ')[0].endswith(".exe")) and (os.name == "posix"):
        cmd = MonoWrap(cmd)
