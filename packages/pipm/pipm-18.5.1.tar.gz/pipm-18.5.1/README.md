...
# TODOs:
 ...
 - pipm update - should update all dependencies according to the version specification in setup.cfg
 - a `sync` command to update requirements.txt files as in setup.cfg - or this should be done automatically
 - pip u <pkg> should be checked in all sections. Now it is duplicated if not correct --dev and --test flag not given
 - pip update - should support pattern flat and --latest flag
