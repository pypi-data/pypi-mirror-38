...
def create_dataset(ctx, account_uid, home, curr_dir = None):
    ...
    if not vc.is_uid_valid(dataset_uid):
        ...
        prompt_msg = 'Please enter a valid dataset name.'
        prompt_msg += '\nSuggested alternative: "{alt}"'.format(alt=suggested)
        prompt_msg += '\nPress enter to use suggested value.'
        dataset_uid = click.prompt(prompt_msg,default='')
    ...
    # The original directory path has been verified and possibly updated
    # Need to update it here so that directory creation will be correct
    head_tail = os.path.split(home)
    home = head_tail[0] + os.sep + dataset_uid

    # If code is being called by datasets_init(), use the current directory
    # for saving the DATASET file. Use the dataset_uid for the remote name.
...
def datasets_init(ctx):
    ...
    if not vc.is_uid_valid(os.path.basename(home)):
        suggested = os.path.basename(home).lower()
        suggested = suggested.replace('_','-')
        prompt_msg = 'Please enter a valid dataset name.'
        prompt_msg += '\nSuggested alternative: "{alt}"'.format(alt=suggested)
        prompt_msg += '\nPress enter to use suggested value.'
    ...
    if create_dataset(ctx, None, home, os.getcwd()):
...
def datasets_create(ctx, name):
    ...
    outcome =  create_dataset(ctx, None, home)
...
@click.pass_context
@login_required
def datasets_push(ctx,message, name,quiet):
   general_push(ctx,message, name,quiet)

def datasets_clone(ctx, path, directory, suppress_output=False):
...
def general_push(ctx,comment='',name='',suppress_output=False):
    files = glob.glob("*")
    if len(files) < 1:
...
def datasets_download(ctx, path, directory,suppress_output=False):
    if directory is None or directory == '.':
        home = os.getcwd()
    else:
        home = os.path.join(os.getcwd(), directory)

    code = datasets_clone(ctx, path, home,suppress_output=suppress_output)
    if code != 0:
