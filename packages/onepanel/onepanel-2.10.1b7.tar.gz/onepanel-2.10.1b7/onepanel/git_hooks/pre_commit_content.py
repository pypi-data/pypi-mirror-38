...
def check_for_large_files():
    ...
    try:
        ...
        if len(too_large_files) > 0:
            print("The total size of your commits are too large.")
            print("Use Onepanel datasets to store datasets or any files with combined size larger than 5.0 MB.")
            print("Use \"git reset HEAD <file>\" to unstage large files and try your commit.")
        ...
        if (kib_val * units + total_size_of_commit) >= max_repo_size * units:
            print("The total size of your repository is too large, greater than or equal to {repo_max_size}.".format(repo_max_size=sizeof_fmt(max_repo_size * units)))
