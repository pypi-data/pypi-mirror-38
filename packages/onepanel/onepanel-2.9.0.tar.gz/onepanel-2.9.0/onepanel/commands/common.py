...
@click.pass_context
@login_required
def clone(ctx, path, directory, include, exclude,quiet):
    entity_type = get_entity_type(path)
    if entity_type == 'project':
        projects_clone(ctx, path, directory, include, exclude)
    elif entity_type == 'dataset':
        datasets_clone(ctx, path, directory, include, exclude,quiet)
    else:
...
@click.pass_context
@login_required
def download(ctx, path, directory,archive,quiet):
    entity_type = get_entity_type(path)
    if entity_type == 'dataset':
        datasets_download(ctx, path, directory,quiet)
...
@click.pass_context
@login_required
def push(ctx, message, name,quiet):
    home = os.getcwd()
