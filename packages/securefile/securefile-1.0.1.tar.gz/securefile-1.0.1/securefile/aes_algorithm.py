...
---------------------------------------------
The more popular and widely adopted symmetric encrypt algorithm
...
against exhaustive key search attack. Triple DES was designed
to overcome this drawback but it was found slow.

The Advanced Encryption Standard (AES), also known by its original
name Rijndael, is a specification for the encrypt of electronic
...
class AES:
    ...
    Encrypt data with your key ::
        ...
        cyphertext = aes.encrypt('00112233445566778899aabbccddeeff', key)
    ...
    def __init__(self, mode, input_type, iv=None):
        ...
        __init__()
        --------------
        :param exb|cbc mode: mode
        :param text|hex|binary|data input_type: type of input
        :param text iv: iv
        """
        self.mode = mode
    ...
    def RevertStateMatrix(state):
        ...
        return ''.join(''.join([columns[0][x], columns[1][x],
                                columns[2][x],
                                columns[3][x]]) for x in range(4))
    ...
    def ShiftRows(self, state, isInv):
        ...
        offset = 0
        if isInv:
            state = re.findall('.' * 2, self.RevertStateMatrix(state))
    ...
    def MixColumns(self, state, isInv):
        ...
        if isInv:
            fixed = [14, 9, 13, 11]
            state = self.StateMatrix(''.join(state))
        else:
            fixed = [2, 1, 1, 3]
        columns = [state[x:x + 4] for x in range(0, 16, 4)]
        row = [0, 3, 2, 1]
    ...
    def key_handler(self, key, isInv):
        ...
        # Raise error on invalid key size
            ...
            raise AssertionError(
                "%s Is an invalid Key!\nUse a 128-bit, 192-bit or 256-bit key!"+
                "\nExample : 000102030405060708090a0b0c0d0e0f" % key)
        # Return the expanded key
        if not isInv:
            return self.ExpandKey(key)
        # Return the inverse expanded key
        if isInv:
            return [re.findall('.' * 2, self.RevertStateMatrix(x)
                               ) for x in self.ExpandKey(key)]

    def aes_main(self, data, key, isInv):
        """ Handle encrypt and decrypt modes
    ...
    def cbc(self, data, expanded_key, isInv):
        ...
        if self.input is 'hex':
            ...
            if not isInv:
                ...
                return ''.join([self.xor(
                    self.InvCipher(expanded_key, data[x]),
                    last[x]) for x in range(len(data))])
        elif self.input is 'data':
            if not isInv:
                data = re.findall('.' * 32,
                                  binascii.hexlify(self.pad(data)).decode()); blocks = [self.iv]
                [blocks.append(self.Cipher(expanded_key, self.xor(blocks[-1], x))) for x in data]
                return b''.join(binascii.unhexlify(x.encode()) for x in blocks[1:])
            elif isInv:
                data = re.findall(
                    '.' * 32, binascii.hexlify(data).decode()); last = [self.iv] + data
    ...
    def ecb(self, data, expanded_key, isInv):
        ...
        # Encrypt an text string
            ...
            if not isInv:
                return self.Cipher(expanded_key, ''.join(
                    '%02x' % x for x in self.pad(data.encode())))
            elif isInv:
                return str(self.unpad(
                    binascii.unhexlify(self.InvCipher(expanded_key, data).encode())))[2:-1]
        # Encrypt a stream of binary data
        elif self.input is 'data':
            if not isInv:
                return b''.join(binascii.unhexlify(self.Cipher(
                    expanded_key,
                    str(binascii.hexlify(x))[2:-1]).encode()) for x in self.unblock(data))
            if isInv:
