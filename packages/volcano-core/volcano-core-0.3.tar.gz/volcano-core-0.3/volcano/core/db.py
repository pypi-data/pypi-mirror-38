...
from volcano.general.xml_reader import LoadException, load_xml_file_le, XmlReader, get_node_location

from .expr import Expr


class Tag:  # pylint: disable=too-many-instance-attributes
    def __init__(self, parent_n: ('Tag', None), short_name: str, tag_id: int, var: variant.Variant = None):
        ...
        if not self.var_.is_void():
            self.quality = Quality.QUALITY_NOT_INIT
...
class Db:
    ...
    def create_tag(self, parent_tag_n: (Tag, None), short_name: str, var: variant.Variant = None) -> Tag:
        assert parent_tag_n is None or isinstance(parent_tag_n, Tag), parent_tag_n
        assert isinstance(short_name, str), short_name
        assert var is None or isinstance(var, variant.Variant), var

        tag_id = len(self.all_tags_list_) + 1
        tag = Tag(parent_tag_n, short_name, tag_id, var)
        if parent_tag_n:
    ...
    def create_tag_by_long_name(self, parent_tag_n: (Tag, None), long_name: str, var: variant.Variant = None) -> Tag:
        ...
        if '.' in long_name:  # short name
            ...
            return self.create_tag(my_parent_tag, self_name, var)

        return self.create_tag(parent_tag_n, long_name, var)
