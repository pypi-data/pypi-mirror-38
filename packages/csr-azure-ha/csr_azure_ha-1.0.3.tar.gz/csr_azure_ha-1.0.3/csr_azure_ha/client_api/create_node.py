...
def main(argv):
    ...
    if cmd_string:
        ...
        if ((params_dict.has_key('index')) and
            ...
            (params_dict.has_key('routeTable')) and
            (params_dict.has_key('nextHopIpAddress'))) :
            ...
            if not (params_dict.has_key('nextHopIpAddress')):
                print "create_node: missing required parameter -n nextHopIpAddress"
                dbg.log(debug_fh, 'ERR', "client: create_node: missing required parameter -n nextHopIpAddress")
            debug_fh.close()
            ha_api.show_usage(argv[0])
            return 4
