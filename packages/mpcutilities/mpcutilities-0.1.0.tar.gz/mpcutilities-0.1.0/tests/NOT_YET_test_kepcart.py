...
def test_kepState2cartPV():
    ...
    # Use elements2cartPV to calculate the posn & velocity arrays
    print("types", type(KSA), type(KSA[0]))
    XYZ, UVW = kc.kepState2cartPV( PHYS.GMsun , KSA)

    # Test datatype & accuracy of returned cartesian posn & velocity vectors
    assert( len(KSA) == N)
    assert(  isinstance(KSA[0], Classes.Elements ) )
    for n in range(N):
        assert(  np.all(
...
def test_cartesian_elements():
    ...
    '''
    ...
    # To be constructed once the above test_kepState2cartPV is working ...
                        
"""



# (ii) Tests of the accuracy of the kepcart package
# ----------------------------------------------------------
