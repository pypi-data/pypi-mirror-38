...
def gen_ssl_key(name):
    ...
    False: Could not change permissions.
    """
    key = crypto.PKey()
    key.generate_key(crypto.TYPE_RSA, 2048)
    with open(targetd_directory + name + ".pem", "w") as ssl_key:
        ssl_key.write(crypto.dump_privatekey(crypto.FILETYPE_PEM, key).decode())

    cmd = "chmod 600 %s" % (targetd_directory + name + ".pem")
    ret = run(cmd)
    if ret != 0:
...
def gen_ssl_cert(name, key_name):
    """
    Generates certificate and exports it as ".pem" file. Function only for testing!
    :param name: Name for the export file.
    :param key_name: Name of a private key to use.
    :return:
    True: Certificate exported successfully.
    False: Provided key does not exist or Could not change permissions.
    """
    if not os.path.isfile(targetd_directory + key_name + ".pem"):
        _print("FAIL: Key does not exist !")
        return False
    with open(targetd_directory + key_name + ".pem", "rb") as pem:
        key = pem.read()

    pkey = crypto.load_privatekey(crypto.FILETYPE_PEM, key)
    req = crypto.X509Req()
    req.get_subject().CN = "CZ"
    req.sign(pkey, "sha1")
    ...
    with open(targetd_directory + name + ".pem", "w") as ssl_cert:
        ssl_cert.write(crypto.dump_certificate(crypto.FILETYPE_PEM, cert).decode())
