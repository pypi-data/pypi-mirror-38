...
def ps(args):
    ...
    # convert name to id, as expected by endpoint
    if args.configuration:
        configuration = client.get_configuration(args.user, args.configuration)
        kwargs['configuration'] = configuration['_id']
    ...
    for run in client.list_runs(**kwargs):
        ...
        created = run['created']['$date'] if 'created' in run else ''
        started = run['started']['$date'] if 'started' in run else ''
        finished = run['finished']['$date'] if 'finished' in run else ''
