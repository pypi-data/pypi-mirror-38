...
def do_run(args):
    ...
    run = client.start_run(args.user, args.configuration, parameters=parameters)

    def on_data(msg, sess):  # source, generated, msg
        if not msg['debug']:
            print('{} [{}] {}'.format(msg['generated']['$date'], msg['source'], msg['msg']))
    ...
    if args.follow:
        ...
        if content:
            print(content)
        subscription = client.follow_logs(run['_id'], on_data=on_data)
        subscription.result()
    else:
        print(run['_id'])
...
def configure_parser(parser):
    ...
    parser.add_argument('-f', '--follow', action='store_true', help=argparse.SUPPRESS)  # Follow log output
