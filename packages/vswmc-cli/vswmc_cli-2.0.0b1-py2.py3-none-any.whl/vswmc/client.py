...
class VswmcClient(object):
    ...
    def list_configurations(self, user):
        path = '{}/configurations/{}'.format(self.api_root, user)
        return self._request('get', path=path).json()

    def get_configuration(self, user, name):
        path = '{}/configurations/{}/{}'.format(self.api_root, user, name)
        return self._request('get', path=path).json()

    def delete_configuration(self, user, name):
        path = '{}/configurations/{}/{}'.format(self.api_root, user, name)
        self._request('delete', path=path)

    def list_runs(self, configuration=None, status=None):
    ...
    def start_run(self, user, configuration, parameters=None):
        path = '{}/configurations/{}/{}/runs'.format(self.api_root, user, configuration)
        data = {
    ...
    def download_logs(self, run, debug=False):
        path = '{}/runs/{}/log'.format(self.api_root, run)
        params = {'raw': 'yes'}
        if debug:
            params['debug'] = 'yes'
        return self._request('get', path=path, params=params).content
