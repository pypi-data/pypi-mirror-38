...
Except_komoran_pos = ['VX','VCN', 'VCP','NA','MAG', 'XR', 'VA', 'NP', 'VV','VA', 'NR']
Except_komoran_pos_num = Except_komoran_pos.copy()
Except_komoran_pos_num.append('SN')
Extra_pos = ['VA','EC','VCN', 'VCP','JX','XSA','JKB','JKO', 'JKG', 'XSN','XSV', 'VV']

ADV_pos = ['MM','MAG', 'MAJ']
Pronouns = ['나', '너', '우리', '그', '그녀', '그대', '그이', '누구', '이분', '저분', '그분', '그들', '그녀들', '그대들', '우리들']
Proper_Nouns = ['갑질', '양치질', '한반도','구하라', '베네수엘라', '평화지대','블록체인','뱅크사인','문재인','히어로', '김정은', '김정숙', '송은이', '분데스리가'] # 명사 단어 중 끝 음절이 조사와 같은 소리로 끝나는 단어들

josa_list = sorted(set(['과의','측','되는','까지','되며','인데','이라는', '라는','하','하여','하며','에선', '에서는', '했','는','하','하는','적인', '적으로', '적','인','보','으로는','겁니다', '겁니','보다', '합니다', '입니다', '됩니다', '합니', '입니', '됩니','하진', '하며', '위한', '하지', '엔', '에서도', '이나마', '만', '토록', '로서는', '하고', '해', '조차', '다', '에', '이나', '님께', '로서야', '은', '에다가', '까지', '로', '이라도', '가', '라든가', '인즉슨', '으로', '로다', '에다', '만큼', '밖에', '에게', '에도', '해야', '에는', '든', '와의', '같', '든가', '로서도', '이면', '이라', '께', '을', '라는', '된', '말고', '의', '며', '이고', '이라고는', '님', '라면', '만치', '이랑', '에게서', '를', '이며', '으로부터', '에서처럼', '하는', '들', '는', '적인', '뿐', '라든지', '로서', '처럼', '있는', '라고는', '한', '로다가', '이다', '로부터', '마저', '되', '이란', '로서의', '이든', '같이', '에게로', '할', '치고야', '없는', '부터', '인즉', '이라고', '이든가', '나', '과', '고', '뿐만', '나마', '에서부터', '하면', '으로써', '께서', '엔들', '하게', '대로', '치고서', '이라든지', '하는한', '에서', '라고', '적', '씨', '이', '라서', '하', '한테서', '커녕', '보다', '라도', '이든지', '도', '인들', '되고', '으로서', '하다', '란', '마냥', '랑', '이라면', '와', '한테로', '치곤', '한테다가', '해서', '마다', '하고는', '치고', '라', '라곤', '한테', '으', '로써', '이라든가']), reverse=True)
Except_words = set(['아니','등으로','즈음','때와는', '때와','반면','여럿','뿐이다', '뿐만아니라', '때마다','뒤이은', '뒤이어','이러한', '그러한', '보고싶다','년째','응접해줘서','그간','추진해나가는','알리는','빠른', '빠르다', '수밖에','밖에','넘으면','천천히', '넣으','연이은', '위한', '하며', '있다', '흘러','이후', '이전', '낮게','흘려', '들을', '만한','지난','이끌어', '이미', '이밖', '이번', '이전','하면','힘든', '힘들다', '하면서','한다', '한다고', '한다면', '이제', '이젠','이하', '중이다', '중이던', '중인', '인한','전혀', '절대', '절반', '인해','이상','이어', '이어질','이날', '이내', '이달', '이동한', '이라며', '이라면서','이런', '이런일', '이렇게', '이루고', '이뤄질', '이르면', '이른바','어느', '어떤', '어떻게', '역시', '열리', '열린', '열심히', '열어', '어려운', '어려울', '어렵다', '어린','안팎', '알고', '알려져', '알려진','아니냐', '아니다', '아닌', '아닐', '아래', '아바나', '아주', '아직', '아찔','보이고', '보인다', '반드시','번째', '생긴','서로','새로운','스스로', '순간', '당시','따라', '따로','만큼', '만약', '명확히' '따르면', '따른', '때문','당일','되면','들어가', '들어간', '듯한',  '되지', '된다', '두고', '동안','대신','대한', '당장', '당초', '대거','높여', '놓고', '누구','그러한', '그렇지', '그분','강력하게', '거듭','걸쳐', '관한', '관련','계속','강력한', '강하게', '강한', '갖고', '갖춘', '같다','거쳐', '거쳐서', '겨우', '경우', '같다','갑자기','각각','가지', '가능한','강하게', '면서','이라며', '가운데','따라','것', '게다가', '결국', '결론적으로', '구체적으로', '그나저나', '그래서', '그러나', '그러니', '그러니까', '그러면', '그러므로', '그런', '그런데', '그럼에도', '그렇게', '그렇기는', '그렇지만', '그리고', '그리하여', '급기야', '끝으로', '다른', '다만', '대해서', '더구나', '더욱이', '덧붙혀', '도리어', '동시에', '따라서', '때문에', '또한', '마지막으로', '마침내', '말하면', '바로', '반대로', '반면에', '불구하고', '뿐만', '사실상', '아니라', '아울러', '어쩌면', '예컨데', '오히려', '왜냐하면', '이같이', '이처럼', '일례로', '점에서', '하물며', '하지만', '한편', '해도'])
...
def extract(text, include_number = False, freq=1, threshold=0.3):
    """
    ...
    """
    text1 = re.sub(r'[\(\)\'\"ㆍ·=~…]', ' ', text) # 특정 기호 없애기
    text1 = re.sub(r'\.',' ',text1) # 마침표 없애기
    cleaned_text = re.sub(r'[^∙\s\w\d]', ' ', text1) # 기호 없애기

    org_ojoels = [ojoel.strip() for ojoel in cleaned_text.split()] #어절 추출
    c = Counter(org_ojoels)
    ojoels_count_dict = dict(c)
    
    ojoels = org_ojoels.copy()
    ...
    for ojoel in set(ojoels): #부사인 어절 제외
        kom_pos = komoran.pos(ojoel)
        if len(kom_pos) == 1:
            if kom_pos[0][1] in ADV_pos:
                while ojoel in ojoels: ojoels.remove(ojoel)
        else: 
            if kom_pos[0][1] in ['VV','VA']: # Komoran으로 분석시 명사가 될 수 없는 품사 태깅 결과 삭제
                while ojoel in ojoels: ojoels.remove(ojoel)
            
    final_Noun_words = []
    for ojoel in ojoels:
        for word in Proper_Nouns: # 고유명사 추가
            if word in ojoel:
    ...
    if include_number: #숫자를 포함하고자 하는 경우
        for word in Noun_words_2nd:
            ...
            if len(kmr_pos) > 1:
                if kmr_pos[1][1] in Extra_pos:
                    #if len(kmr_pos[0][0]) > 1:
                    final_Noun_words.append(kmr_pos[0][0])
                    continue
                    #else: 
                    #    Noun_words_2nd.remove(word)
                elif kmr_pos[1][0] == '시키':
                    final_Noun_words.append(kmr_pos[0][0])
                    continue
    ...
    return list(set(final_Noun_words))