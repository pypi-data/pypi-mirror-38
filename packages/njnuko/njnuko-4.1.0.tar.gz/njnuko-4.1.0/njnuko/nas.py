...
def check_files(fromfd,tofd,dblink):
    global checking
    checking = []
    log = os.path.join(tofd,'check_files.log')
    if os.path.exists(log):
        os.remove(log)
    f = open(log,'w')
    f.close()  
    process_checking(fromfd,tofd,log)

    InsertData('checking'+str(fromfd).split('/')[-1],log,dblink)
    
def process_checking(fromfd,tofd,log):
    global checking 
    for i in os.listdir(fromfd):
        ...
        if os.path.isdir(os.path.join(fromfd,i)):
            ...
            if i[0] not in ['@','.']:
                ...
                process_checking(os.path.join(fromfd,i),tofd,log)
        else:
            hash = ''
            the_md5=hashlib.md5()
            ...
            if hash in checking:
                with open(log,'a') as f:
                    writer = csv.writer(f)
                    writer.writerow(['duplicate',os.path.join(fromfd,i),'',hash])
                os.remove(os.path.join(fromfd,i)) 
            else:
                with open(log,'a') as f:
                    writer = csv.writer(f)
                    writer.writerow(['new',os.path.join(fromfd,i),'',hash])
...
'''
            ...
            for a in list1:
                if a[3] == hash:
                    with open(log,'a') as f:
                        writer = csv.writer(f)
                        writer.writerow(['duplicate',os.path.join(fromfd,i),a[2],hash])
                    os.remove(os.path.join(fromfd,i))
                    ex = 1
                    break
            if ex == 0:
