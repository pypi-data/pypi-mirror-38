...
const smInt64_t INVALID_VALUE = 9223372036854775807;
        ...
        const smInt64_t MIN_VALUE = -(smInt64_t)9223372036854775808;
...
struct Fix64Scalar
{
	...
	void setRawValue(smInt64_t val)
	{
		m_rawValue1 = val;
#ifdef BT_DEBUG
		m_debugValue = btScalar(m_rawValue1) / btScalar(ONE);
#endif
	}

	btScalar getScalar() const
	{
		return btScalar(m_rawValue1) / btScalar(ONE);
	}

	void setScalar(btScalar fl)
	{
		setRawValue(fl*ONE);
	...
	static Fix64Scalar fromScalar(btScalar fl)
	{
		Fix64Scalar res;
		res.setRawValue(fl*ONE);
        printf("converted fromScalar %f to %lld\n", fl,res.m_rawValue1);
		return res;
	}
	...
	{
		Fix64Scalar res;
		res.setRawValue(3.1415926535897932384626433832795029*ONE);
		return res;
	}
	...
	{
		Fix64Scalar res;
		res.setRawValue(2.*3.1415926535897932384626433832795029*ONE);
		return res;
	}
	...
	{
		return this->m_rawValue1 == INVALID_VALUE;
	}
	...
	//todo: use fixed point values, to avoid cross-platform issues of conversion from float 3 to fixed point 3
	...
	static Fix64Scalar sqrt(const Fix64Scalar& x);

	B3_FORCE_INLINE Fix64Scalar operator-() const
	{
		Fix64Scalar v = *this;
	...
	B3_FORCE_INLINE Fix64Scalar& operator+=(const Fix64Scalar& b)
	{
		Fix64Scalar a = *this;
		...
		if (((~(a.m_rawValue1 ^ b.m_rawValue1) & (a.m_rawValue1 ^ m_rawValue1)) & MIN_VALUE) != 0)
		{
...
bool operator> (const Fix64Scalar& a, const Fix64Scalar& b)
{
...
Fix64Scalar Fix64Scalar::sqrt(const Fix64Scalar& x) 
{
...
Fix64Scalar operator+(const Fix64Scalar& a, const Fix64Scalar& b)
{
	Fix64Scalar c;
	c.setRawValue(a.m_rawValue1+b.m_rawValue1);
	
	// if signs of operands are equal and signs of sum and x are different
	if (((~(a.m_rawValue1 ^ b.m_rawValue1) & (a.m_rawValue1 ^ c.m_rawValue1)) & MIN_VALUE) != 0)
	{
...
Fix64Scalar operator -=(Fix64Scalar& x, const Fix64Scalar& y) 
{
...
Fix64Scalar operator /(const Fix64Scalar& x, const Fix64Scalar& y) 
{
	Fix64Scalar res;
	res.setRawValue(MAX_VALUE);
...
B3_FORCE_INLINE Fix64Scalar& operator*=(Fix64Scalar& a, const Fix64Scalar& b)
{
	Fix64Scalar res = a*b;
...
static Fix64Scalar LutInterval = Fix64Scalar::fromRawInt64(884273636704256);//Fix64Scalar::fromScalar(LUT_SIZE - 1) / PiOver2;

static smInt64_t Fix64ClampSinValue(smInt64_t angle, bool& flipHorizontal, bool& flipVertical) {
...
static Fix64Scalar Floor(Fix64Scalar value) 
{
	...
	return Fix64Scalar::fromRawInt64(((smUint64_t)value.m_rawValue1 & 0xFFFFFFFF00000000));
}
...
static Fix64Scalar Sign(Fix64Scalar v)
{
	...
	return
		raw < 0 ? Fix64Scalar::minusOne()  :
		raw > 0 ? Fix64Scalar::one() :
		Fix64Scalar::zero();
}
...
Fix64Scalar Fix64Sin(const Fix64Scalar x)
{
    bool flipHorizontal, flipVertical;
	smInt64_t clampedL = Fix64ClampSinValue(x.m_rawValue1, flipHorizontal, flipVertical);
	Fix64Scalar clamped;
	...
	int idx = flipHorizontal ? sinlutlen - 1 - (int)roundedIndex.getScalar() : (int)roundedIndex.getScalar();
	Fix64Scalar nearestValue = Fix64Scalar::fromRawInt64(Fix64SinLookupTable[idx]);
    int idx2 = flipHorizontal ? sinlutlen - 1 - (int)roundedIndex.getScalar() - SignI(indexError) : (int)roundedIndex.getScalar() + SignI(indexError);
	Fix64Scalar secondNearestValue = Fix64Scalar::fromRawInt64(Fix64SinLookupTable[idx2]);

	smInt64_t delta = (indexError * Fix64Abs(nearestValue - secondNearestValue)).m_rawValue1;
	smInt64_t interpolatedValue = nearestValue.m_rawValue1+ (flipHorizontal ? -delta : delta);
    smInt64_t finalValue = flipVertical ? -interpolatedValue : interpolatedValue;
	return Fix64Scalar::fromRawInt64(finalValue);
        
}

static Fix64Scalar Fix64Cos(Fix64Scalar x) 
{
	smInt64_t xl = x.m_rawValue1;
    smInt64_t rawAngle = xl + (xl > 0 ? -PI - PI_OVER_2 : PI_OVER_2);
	return Fix64Sin(Fix64Scalar::fromRawInt64(rawAngle));
}


Fix64Scalar  Fix64Max(Fix64Scalar a, Fix64Scalar b)
{
	if (a > b)
		return a;
	return b;
...
Fix64Scalar  Fix64Min(Fix64Scalar a, Fix64Scalar b)
{
	if (a < b)
		return a;
	return b;
