...
def get_list_files_dir(path, filter=None):
    """
    ...
    if filter:
        if type(filter) == list:
            for f in filter:
                list_files.extend([x for x in os.listdir(path) if f in x])
        if type(filter) == str:
            list_files = [x for x in os.listdir(path) if filter in x]
    else:
...
def get_list_files_path(path, filter=None):
    """
    Get filelist with path in folder
    :param path: Folder witch contains files
    :param filter: filter, could be list of str or just str. None by default
    ...
    if filter:
        if type(filter) == list:
            for f in filter:
                list_files.extend([os.path.join(path, x) for x in os.listdir(path) if f in x])
        if type(filter) == str:
            list_files = [os.path.join(path, x) for x in os.listdir(path) if filter in x]
    else:
        list_files = [os.path.join(path, x) for x in os.listdir(path)]
