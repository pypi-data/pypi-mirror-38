...
def capture_piccompare(bbox,image1, image2, differencespicfile):
    im = ImageGrab.grab(bbox)
    im.save(image1)
    # 图片对比
    sleep(3)
    currentpic = Image.open(image1)
    expectedpic = Image.open(image2)
    diff = ImageChops.difference(currentpic, expectedpic)
    diff.save(differencespicfile)
    classfiy_histogram_with_split(currentpic, expectedpic)
...
#   image1 = Image.open(r"E:\PycharmProjects\testonebyone\picture\current.jpeg")
#   image2 = Image.open(r"E:\PycharmProjects\testonebyone\picture\expected.jpeg")
