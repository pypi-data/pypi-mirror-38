...
#------------------------------------------------------------------------------
def colorize(msg, colorname, reset=True, xterm=False):
    ...
    if reset:
        if xterm:
            return '%s%s%s' % (setBGColor(colorname), msg, resetBGColor())
        else:
            return '%s%s%s' % (setBGColor256(colorname), msg, resetBGColor())
    else:
        if xterm:
            return '%s%s' % (setBGColor(colorname), msg)
        else:
            return '%s%s' % (setBGColor256(colorname), msg)
...
if __name__ == '__main__':
    for i, color in enumerate(COLOR_ALPHABET):
        print(colorize("%2d XTerm %15s " % (i, color), color, xterm=True))
        print(colorize("%2d ANSI %15s " % (i, color), color))
