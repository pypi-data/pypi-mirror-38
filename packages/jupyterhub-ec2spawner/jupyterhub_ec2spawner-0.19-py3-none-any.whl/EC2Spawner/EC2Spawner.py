...
class EC2Spawner(Spawner):
    ...
    async def start_ec2_instance(self):
        instanceDef= {
            'AWS_AMI_ID': os.getenv("AWS_AMI_ID"),
            'AWS_KEYNAME': os.getenv("AWS_KEYNAME"),
        ...
        return self.buildInstance(instanceDef)
    ...
    def start(self):
        ...
        return (notebook_server_ip, os.getenv('AWS_NOTEBOOK_SERVER_PORT'))
    ...
    def poll(self, timeout=250):
        ...
        while loop.time() - tic < timeout:
            try:
                socket.create_connection((self.instance_ip, os.getenv('AWS_NOTEBOOK_SERVER_PORT')))
            except socket.error as e:
                if e.errno != errno.ECONNREFUSED:
                    app_log.error("Unexpected error waiting for %s:%i %s",
                        self.instance_ip, os.getenv('AWS_NOTEBOOK_SERVER_PORT'), e
                    )
                yield gen.Task(loop.add_timeout, loop.time() + 1)
            else:
                return
        raise TimeoutError("Server at {self.instance_ip}:{os.getenv('AWS_NOTEBOOK_SERVER_PORT')} didn't respond in {timeout} seconds".format(
            **locals()
        ))
