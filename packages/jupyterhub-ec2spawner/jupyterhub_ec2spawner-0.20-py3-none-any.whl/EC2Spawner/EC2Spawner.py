...
class EC2Spawner(Spawner):
    ...
    ec2_instance_port = os.getenv('AWS_NOTEBOOK_SERVER_PORT')
    ...
    def poll(self, timeout=250):
        ...
        while loop.time() - tic < timeout:
            try:
                socket.create_connection((self.instance_ip, self.ec2_instance_port))
            except socket.error as e:
                if e.errno != errno.ECONNREFUSED:
                    app_log.error("Unexpected error waiting for %s:%i %s",
                        self.instance_ip, self.ec2_instance_port, e
                    )
                yield gen.Task(loop.add_timeout, loop.time() + 1)
            else:
                return
        raise TimeoutError("Server at {self.instance_ip}:{self.instance_port} didn't respond in {timeout} seconds".format(
            **locals()
        ))
