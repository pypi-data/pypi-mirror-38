...
def get_status_updater(config: ISidecarConfiguration,
    ...
    if isinstance(config, KubernetesSidecarConfiguration):
        ...
        app_service = K8sAppService(api=kas, sandbox_id=k8s_config.sandbox_id, own_public_ip=own_public_ip, logger=_logger)
...
def configure_debug():
    ...
    _config = AwsSidecarConfiguration(environment="test", sandbox_id="t7onu02wct02c1", region_name="eu-west-1",
                                      ...
                                      cloud_external_key="8f70be32-3041-4a00-b96e-29f7a79cc854", apps=[
            SidecarApplication(name="fasty", instances_count=1, dependencies=[],
                               env={"WELCOME_STRING": "Welcome to Quali Colony!", "PORT": "3001"},
                               healthcheck_timeout=60, default_health_check_ports_to_test=[], healthcheck_script="fasty.sh")],
                                      messaging=MessagingConnectionProperties(host="wombat.rmq.cloudamqp.com",
                                                                              user="rhpjvtoj",
                                                                              password="M7dZ2D5hWkIEOLPj6poiB4fF1t9cmNIf",
                                                                              queue="nefmwtza4r121", exchange="",
                                                                              routingkey="nefmwtza4r121",
                                                                              virtualhost="rhpjvtoj", port=5672,
                                                                              queuetype="fanout", expires=60000,
                                                                              usessl=False))
