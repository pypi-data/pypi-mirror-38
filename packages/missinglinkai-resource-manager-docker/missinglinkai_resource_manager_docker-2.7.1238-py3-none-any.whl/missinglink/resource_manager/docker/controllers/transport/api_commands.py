...
class WsAPI:
    ...
    def _job_stared(self, job_id, task):
        job: RunningJob = RunningJob(job_id=job_id, task=task)
    ...
    async def run(self, send_async=None, **kwargs):
        ...
        task = asyncio.ensure_future(DockerExecution.create(send_async, **kwargs).run_manged())
        self._job_stared(job_id, task)
