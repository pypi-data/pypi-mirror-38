...
@server.command('ilist',
                help='show list of server instances for a given server type')
@click.option('-t', '--type', default=None, required=True, help='type of server')
@click.option('--filter', default=None, show_default=True,
              help='filter server instances (supports pattern matching *,?,[])')
def server_instance_list(type, filter):
    with error():
        click.echo(tangoctl.server_instance_list(type=type, filter=filter))
