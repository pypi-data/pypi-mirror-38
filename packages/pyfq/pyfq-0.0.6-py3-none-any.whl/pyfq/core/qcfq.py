...
class FastqQualityCheck(Fastq):
    ...
    def translatephredscores(self, to='q'):
        for read_record in self:
            if to == 'q':
                yield [ord(phred_score) - 33 for phred_score in read_record[2]]
            elif to == 'p':
                yield [10**-((ord(phred_score) - 33) / 10) for phred_score in read_record[2]]

    def per_base_sequence_quality(self):
        self.fig_name = 'per_base_sequence_quality'
        fig, ax = plt.subplots()
        ax.boxplot(
            list(map(list, zip(*[q_scores for q_scores in self.translatephredscores(to='q')]))), 0, '')
    ...
    def per_sequence_quality_scores(self):
        ...
        fig, ax = plt.subplots()
        average_qualities = [stats.describe(q_scores).mean for q_scores in self.translatephredscores(to='q')]
    ...
    def create_plots(self):
        ...
        for plot_function in plot_functions:
            fig = plot_function()
            fig_rel_path = os.path.join(self.img_dir.split('/')[-1], '{}.png'.format(self.fig_name))
            fig_full_path = os.path.join(self.img_dir, '{}.png'.format(self.fig_name))
            fig.savefig(fig_full_path)
            self.figs_dict[self.fig_name] = fig_rel_path
    ...
    def create_html_report(self):
        ...
        with open(self.output_file, 'w') as f:
            ...
            f.write('</ul>\n</div>\n')
            # Create images
            for fig_name, fig_rel_path in self.figs_dict.items():
                f.write(
                    '<div id="{0}"><img src="{1}" alt = "{0}"></div>\n'.format(fig_name, fig_rel_path))
            f.write('</body>\n</html>')
