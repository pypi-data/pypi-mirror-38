...
class S3Location:
    ...
    def __init__(self, str, ignore_double_slash=False):
        ...
        if result.scheme == "":
            if result.path.startswith("/"):
                ...
                self._path = "/".join(result.path.split("/")[1:])

        elif result.scheme == "s3":
            self._bucket = result.hostname
            self._path = result.path[1:]
            ...
            if result.hostname.startswith("s3") and result.hostname.endswith(".amazonaws.com"):
                ...
                self._path = "/".join(result.path.split("/")[2:])
            else:
    ...
    @property
    def key(self):
        return self._coalesce_empty(self._path.split("/")[-1], None)

    @property
    def prefix(self):
        if self._path is not None:
            return self._coalesce_empty("/".join(self._path.split("/")[0:-1]), None)
        else:
            return self._path
    ...
    def path(self):
        if self._path is not None:
            return self._coalesce_empty(self._path, None)
        else:
            return self._path
