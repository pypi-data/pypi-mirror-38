...
class AthenaClient(TaskQueue):
    ...
    def __del__(self):
        ...
        try:
            ...
            logger.error("Error while deleting an AthenaClient object: {0}".format(repr(e)))
    ...
    def create_table(self,
        ...
        - crawler_target: the definition of where the crawler should crawl, this can either be an S3 URL or a
          crawler_target from the AWS glue API
        - table_name:  this is the expected name for Glue to generate, NOT the name of the table that will be made
    ...
    def _stop_all_active_tasks(self):
        ...
        while len(self.active_queue) > 0:
            ...
            if task.id is not None:
                self.athena.stop_query_execution(QueryExecutionId=task.id)
            task.is_complete = True
