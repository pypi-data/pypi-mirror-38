...
class JapanHoliday(object):
    ...
    def get_holiday_calender_from_google(self, year):
        ...
        if response.status_code != 200:
            raise ConnectionError.new(ConnectionError.message)

        _json = json.loads(response.text)
        if 'items' not in _json:
            raise JsonParseError.new(JsonParseError.message)
        result = [CalenderHoliday(item) for item in _json.get('items')]

        if len(result) < 2:
            raise CalenderDoesNotExistError.new(CalenderDoesNotExistError.message)
