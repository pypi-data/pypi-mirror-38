...
def propagate(parameters, X, Y, activation="sigmoid", regularization="L2", lambd=0, keep_prob=1):
  assert(parameters['W1'].shape[1] == X.shape[0])
  assert(X.shape[1] == Y.shape[1])

  A, caches = _propagate_forward(parameters, X, activation, "sigmoid", keep_prob)
  
  Y = Y.reshape(A.shape) # ensure Y is the same shape as AL
  
  grads = _propagate_back(A, Y, caches, regularization, lambd, keep_prob)
  return grads, _cost(A, Y, parameters, regularization, lambd)

def _propagate_forward(parameters, A_prev, activation="relu", output_activation="sigmoid", keep_prob=1):
  caches = []
  L = len(parameters) // 2
  ...
  for l in range(1, L):
    ...
    A_prev, cache = _step_forward(A_prev, W, b, activation, keep_prob)
    caches.append(cache)

  # Output layer
  W = parameters['W{}'.format(L)]
  b = parameters['b{}'.format(L)]
  AL, cache = _step_forward(A_prev, W, b, output_activation, keep_prob=1)
  caches.append(cache)
...
def _cost(A, Y, parameters, regularization, lambd):
  m = Y.shape[1]
...
def _propagate_back(AL, Y, caches, regularization, lambd, keep_prob):
  grads = {}
  ...
  for l in reversed(range(L)):
    ...
    dA, grads[dw], grads[db] = _step_backward(dA, current_cache, regularization, lambd, keep_prob)
  
  return grads


def _step_backward(dA, cache, regularization, lambd, keep_prob):
  ((A_prev, W, b), (Z, D_prev, activation)) = cache
  ...
  dA_prev = np.dot(np.transpose(W), dZ) * D_prev / keep_prob
