...
def initialize_parameters(layer_dimensions, W_multiplier=0.01, use="random", x_over_n=2):
    """
    Inputs:
    ...
    for l in range(1, L):
        ...
        if use == "zeros":
            parameters[W] = np.zeros(
                (current_layer_nodes_count, previous_layer_nodes_count))
        elif use == "random":
            parameters[W] = np.random.randn(
                current_layer_nodes_count, previous_layer_nodes_count) * W_multiplier
        elif use == "he":
            parameters[W] = np.random.randn(
                current_layer_nodes_count, previous_layer_nodes_count) / np.sqrt(x_over_n/previous_layer_nodes_count)
        else:
