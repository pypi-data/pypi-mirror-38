...
class YunpianSms:
    ...
    @staticmethod
    def register_code(mobile, request=None):
        ...
        sms_code.code = YunpianSms.gen_code()
        sms_code.save()
        if request:
        ...
        param = {
            YC.MOBILE: mobile,
            YC.TEXT: settings.SMS_TEMPLATE.get('register').format(name=title, time=20, code=sms_code.code)
        }
    ...
    @staticmethod
    def notice_order(username, mobile):
        param = {
            YC.MOBILE: mobile,
            YC.TEXT: settings.SMS_TEMPLATE.get('notice').format(name=username, site=settings.SITE_URL)
        }
        return client.sms().single_send(param)
