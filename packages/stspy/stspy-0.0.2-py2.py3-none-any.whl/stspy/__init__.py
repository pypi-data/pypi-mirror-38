...
class STM:
    def __init__(self, spec_length_step = 0.2, LockinRC_factor = 7960., skiprows = 576, current_conv = 5e13):
        self.spec_length_step = spec_length_step #minimum number of step size in seconds i.e. spec_length_step*Spec*Vertmandelay = Speclength[s]
        self.LockinRC_factor = LockinRC_factor #mutliplicative factor to get LockinRC param in Hz
        self.skiprows = skiprows #number of rows to skip to get to spectra data
        self.current_conv = current_conv #conversion factor to get current (and dIdV) in Amps
    def __str__(self):
        ret = 'LockinRC_factor: %f\n' %self.LockinRC_factor
        ret += 'current_conv: %e\n' %self.current_conv
        ret += 'spec_length_step: %f\n' %self.spec_length_step
        ret += 'skiprows: %d' %self.skiprows
        return ret

class spectra:
    def __init__(self, filename, stm = STM()):
        ...
        for line in f:
            ...
            if "Vertmandelay" in line:
                Speclength = float(line[line.find('=')+1:-1])*stm.spec_length_step #seconds
        ...
        self.I = data[:, 3] / stm.current_conv #current in amps
        self.dIdV = data[:,4] / stm.current_conv #conductance
    ...
    #not if you want to get the original values after this, just call e.g. epctra_object.I0
        ...
        if n > 1:
            ...
            self.I = I/n/ self.stm.current_conv
            self.dIdV = dIdV/n / self.stm.current_conv
        else:
