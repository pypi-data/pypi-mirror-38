...
class ReversalApi(object):
    ...
    def get_auth_reversal(self, id, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_auth_reversal(id, callback=callback_function)

        :param callback function: The callback function
            for asynchronous request. (optional)
        :param str id: The authorization reversal ID returned from a previous authorization reversal request. (required)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_auth_reversal_with_http_info(id, **kwargs)
        else:
            (data) = self.get_auth_reversal_with_http_info(id, **kwargs)
            return data

    def get_auth_reversal_with_http_info(self, id, **kwargs):
        """
        Retrieve an Authorization Reversal
        Include the authorization reversal ID in the GET request to retrieve the authorization reversal details. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_auth_reversal_with_http_info(id, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_auth_reversal" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'id' is set
        if ('id' not in params) or (params['id'] is None):
            raise ValueError("Missing the required parameter `id` when calling `get_auth_reversal`")
        ...
        return self.api_client.call_api('/v2/reversals/{id}', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
