...
_EIGRP_NEIGHBOR_CMD = "show ip eigrp neighbor"
...
class CatSwitch(_CiscoSwitch):
    ...
    def fetch_eigrp_neighbors(self):
        print(f"Collecting EIGRP neighbors from {self.host} ...")
        eigrp_dicts = self.get_command_output(_EIGRP_NEIGHBOR_CMD)
        if not eigrp_dicts:
            print("No BGP collected")
            self.eigrp_neighbors = None
            return None
        self.eigrp_neighbors = layer3.EigrpNeighbors(eigrp_dicts)
