...
# Setup a dedicated logger for jt.jpyutil.
# This way importing jt.jpyutil does not interfere with logging in other modules
logger = logging.getLogger("jt.jpyutil")
...
def _add_paths_if_exists(path_list, *paths):
    ...
    for item in paths:
        if os.path.exists(item) and not item in path_list:
...
def _get_module_path(name, fail=False, install_path=None):
    ...
    if not module and fail:
        raise RuntimeError("can't find module '{}'".format(name))
    module_path = module[1]
    if not module_path and fail:
        raise RuntimeError("module '{}' is missing a file path".format(name))
    
    if install_path:
        return os.path.join(install_path, os.path.split(module_path)[1])
...
def _find_file(search_dirs, *filenames):
    ...
    for filename in filenames:
        for search_dir in search_dirs:
            file_path = os.path.normpath(os.path.join(search_dir, filename))
            path_exists = os.path.exists(file_path)
            logger.debug("Exists '{}'? {}".format(file_path, "yes" if path_exists else "no"))
            if path_exists:
                return file_path
    return None


def _get_java_api_properties(fail=False, install_path=None):

    jpy_config = Properties()
    jpy_config.set_property("jpy.jpyLib", _get_module_path("jt",  fail=fail, install_path=install_path))  #!!! "jpy"
   #jpy_config.set_property("jpy.jdlLib", _get_module_path("jdl", fail=fail, install_path=install_path))  #!!!
    ...
    try:
        ...
        for part in output.split("\n"):
            if part.startswith("Java home:"):
                jre_dir = part.split(":")[1].strip()
                if jre_dir.endswith("jre"):
                    return jre_dir[0:-3]
    except Exception:
...
def find_jvm_dll_file(java_home_dir=None, fail=False):
    ...
    if jvm_dll_path:
        logger.debug("No JVM shared library file found in all search paths. "
                     "Using fallback {!r}".format(jvm_dll_path))
    elif fail:
...
def _get_jvm_lib_dirs(java_home_dir):
    ...
    arch = "i386" if is_32bit else "amd64"
    return (
        os.path.join(java_home_dir, "bin"),
...
def _find_jvm_dll_file(java_home_dir):
    ...
    logger.debug("Searching for JVM shared library file in {!r}".format(java_home_dir))
...
def _find_python_dll_file(fail=False):
    ...
    logger.debug("Potential Python shared library search dirs: {!r}".format(search_dirs))
    ...
    logger.debug("Potential Python shared library file names: {!r}".format(file_names))
    ...
    if python_dll_path:
        logger.debug("No Python shared library file found in all search paths. "
                     "Using fallback {!r}".format(python_dll_path))
    elif fail:
...
def preload_jvm_dll(jvm_dll_file=None,
    ...
    if jvm_dll_file:
        logger.debug("Preloading JVM shared library {!r}".format(jvm_dll_file))
...
def get_jvm_options(jvm_maxmem=None,
    ...
    options = []
    if jvm_maxmem:
        options.append("-Xmx{}".format(jvm_maxmem))
    if jvm_cp:
        options.append("-Djava.class.path={}".format(jvm_cp))
    if jvm_properties:
        for key in jvm_properties:
            value = jvm_properties[key]
            options.append("-D{}={}".format(key, value))
    if jvm_options:
        options += jvm_options
...
def init_jvm(java_home=None,
    ...
    :param jvm_classpath: The JVM search paths for Java class files. Separated by colons (Unix) or semicolons
                          (Windows). Refer to the java executable '-classpath' option.
    ...
    if not jpy.has_jvm():
        jvm_options = get_jvm_options(jvm_maxmem=jvm_maxmem,
                                      ...
                                      config=config)
        logger.debug("Creating JVM with options {!r}".format(jvm_options))
            ...
            for line in lines:
                if line and len(line) > 0 and not line.startswith("#"):
                    ...
                    if len(tokens) == 2:
                        ...
                        raise ValueError("illegal Java properties format {}".format(line))
...
def write_config_files(out_dir=".",
    ...
    if jvm_dll_file:
        ...
        try:
            with open(py_api_config_file, "w") as f:
                f.write("# Created by '{}' tool on {}\n".format(tool_name, str(datetime.datetime.now())))
                f.write("# This file is read by the 'jt.jpyutil' module in order to load and configure the JVM from Python\n")
                if java_home_dir:
                ...
                f.write("jvm_options = []\n")
            logger.info("jpy Python API configuration written to '{}'".format(py_api_config_file))
        except Exception:
    ...
    try:
        ...
        java_api_properties = _get_java_api_properties(fail=req_java_api_conf, install_path=install_dir)
        java_api_properties.store(java_api_config_file, comments=[
            "Created by '{}' tool on {}".format(tool_name, str(datetime.datetime.now())),
            "This file is read by the jpy Java API (org.jpy.PyLib class) in order to find shared libraries"])
        logger.info("jpy Java API configuration written to '{}'".format(java_api_config_file))
    except Exception:
...
def _main():
    ...
    parser.add_argument("-o", "--out", action="store", default=out_default,
                        help="Output directory for the generated configuration files. "
                             "Defaults to {}".format(out_default))
    parser.add_argument("--java_home", action="store", default=None,
                        help="Java JDK or JRE home directory. Can also be set using "
                             "one of the environment variables " + " | ".join(JRE_HOME_VARS) + ".")
    parser.add_argument("--jvm_dll", action="store", default=None,
                        help="Java JVM shared library file. Usually inferred "
                             "from java_home option. Can also be set using "
                             "environment variable JPY_JVM_DLL.")
    parser.add_argument("--log_file", action="store", default=None,
    ...
    parser.add_argument("--install_dir", action="store", default=None,
                        help="Optional. Used during pip install of JPY")
