...
from facenet_sandberg.config import AlignConfig
...
def align_dataset(
        ...
        input_dir: str,
        output_dir: str,
        is_flat_dir: bool=False):
    """Aligns an image dataset
    """
    config = AlignConfig(config_file)
    config.input_dir = input_dir
    config.output_dir = output_dir
    config.is_flat_dir = is_flat_dir
    output_dir = os.path.expanduser(config.output_dir)
    os.makedirs(output_dir, exist_ok=True)

    dataset = get_dataset(config.input_dir, is_flat_dir)
    ...
    if num_processes > 1:
        ...
        process_pool.imap(
            align_person_multiprocess, zip(
                dataset, [config] * len(dataset)))
        process_pool.close()
        process_pool.join()
    else:
        align_person_singleprocess(dataset, config)

    transform_to_lfw_format(output_dir, num_processes)

    TIMER.finish()
    logging.info('Total number of images: %d' % int(NUM_IMAGES_TOTAL.value))
    logging.info('Number of faces found and aligned: %d' %
          int(NUM_SUCESSFUL.value))
    logging.info('Number of unsuccessful: %d' %
          int(NUM_UNSECESSFUL.value))


def align_person_multiprocess(data: Tuple[PersonClass, AlignConfig]) -> None:
    person, config = data
    if config.is_flat_dir:
        output_class_dir = config.output_dir
    else:
    ...
    align_person_helper(output_class_dir, person, detector, config)


def align_person_singleprocess(
        dataset: List[PersonClass],
...
def align_person_helper(
    ...
    if config.detect_multiple_faces and config.facenet_model_checkpoint:
        align_with_facenet(
            output_class_dir,
            person,
            detector,
            config,
            all_faces)
    elif config.detect_multiple_faces:
        align_choose_all(all_faces)
    else:
        align_choose_first(all_faces)
    del detector
    TIMER.update(int(NUM_IMAGES_TOTAL.value))


def align_with_facenet(
        output_class_dir: str,
        person: PersonClass,
        detector: align.Detector,
        config: AlignConfig,
    ...
    if anchor:
        ...
        logging.warning('Failed to find anchor... not using facenet')
    if encoder:
        encoder.tear_down()
        del encoder


def align_choose_first(all_faces: FacesGenerator):
    for faces in all_faces:
        if faces:
            cv2.imwrite(
                faces[0].name, cv2.cvtColor(
...
def align_choose_all(all_faces: FacesGenerator):
    for faces in all_faces:
        if faces:
            for index, face in enumerate(faces):
                image_name = append_index(face.name, str(index))
                cv2.imwrite(
                    image_name, cv2.cvtColor(
...
def append_index(filename: str, index: str):
    name, ext = os.path.splitext(filename)
    return "{name}_{index}{ext}".format(name=name, index=index, ext=ext)
