...
def make_square(image: Image, min_size=300):
    img = PIL.Image.fromarray(image)
    x, y = img.size
    size = max(min_size, x, y)
    new_im = PIL.Image.new('RGB', (size, size), (0, 0, 0))
    new_im.paste(img, (int((size - x) / 2), int((size - y) / 2)))
    resized = np.array(new_im)
    return fix_image(resized)


def max_size(image: Image, max_dim: int=600):
    w, h, _ = image.shape
    if w > max_dim or h > max_dim:
        image = resize(image, max_dim, max_dim)
    return image
...
def download_image(url: str, is_rgb: bool = True) -> Optional[Image]:
    try:
        ...
        logging.warning('Couldn\'t read: {}'.format(url))
        return None
...
def get_image_from_path_rgb(image_path: str) -> Optional[Image]:
    ...
    try:
        ...
        logging.warning('Couldn\'t read: {}'.format(image_path))
        return None
...
def get_dataset(path: str, is_flat: bool = False) -> List[PersonClass]:
    ...
    if is_flat:
        people = [os.path.basename(path) for path in get_image_paths(path_exp)]
