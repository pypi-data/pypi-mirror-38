...
import numpy as np
from facenet_sandberg.common_types import PairMatch, PairMismatch
...
def write_pairs_to_file(fname: str,
                        match_folds: List[List[PairMatch]],
                        mismatch_folds: List[List[PairMismatch]],
                        num_folds: int,
...
def _make_matches(image_dir: str,
                  people: List[str],
                  total_matches: int) -> List[PairMatch]:
    matches = cast(Set[PairMatch], set())
    curr_matches = 0
...
def _make_mismatches(image_dir: str,
                     people: List[str],
                     total_matches: int) -> List[PairMismatch]:
    mismatches = cast(Set[PairMismatch], set())
    curr_matches = 0
    while curr_matches < total_matches:
