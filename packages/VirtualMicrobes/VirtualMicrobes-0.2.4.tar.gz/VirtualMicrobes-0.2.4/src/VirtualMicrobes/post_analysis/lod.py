...
class LOD_Analyser(object):
    ...
    def pop_cells(self, runtime=None, tcs=False):
        ...
        self.ref_pop_hist.dump_pop_cells(runtime)
...
class PopulationHistory(object):
    ...
    def dump_pop_cells(self,time):
        ...
        for i,cell in enumerate(self.population.cells):
            write_obj.write_cell(cell, os.path.join(directory+'/Cellfile_inpop_'  + cell._unique_key +'.cell'))
    ...
    def dump_anc_cells(self,time):
        ...
        for cell in self.population.current_ancestors:
            write_obj.write_cell(cell, os.path.join(directory+'/Cellfile_ancestor_'  + cell._unique_key +'.cell'), alive=True)
            self.sim.data_store.add_cell_tc(cell,directory,
                                        self.sim.graphs.attribute_mapper, time, cell._unique_key)
    ...
    def dump_lod_cells(self,time):
        ...
        for key, lod in self.lods.items():
            ...
            util.ensure_dir(directory)
            for cell in lod:
                write_obj.write_cell(cell, os.path.join(directory+'/Cellfile_lod_'  + cell._unique_key +'.cell'), alive=True)
                self.sim.data_store.add_cell_tc(cell,directory,
                                    self.sim.graphs.attribute_mapper, time, cell._unique_key)
