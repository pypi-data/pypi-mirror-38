...
class Simulation(object):
    ...
    def simulate(self):
        ...
        while self.run_time <= self.params.duration:
            ...
            # microbes are randomly redistributed on the grid
            # metabolites are evened out perfectly
            if self.params.perfect_mix and self.run_time % self.params.perfect_mix == 0:
                self.system.environment.perfect_mix()

            # random spatial diffusion of microbes
...
def load_sim(file_name, verbose=False, **param_updates):
    ...
    # # NO-PUB F0001 BEGIN ## Backwards compatibility stuff. ## Flagged for removal after done with "Stringent response" project.
    #
    # if param_updates.has_key('compete_cells'):
    #     param_updates['compete_with_anc'] = None
    #     if not param_updates.has_key('compete_mixed'): param_updates['compete_mixed'] = False
    #     if not param_updates.has_key('compete_density'): param_updates['compete_density'] = 1.0
    # elif param_updates.has_key('compete_with_anc'):
    #     param_updates['compete_cells'] = None
    #     if not param_updates.has_key('compete_mixed'): param_updates['compete_mixed'] = False
    #     if not param_updates.has_key('compete_density'): param_updates['compete_density'] = 1.0
    #
    # if not param_updates.has_key('prune_csv_from_time'): param_updates['prune_csv_from_time'] = 0

    # NO-PUB F0001 END ## Backwards compatibility stuff. ## Flagged for removal after done with "Stringent response" project.
    ...
    if hasattr(sim.params, 'env_from_file'): # Compatibility with old version of Vmicrobes
        if sim.params.env_from_file is not None and sim.params.upgrade_environment:
            sim.init_time_course_lengths()
            sim.init_data_store()
            sim.init_graphs()
...
def mut_func_single_param_step_uniform(val, rand_g, mut_par_space):
    if (rand_g.uniform(0.0,1.0) < mut_par_space.randomize):
        return rand_g.uniform(mut_par_space.min, mut_par_space.max)
    else:
        return max( min(val+ rand_g.uniform(mut_par_space.lower, mut_par_space.upper),
                        mut_par_space.max),
                   mut_par_space.min)
...
def partial_mut_func_single_param(val, rand, mut_par_space):
    if mut_par_space.uniform:
        return mut_func_single_param_step_uniform(val, rand, mut_par_space)
    else:
        return mut_func_single_param(val, rand, mut_par_space)

def mut_ks_dict_func(kss, rand_gen, mut_par_space, mutate_single_param):
        single_ks = rand_gen.choice(kss.keys())
        kss[single_ks] = mutate_single_param(kss[single_ks], rand_gen, mut_par_space)
        return kss
