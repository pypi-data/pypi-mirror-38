...
class DevOpsApp:
    ...
    def __init__(self, app_name, log_level='info'):
        ...
        cloud_init_groups = os.environ['PUPPY_CLOUD_INIT_GROUPS'].split(',') if 'PUPPY_CLOUD_INIT_GROUPS' in os.environ else []
        ansible_groups = os.environ['PUPPY_ANSIBLE_GROUPS'].split(',') if 'PUPPY_ANSIBLE_GROUPS' in os.environ else []
        self.group_names = list(set(cloud_init_groups) | set(ansible_groups))
    ...
    def shell_run(self, cmd, keep_pipeline=True):
        self.logger.info('shell_run: %s', cmd)
        if keep_pipeline:
            # https://stackoverflow.com/questions/10479825/python-subprocess-call-broken-pipe
            return subprocess.call(cmd, shell=True, preexec_fn=lambda: signal(SIGPIPE, SIG_DFL))
