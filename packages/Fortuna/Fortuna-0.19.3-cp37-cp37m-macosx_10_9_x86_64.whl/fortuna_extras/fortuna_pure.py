...
def distribution_timer(func: staticmethod, *args, call_sig=None, max_distribution=50, num_cycles=10000, **kwargs):
    ...
    if len(unique_results) <= max_distribution:
        result_obj = {
            key: f"{round(results.count(key) / (num_cycles / 100), 3)}%" for key in sorted(list(unique_results))
        }
...
if __name__ == "__main__":
    t0 = datetime.now()
    print("\nFortuna Pure 0.19.3 Sample Distribution and Performance Test Suite\n")
