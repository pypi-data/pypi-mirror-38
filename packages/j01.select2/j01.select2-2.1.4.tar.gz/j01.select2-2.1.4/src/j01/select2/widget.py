...
##############################################################################
"""
$Id: widget.py 4941 2018-11-09 23:47:29Z roger.ineichen $
"""
                ...
                if (this.children) {
                    if (this.children.length > 0) {
                        ...
                        for (var i = 0; i < this.children.length; i++) {
                            ...
                            children.push({id: obj.id, text: obj.text});
                        }
                        data.results.push({text: this.text, children:children});
                    }
                }else{
...
class InitDataMixin(object):
    """Initialize data mixn class"""
        ...
        if vocab is not None:
            ...
            for term in vocab:
                ...
                if self.isGroupTerm(term):
                    ...
                    children.append(
                        '{id: "%s", text: "%s"}' % (term.title, term.title)
                    )
            if group is not None:
                # add latest group and children to terms
                terms.append(
                    '{text: "%s", children: [%s]}' % (group, ",".join(children))
                )
            else:
                # no groups, use children as terms
                terms = children
            return "[%s]" % ",".join(terms)
        else:
...
class TagListSelect2Widget(InitDataMixin, Select2WidgetBase,
...
class SingleTagSelect2Widget(InitDataMixin, Select2WidgetBase,
    z3c.form.browser.text.TextWidget):
