...
(function () {
    ...
    cef.virtualEvents = (selector,obj) => {
      ...
      init(Input, obj) {
        ...
        //  input 包裹节点
        if (document.querySelector('#wrapSpan') === null) {
          this.parseSpan()
        }
        //  键盘按下，失去焦点，点击事件
      ...
      parseSpan() {
        this.wrapOuter(
          this.Input,
          "<div id='wrapSpan' style='position: relative;'></div>"
        )
        ...
        document.querySelector('#wrapSpan').appendChild(this.span)
        document.querySelector('#wrapSpan').appendChild(this.em)
      }
      // 虚拟dom
      parseHTML(str) {
