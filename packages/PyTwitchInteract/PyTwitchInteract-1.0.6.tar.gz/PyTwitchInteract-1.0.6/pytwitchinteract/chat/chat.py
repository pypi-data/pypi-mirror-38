...
class TwitchChat:
    ...
    def __reconnect(self):
        ...
        self.send_raw_message("PASS {}".format(self.token))

        # Nickname doesn't actually matter, only requires to be sent
        self.send_raw_message("NICK PyTwitch")
        ...
        self.send_raw_message("CAP REQ :twitch.tv/membership".format(self.channel))
        self.send_raw_message("CAP REQ :twitch.tv/tags".format(self.channel))
        self.send_raw_message("CAP REQ :twitch.tv/commands".format(self.channel))

        self.send_raw_message("JOIN {}".format(self.channel))
    ...
    def send_chat_message(self, message):
        self.send_raw_message("PRIVMSG {} :{}".format(self.channel, message))
    ...
    def on_connect_call(self, callback):
...
class Message:
    ...
    def __init__(self, chat, message):
        ...
        if data.group(1) is not None:
            self.info = {}
            for i in data.group(1).split(";"):
                k, v = i.split("=", maxsplit=1)
                self.info[k] = v

        self.sender = data.group(2)
        self.type = data.group(3)
        self.target = data.group(4)
        self.content = data.group(5)
