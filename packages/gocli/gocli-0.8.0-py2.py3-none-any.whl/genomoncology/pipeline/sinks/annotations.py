...
async def make_call(sdk, data_set, data_set_version, in_queue):
    unit = in_queue.get()

    while unit != EOL:
        await kms.annotations.async_load_annotations(
            unit, sdk=sdk, data_set=data_set, data_set_version=data_set_version
        )
        unit = in_queue.get()
...
def background_runner(state, data_set, data_set_version, queue):
    ...
    loop.run_until_complete(
        asyncio.gather(make_call(sdk, data_set, data_set_version, queue))
    )
...
class LoadAnnotationSink(Sink):
    ...
    def write(self, data):
        ...
        data = map(set_uuid(self.data_set, self.data_set_version), data)
