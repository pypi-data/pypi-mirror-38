...
def run_holdout_deepmg(options,args, special_usecase = '', txt_time_pre =''):
    ...
    #check whether this experiment had done before
    if special_usecase not in ['y','yes']:  
        if options.search_already == 'y':
            prefix_search = utils.name_log_final(options, n_time_text= '*')        
    ...
    if special_usecase in ['predict'] and not os.path.isfile(os.path.join(path_read, dataset_name + '_y.csv')):
        print 'predict without labels'
        labels = []
    else:           
        labels = pd.read_csv(os.path.join(path_read, dataset_name + '_y.csv'))
        labels = labels.iloc[:,1] 
