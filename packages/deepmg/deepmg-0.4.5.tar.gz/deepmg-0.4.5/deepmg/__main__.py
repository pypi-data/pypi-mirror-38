...
if options.cudaid > -1 : #specify idcuda you would like to use
    #use gpu
    print 'you are uing gpu: ' + str(options.cudaid)    
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"   # see issue #152
    os.environ["CUDA_VISIBLE_DEVICES"] = str(options.cudaid)
elif  options.cudaid == -1: #specify cpu
    print 'you are uing cpu'
    os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"   # see issue #152
...
if __name__ == "__main__":   
    if options.type_run in ['vis','visual']:
        ...
        if options.save_entire_w in ['y']:        #if get weights on whole dataset  
