...
class StepReport(object):
    ...
    def __init__(self, samples, out_file, template_input_file):
    ...
    def get_step_path(self, file):
        ...
        for step_path in os.listdir(os.path.dirname(os.path.dirname(file))):
            if template_step[2:] in step_path:
...
class CountMatrix(StepReport):
    def __init__(self, samples, out_file, template_input_file):
        super(CountMatrix, self).__init__(samples, out_file, template_input_file)
...
class NonFeatureCounts(StepReport):
    def __init__(self, samples, out_file, template_input_file):
        super(NonFeatureCounts, self).__init__(samples, out_file, template_input_file)
...
class TrimStats(StepReport):
    def __init__(self, samples, out_file, template_input_file):
        super(TrimStats, self).__init__(samples, out_file, template_input_file)
...
class MappingStats(StepReport):
    def __init__(self, samples, out_file, template_input_file):
        super(MappingStats, self).__init__(samples, out_file, template_input_file)
...
class StarCountsStats(StepReport):
    def __init__(self, samples, out_file, template_input_file, stranded):
        self.stranded = stranded
        super(StarCountsStats, self).__init__(samples, out_file, template_input_file)
...
class ComparingFiles(object):
    def __init__(self, samples, samples_deseq, factors, batches, output_dir, run_id):
        ...
        self.desc_out_file = os.path.join(output_dir, 'sample_desc'+run_id+'.csv')
        self.comp_out_file = os.path.join(output_dir, 'comparisons'+run_id+'.csv')
...
class CreateArgs(object):
    ...
    def getargs(self, indir, infile, outfile):
        ...
        return (self.samples, out_file, template_input_file)
