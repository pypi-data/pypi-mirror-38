...
def InitShading( width, height, glslDirectory, substitutions ):
	...
	#GL.glEnable( GL.GL_DEPTH_TEST ) # enabling this seems to cause all kinds of confusion with the direction of z that works and the direction that does not; other changes are also needed - see comment about GL_DEPTH_BUFFER_BIT
...
def DrawWorld( world, dt=None, t=None ):
	...
	if firstFrame:
		...
		GL.glClear( GL.GL_COLOR_BUFFER_BIT )  #   if depth test is enabled, need to | this with GL.GL_DEPTH_BUFFER_BIT;  also figure out correct glDepthFunc and glClearDepth
