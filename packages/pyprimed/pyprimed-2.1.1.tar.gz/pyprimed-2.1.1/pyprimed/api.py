...
class Client:
    ...
    def _check_for_errors(self, response, status_code, opname):
        if response.headers["content-type"] != "application/json; charset=utf-8":
            msg = (
                f"{opname}: "
                + f"expected response to be 'application/json; charset=utf-8', was {response.headers['content-type']}"
            )
        ...
        if response.status_code != status_code:
            ...
            if "errors" in response_body:
                ...
                msg = f"{opname}: " + ", ".join(errors)
            else:
                msg = f"{opname}: " + "An unexpected error has occurred"
            raise ServerError(msg)
