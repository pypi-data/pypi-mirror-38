...
from auditlogger import settings
...
class CloudWatchLog:
    ...
    def __init__(self):  # type () -> None
        ...
        self.client = boto3.client(
            'logs',
            region_name=settings.AUDIT_LOGGER_REGION,
            aws_access_key_id=settings.AUDIT_LOGGER_KEY_ID,
            aws_secret_access_key=settings.AUDIT_LOGGER_SECRET_ACCESS_KEY,
            config=CONFIG,
        )
    ...
    def create_log_group(self):  # type () -> None
        ...
        try:
            self.client.create_log_group(
                logGroupName=settings.AUDIT_LOGGER_GROUP_NAME,
            )
        except ClientError:
    ...
    def create_log_stream(self):  # type () -> None
        ...
        try:
            self.client.create_log_stream(
                logGroupName=settings.AUDIT_LOGGER_GROUP_NAME,
                logStreamName=settings.AUDIT_LOGGER_STREAM_NAME,
            )
        except ClientError:
        ...
        try:
            ...
            if e.response.get('Error', {}).get('Code') in (
                ...
                self.next_sequence_token = \
                    e.response['Error']['Message'].rsplit(' ', 1)[-1]
