...
def getCFInstanceCredentials(instance_params):
    ...
    print('\t - Looking for existing instances of service "{}"'.format(service_name))
    ...
    for instance in existingInstances:
        if instance['service'] == service_name:
            print('\t - Found existing instance "{}" of "{}"'.format(instance['name'], service_name))
            if existingInstanceName is None:
                existingInstanceName = instance['name']
    if existingInstanceName is not None:
        print('\t - Using instance {}'.format(existingInstanceName))
        instance_name = existingInstanceName
    else:
        print('\t - No existing instance of "{}" found.'.format(service_name))
        print('\t - Creating a new instance "{}" for service "{}"'.format(instance_name, service_name))
        instanceList = createCFServiceInstance(
            service_name, service_plan_name, instance_name)
        for instance in instanceList:
            print('\t - Created instance "{}" of "{}"'.format(instance['name'], service_name))
            existingInstanceName = instance['name']

    print('\t - Looking for existing credentials with name "{}"'.format(key_name))
    credentials = getCFServiceKey(instance_name, key_name)

    if credentials is None:
        print('\t - Existing credentials with name "{}" not found'.format(key_name))
        print('\t - Generating credentials with name "{}"'.format(key_name))
        createCFServiceKey(instance_name, key_name)
        credentials = getCFServiceKey(instance_name, key_name)
        print('\t - Generated "{}" for "{}"'.format(key_name, instance_name))
    else:
        print('\t - Existing credentials with name "{}" for instance "{}" found, reusing'.format(key_name, instance_name))
