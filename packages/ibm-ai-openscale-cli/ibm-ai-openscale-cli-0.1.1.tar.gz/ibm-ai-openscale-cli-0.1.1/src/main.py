...
from src.utils import jsonFileToDict, getIamHeaders, getExplainabilityBody, verifyVersionOfLib
...
class AIOSTool(object):
    ...
    def setupWMLModels(self, credentials, env):
        ...
        print('Checking for model named: {}'.format(myModelName))
    ...
    def getAIOSPythonClient(self, aiosCredentials):
        ...
        print('Using AI Openscale (AIOS) Python Client version: {}'.format(aiosClient.version))
        return aiosClient
    ...
    def createDatamart(self, datamartName, aiosClient, postgresCredentials):
        ...
        print('Datamart {} created'.format(datamartName))
    ...
    def addSubscription(self, modelName, aiosClient, modelGuid):
        print('Register WML instance to AIOS: {}'.format(modelName))
    ...
    def generateSampleScoring(self, datamartName, aiosCredentials, modelName, modelGuid, deployGuid, bindingGuid, subscription, aiosClient, env, historyDays):
        ...
        for day in range(historyDays):
            ...
            for hour in range(24):
                ...
                requests.post(metricsurl, json=[qualityMetric], headers=iamHeaders)
...
def main():
    ...
    verifyVersionOfLib('ibm-ai-openscale', [1, 0, 89], 'pip install -I --user --index https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple ibm-ai-openscale')
    verifyVersionOfLib('watson-machine-learning-client', [1, 0, 339], 'pip install -I --user --index https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple watson-machine-learning-client')
    args = process_args()
    ...
    tool.createDatamart(datamartName, aiosPythonClient, postgresCredentials)

    wmlBindingGuid = tool.bindWMLToAIOS(aiosPythonClient, wml_credentials)
    ...
    print('The AI OpenScale dashboard can be accessed at: {}/aiopenscale'.format(environment['aios_url']))
