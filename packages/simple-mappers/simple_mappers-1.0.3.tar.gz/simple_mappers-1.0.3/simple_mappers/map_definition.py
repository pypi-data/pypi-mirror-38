...
class MapDefinition(BaseMapper):
    ...
    def to_object(self, obj, remove_nones=False):
        ...
        for prop_name, prop_def in self.properties.items():
            ...
            attr_val = prop_def.deflate(attr_val, remove_nones=remove_nones)  # TODO when val is a MAPPING DEFINITION CALL TO_OBJECT???
        ...
        for prop_name, prop_def in self.properties.items():
            ...
            if not isinstance(attr_val, BaseProperty) and not inspect.isclass(attr_val):
                ...
                if isinstance(attr_val, MapDefinition):
                    attr_val = attr_val.to_dict(should_map=should_map, remove_nones=remove_nones)
                else:
                    attr_val = prop_def.deflate(attr_val, should_map=should_map, remove_nones=remove_nones)
