...
def packages_manage(package_file_data, manage, targets = None, is_dev = False):
    ...
    if manage == Manage.GET:
        ...
        for target in targets:
            pkg = cover_pkg(target)
            pkg_key = pkg.keys()[0]
            if pkg_key in package_file_data['dependencies']:
                del package_file_data['dependencies'][pkg_key]
            if pkg_key in package_file_data['devDependencies']:
                del package_file_data['devDependencies'][pkg_key]
    ...
    if not utils.check_file_exists(os.path.join(params.name, package_file) if params.name else package_file):
        ...
        if params.env:
            virtualenv = {
                'cmd': 'virtualenv',
                'args': [
                    os.path.join(os.getcwd(), os.path.dirname(os.path.join(params.name, package_file) if params.name else package_file), 'ENV')
                ]
            }
            ...
            if not utils.cmd_with_check_os_value(cmd_string):
                utils.cmd_with_check_os_value('source ${0}'.format(os.path.join(os.getcwd(), os.path.dirname(os.path.join(params.name, package_file) if params.name else package_file), 'venv', 'bin', 'activate')))
