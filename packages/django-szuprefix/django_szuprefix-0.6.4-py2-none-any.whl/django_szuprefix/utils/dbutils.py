...
def transfer_table(db_src, db_dist, sql_select, table_name, chunk=1000, insert_type="", primary_keys=None,
                   update_timestamp_field=None, lower_field_name=False):
    ...
    field_holders = ['%s'] * len(cur_src.description)
    if update_timestamp_field:
        fields.append(update_timestamp_field)
        field_holders.append('now()')
    ...
    if vendor == "mysql":
        ...
        if insert_type == "replace":
            fns = ",".join(["%s=excluded.%s" % (f, f) for f in fields])
            qs_end = " on conflict(%s) do update set %s" % (primary_keys, fns)
        else:
            qs_end = " on conflict(%s) do nothing" % primary_keys
    field_names = ",".join(fields)
    field_values = ",".join(field_holders)
    query = "%s into %s (%s) values (%s) %s" % (qs_type, table_name, field_names, field_values, qs_end)
    print query
    i = 0
    c = 0
    for row in cur_src:
