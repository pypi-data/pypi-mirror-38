...
def transfer_table(db_src, db_dist, sql_select, table_name, chunk=1000, insert_type="", primary_keys=None,
    ...
    if update_timestamp_field:
        update_values[update_timestamp_field] = 'now()'
    sql = gen_batch_insert_sql(table_name, fields, primary_keys, insert_type=insert_type, update_values=update_values,
                               vendor=vendor)
    # print sql
...
def gen_batch_insert_sql(table_name, fields, primary_keys, insert_type='replace', insert_values={}, update_values={},
    ...
    return "%s into %s (%s) values (%s) %s %s" % (qs_type, table_name, insert_field_names, insert_field_values, praise_on_update, qs_end)
