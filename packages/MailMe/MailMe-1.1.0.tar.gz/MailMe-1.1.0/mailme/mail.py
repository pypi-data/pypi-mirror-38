...
class GenericMessage(MIMEMultipart):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for file in filter(isfile, self.attachments):
...
class Sender:
    ...
    def __enter__(self):
        if self.ssl:
            self.logger.info('Started SMTP server with TLS connection.')
            self.server = smtp.SMTP_SSL(f'{self.host}: {str(self.port)}')

        else:
            self.logger.info('Started SMTP server.')
            self.server = smtp.SMTP(f'{self.host}: {str(self.port)}')

        if self.tls:
            self.logger.debug('Initiated communication using TLS.')
            self.server.starttls()

        self.server.ehlo()

        response = self.server.ehlo_resp.decode('utf-8')
        for index, message in enumerate(response.split('\n')):
            self.logger.debug(f'EHLO response: line {str(index)} - {message}')

        login_response = self.server.login(self.user, self.password)

        for index, message in enumerate(login_response):
            msg = message.decode('utf-8') if isinstance(message, bytes) else message
            self.logger.debug(f'Login response: line {str(index)} - {msg}')

        return self.server

    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.server:
            self.logger.info('Connection with SMTP server terminated.')
            self.server.close()
