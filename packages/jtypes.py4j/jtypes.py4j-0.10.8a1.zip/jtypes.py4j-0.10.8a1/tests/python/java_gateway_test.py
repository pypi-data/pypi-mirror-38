...
import sys                         # <AK> added
PY3 = sys.version_info.major >= 3  # <AK> added
...
from jt.py4j.compat import range, long                               # <AK> modified
if PY3:                                                              # <AK> added
    from queue import Queue                                          #    -||-
else:                                                                #    -||-
    from Queue import Queue                                          #    -||-
isbytearray = lambda s: isinstance(s, bytearray)                     #    -||-
bytearray2 = bytes if PY3 else bytearray                             #    -||-
ispython3bytestr = lambda s: isinstance(s, bytes) if PY3 else False  #    -||-
from jt.py4j.finalizer import ThreadSafeFinalizer
...
PY4J_JAVA_PATHS = [
    # <AK> modified
    os.path.join(top_dir,"src","jt","py4j","_java","py4j.jar"),
    os.path.join(test_dir,"java","classes"),
    os.path.join(test_dir,"java","resources"),
   #os.path.join(PY4J_PREFIX_PATH,"../../../../py4j-java/bin"),
    # </AK> modified
]
        ...
        try:
            ...
            with open(errpath, "r") as stderr:
                ...
                if not stderr_is_polluted(lines[0]):
                    ...
                    # Go figure
                    self.assertEqual("Test2" + os.linesep, lines[0])  # <AK> was: ("Test2\n",
        finally:
            os.close(out_handle)
            os.close(err_handle)
            if os.path.exists(outpath): os.unlink(outpath)  # <AK> was: os.unlink(outpath)
            if os.path.exists(errpath): os.unlink(errpath)  # <AK> was: os.unlink(errpath)
