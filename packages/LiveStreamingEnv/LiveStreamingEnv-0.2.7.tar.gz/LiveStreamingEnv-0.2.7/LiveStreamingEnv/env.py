...
ADD_FRAME = 0 
class Environment:
    def __init__(self, all_cooked_time, all_cooked_bw, all_cooked_rtt, random_seed=RANDOM_SEED, logfile_path='./', VIDEO_SIZE_FILE ='./video_size_', Debug = True):
    ...
    def get_video_frame(self, quality, target_buffer):
        ...
        if self.time < self.cdn_arrive_time[quality][self.video_chunk_counter] and not end_of_video: # CDN can't get the frame
            ...
            # calculate the play time , the real time ,the latency
                ...
                self.latency = (self.newest_frame - self.video_chunk_counter) * 0.04  + self.buffer_size
                ...
                if self.Debug:
                    self.log_file.write("real_time %.4f\t"%self.time +  
                                      ...
                                      "play_time_len %.4f\t"% (cdn_rebuf_time * play_duration_weight) +
                                      "download_id %d\t"%self.video_chunk_counter + 
                ...
                return  [self.time,                             # physical time
                        ...
                        CHUNK_TIME_LEN,                         # frame time len
                        rebuf,                                  # rebuf len
                        self.buffer_size,                       # client buffer
                        rtt,                                    # rtt
                        (cdn_rebuf_time * play_duration_weight),# play time len
