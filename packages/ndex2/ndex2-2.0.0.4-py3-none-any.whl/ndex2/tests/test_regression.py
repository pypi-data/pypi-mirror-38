...
class TestRegression(unittest.TestCase):
    ...
    def test_missing_nodes(self):
        print('Testing: Missing nodes')
        nice_cx_builder = NiceCXBuilder()
        nice_cx_builder.add_network_attribute(name='void', values='where prohibited')
        nice_cx = nice_cx_builder.get_nice_cx()
        try:
            self.assertRaises(Exception, nice_cx.upload_to(upload_server, upload_username, upload_password))
        except Exception as e:
            print('Corectly detected missing nodes')
    ...
    def test_create_from_server(self):
        ...
        nice_cx = ndex2.create_nice_cx_from_server(server='dev.ndexbio.org', username='scratch', password='scratch',
                                                  uuid='0b5ab6c1-d160-11e8-adc1-525400c25d22')

        nice_cx.to_neo4j()

        print('Done')

    def test_500k_edges(self):
        with open('rf_500k.txt', 'r', encoding='utf-8', errors='ignore') as tsvfile:
            header = ['s', 't', 'weight']
            df = pd.read_csv(tsvfile, delimiter='\t', na_filter=False, engine='python', names=header,
                             dtype=str, error_bad_lines=False)

            #nice_cx_df_3_column = ndex2.create_nice_cx_from_pandas(df, source_field='s', target_field='t',
            #source_node_attr=[], target_node_attr=[], edge_interaction='interacts-with', edge_attr=['weight'])

            nice_cx_df_3_column = ndex2.create_nice_cx_from_pandas(df, source_field='s', target_field='t',
            source_node_attr=[], target_node_attr=[], edge_interaction='weight', edge_attr=[])

            #nice_cx_df_3_column = ndex2.create_nice_cx_from_pandas(df)

            nice_cx_df_3_column.print_summary()

            upload_message = nice_cx_df_3_column.upload_to(upload_server, upload_username, upload_password)
