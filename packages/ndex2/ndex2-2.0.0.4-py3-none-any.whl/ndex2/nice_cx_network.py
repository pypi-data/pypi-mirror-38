...
class NiceCXNetwork():
    ...
    def _to_neo4j(self):
        ...
        for k, v in self.get_nodes():
            if self.get_node_attributes(v) is not None:
                node_attrs = {n.get('n'): n.get('v') for n in self.get_node_attributes(v)}
            else:
                node_attrs = {}
        ...
        for k, v in self.get_edges():
            if self.get_edge_attributes(v) is not None:
                edge_attrs = {n.get('n'): n.get('v') for n in self.get_edge_attributes(v)}
            else:
                edge_attrs = {}
        ...
        '''
        raise Exception('Not currently supported.  Please see example tutorials for how to convert to neo4j')
