...
class RethinkDBBackendAdapter(AbstractBackendAdapter):
    ...
    def ensure_representation_compatibility(self, value):
        if isinstance(value, datetime):
            return value.replace(microsecond=0)
        if isinstance(value, dict):
            return {self.ensure_representation_compatibility(k): self.ensure_representation_compatibility(v) for k, v in value.items()}
        if isinstance(value, (list, tuple)):
            return type(value)(self.ensure_representation_compatibility(x) for x in value)
        return value
