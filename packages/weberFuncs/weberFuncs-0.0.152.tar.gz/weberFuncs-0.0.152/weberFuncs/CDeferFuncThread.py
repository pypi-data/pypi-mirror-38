...
class CDeferFunc:
    # 延迟函数
    def __init__(self, sFuncName, cbFunc, iDeferSeconds, *args, **kwargs):
    ...
    def __str__(self):
        return 'CDeferFunc(%s,%s,%s)' % (self.sFuncName, self.sYmdHnsAddTime, self.iDeferSeconds)
...
class CDeferFuncThread:
    ...
    def AddDeferFunc(self, sFuncName, cbFunc, iDeferSeconds, *args, **kwargs):
        # 增加一个延迟事件
        oEvent = CDeferFunc(sFuncName, cbFunc, iDeferSeconds, *args, **kwargs)
