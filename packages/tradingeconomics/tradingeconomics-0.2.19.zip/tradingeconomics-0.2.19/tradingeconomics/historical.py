...
#        credentials = 'guest:guest'
    linkAPI = 'http://api.tradingeconomics.com/historical/country/' + urllib.quote(country) + '/indicator/' + urllib.quote(indicator) + '?c='+lg.apikey;
...
#        credentials = 'guest:guest'
#    else:
    if lg.apikey !='guest:guest':
        fn.credCheck(lg.apikey)
    linkAPI = linkAPI + '?c='+lg.apikey
    webResults = json.load(urllib.urlopen(linkAPI))
    if len(webResults) > 0:
        date = [d['DateTime'] for d in webResults]       
