...
def s3_throughput(s3_bucket_name, trial=1, filesize_list=[64, 256, 1024], max_concurrency_list=[10], max_io_queue_list=[1000], num_process_list=[1], random_data=True, debug=False):
...
def main():
    ...
    if args.job == 's3-throughput-small':
        if args.target_s3:
            result = s3_throughput(args.target_s3, debug=args.debug)
            print(result.df)
            if args.bucket:
