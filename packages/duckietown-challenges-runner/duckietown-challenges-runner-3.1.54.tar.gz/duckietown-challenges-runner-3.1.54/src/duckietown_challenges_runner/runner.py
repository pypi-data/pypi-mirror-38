...
from duckietown_challenges.challenge_results import read_challenge_results, ChallengeResults, ChallengeResultsStatus, \
    NoResultsFound
...
from duckietown_challenges.utils import safe_yaml_dump, friendly_size, indent
...
def setup_logging(wd):
    ...
    my_stderr = Tee(os.path.join(wd, 'evaluator.stderr.log'), sys.stderr)
    sys.stderr = my_stderr
    my_stdout = Tee(os.path.join(wd, 'evaluator.stdout.log'), sys.stdout)
    sys.stdout = my_stdout
    ch = logging.StreamHandler(my_stderr.file)
...
def go_(submission_id, do_pull, more_features, do_upload, delete, reset, evaluator_name, machine_id, tmpdir,
    ...
    try:
        ...
        elogger.error(msg)
        status = ChallengeResultsStatus.ABORTED
...
def get_id_for_service(wd, project, service_name):
...
def write_logs_worker(service_name, container_id):
    ...
    # my_logger = elogger.getChild(service_name)
...
def run_one(wd, project, do_pull, monitor, services, timeout=600):
    client = check_docker_environment()
...
def run_docker(cwd, project, cmd0, get_output=False):
    ...
    tee_stdout = Tee('docker-stdout.txt', sys.stdout)
    tee_stderr = Tee('docker-stderr.txt', sys.stderr)
...
def logs_for_container(client, container_id):
    ...
    logs = container.logs(stdout=True, stderr=True, timestamps=True)  # , until=now)
