...
def is_valid_pair(base_fn):
    return (
        path.exists("{}.txt".format(base_fn)) or \
        path.exists("{}.TXT".format(base_fn))
    )
...
def do(dir_base):
    ...
    def text_file(base):
        if path.isfile(base + ".txt"):
            return base + ".txt"
        elif path.isfile(base + ".TXT"):
            return base + ".TXT"
        else:
            raise ValueError("{} is not the base name of a .{txt,TXT} file.".format(base))
    ...
    for fn in fns_valid:
        keyword = read_keyword(text_file(fn))
