...
from lightroom_export_organizer.make_folders import do, undo, validate_file_pairs, is_valid_pair, read_keyword
...
def test_do(test_info, tmpdir_factory):
    ...
    for base_name, (old_path, keyword) in info['valid'].items():
        ...
        assert path.isfile(path.join(dir_base, keyword, base_name) + ".TXT")
