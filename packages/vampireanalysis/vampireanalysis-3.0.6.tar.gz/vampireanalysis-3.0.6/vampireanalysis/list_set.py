...
def modeling_ledger(exp):
	d= {'condition': [],
		'set number':[],
		'boundary stack location':[],
		'note':[]}	
	emptycsv_df = pd.DataFrame(data=d)
	emptycsv_df = emptycsv_df[['condition','set number','boundary stack location','note']]
...
def list_set(folder): #folder == cpoutput folder
	...
	d= {'condition': expname,
		...
		'boundary stack location':setfolderpaths,
		'note':['None']*len(setfolders)}	
	dd = dict([ (k,pd.Series(v)) for k,v in d.items()])
	df = pd.DataFrame(data=dd)
	df = df[['condition','set number','boundary stack location','note']]
