...
def createimstack(ch,setfolder):
	...
	imlist = [_ for _ in imlist if re.split('_+',_)[0].lower() == ch.lower()] 
...
def getboundary(chlist,folder,progress_bar):
	...
	for setfolderidx, setfolder in enumerate(setpaths):
		...
		for ch in chlist:
			...
			for imidx, im in enumerate(imstack):
				...
				#dst = os.path.join(setfolder,imlist[imidx]+'_ssam_D_stack.pickle')
...
# c1 = 'Cells' 
# c2 = 'Nuclei'
# getboundary([c1,c2],'C:\\Users\\kuki\\Desktop\\cpoutput315\\Experiment1\\labeled image')
