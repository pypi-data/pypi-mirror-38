...
from .common import MAX_NUM_BLOCKS, BLOCK_SIZE, NUM_FEATURES, NUM_BLOCKS, module_logger, \
    new_shap_api, minimum_shap_version
...
class BaseExplainer:
    ...
    def _order_imp(self, summary):
        if new_shap_api:
            ...
            module_logger.debug(
                'An older version of SHAP than the mimimum requirement {} is used'.format(minimum_shap_version))
    ...
    def _upload_artifact(self, upload_dir, artifact_name, values):
        try:
            ...
            self.run.upload_file('{}.json'.format(artifact_name), stream)
    ...
    def _upload_model_summary(self, upload_dir, name, summary):
        ...
        for idx in range(num_blocks):
            ...
            artifacts[idx]['prefix'] = os.path.normpath('{}/{}/{}'.format(RUN_ORIGIN,
                                                                          self.run.id,
                                                                          upload_dir + block_name))
            start += block_size
        res = self.assets_client.create_asset(name, artifacts,
                                              metadata_dict=None,
                                              run_id=self.run.id)
