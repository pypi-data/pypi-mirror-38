...
def fast_login(steam_id, token, session: requests.Session):
    ...
    if resp and resp.json():
        _data = resp.json()['response']
        session.cookies.set('steamLogin', '{}%7C%7C{}'.format(steam_id, _data['token']))
        session.cookies.set('steamLoginSecure', '{}%7C%7C{}'.format(steam_id, _data['token_secure']))
        return True
