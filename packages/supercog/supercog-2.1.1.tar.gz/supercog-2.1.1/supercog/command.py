...
class Command:
    ...
    def __init__(self, *, alternatives = None, info = None, nsfw = None, restriction_info = None, run_in_private = None, server_moderator_only = None, bot_moderator_only = None, min_parameters = None, max_parameters = None, parameters = None, errors = None, command = None, commandDict = None):
        """Creates a new Command object
        There are two (2) different ways to make a `Command` object:
            1.) Clarify the alternatives and info parameters in the constructor.
            2.) Clarify a dictionary that holds the alternatives and the info for the command.
    ...
    def getHTML(self):
        ...
        html += ("  <li><strong><code>{}</code></strong><code>{} {}</code> - {}</li>\n".format(
            "NSFW " if self.isNSFW() else "",
