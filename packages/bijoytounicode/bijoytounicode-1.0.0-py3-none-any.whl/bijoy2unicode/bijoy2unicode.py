...
'কোনো কোনো দিন, অাকাশজোড়া মেঘ করে, কাজে মন বসে না। ডেস্কের সামনের জানালার পর্দা তুলে দিই। স্ক্রিনে তাকাই, দেখি না, দূরবর্তী অাকাশ ও ইমারতে কিছু খোঁজ থাকে। এমন সব দিনের নাম কোমলগান্ধার।'
#TEST_TEXT = "†W‡¯‹i"


def create_conversion_pattern(symbols):
    return '|'.join(sorted(re.escape(k)
                           for k in symbols))


CONVERSION_SET = create_conversion_pattern(CONVERSION_MAP)

SYMBOLS_CONVERSION_PATTERN = re.compile(r'({})'.format(
    create_conversion_pattern(ALL_SYMBOLS)), re.MULTILINE)

MAIN_CONVERSION_PATTERN = re.compile(r'([wx†‡ˆ‰Š]?)(({})*({})?({})*)({})?([vyz“–~ƒ‚„…]?)'.format(
    create_conversion_pattern(PRE_SYMBOLS_MAP), CONVERSION_SET, create_conversion_pattern(POST_SYMBOLS_MAP), create_conversion_pattern(REFF)), re.MULTILINE)

HASAANT_PATTERN = re.compile(r'({})+'.format(re.escape('্')), re.MULTILINE)

PRE_CONVERSION_PATTERN = re.compile(r'({})'.format(
    '|'.join(sorted(re.escape(k) for k in PRE_CONVERSION_MAP))), re.MULTILINE)


def replace_hasant(m):
    return "্"
...
def main_converter(match):
    ...
    core = HASAANT_PATTERN.sub(replace_hasant, core)
