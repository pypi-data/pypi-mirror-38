...
class Talk(object):
    ...
    @loggedIn
    def Qingqun(self):
        for groups in self.talk.getGroupIdsInvited():
            self.talk.rejectGroupInvitation(0, groups)
            print("已取消[ "+self.getGroup(groups).name+" ]群組邀請")
        return
    
    @loggedIn
    def Qingqunn(self):
        for groups in self.talk.getGroupIdsInvited():
            if '邀機' in self.getGroup(groups).name:
                self.talk.acceptGroupInvitation(0, groups)
                self.talk.leaveGroup(0, groups)
            else:
                self.talk.acceptGroupInvitation(0, groups)
        return
