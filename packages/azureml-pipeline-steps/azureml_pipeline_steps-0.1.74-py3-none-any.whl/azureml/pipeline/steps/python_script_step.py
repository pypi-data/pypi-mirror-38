...
class PythonScriptStep(PipelineStep):
    ...
    :type target: azureml.core.compute.DsvmCompute, azureml.core.compute.BatchAiCompute,
        azureml.core.compute.RemoteTarget, azureml.core.compute.HDIClusterTarget,
        azureml.core.compute_target.BatchAITarget, str
    ...
    def __init__(self, script_name, name=None, arguments=None, target=None, runconfig=None,
                 inputs=None, outputs=None, params=None, source_directory=None,
        ...
        :type arguments: [str]
        :param target: Compute target to use
        :type target: DsvmCompute, BatchAiCompute, RemoteTarget, HDIClusterTarget, BatchAITarget, str
        :param runconfig: Runconfig to use
    ...
    def create_node(self, graph, default_datastore, context):
        ...
        for param_name in self._params:
            param_defs.append(ParamDef(name=param_name, set_env_var=True, default_value=self._params[param_name],
                                       env_var_override=self._get_parameter_with_prefix(param_name)))

        param_defs.extend(self._module_param_provider.get_params_list())
        ...
        self._set_compute_params(node, context)
        return node
    ...
    def _set_compute_params(self, node, context):
        """Compute params.
    ...
    def set_batchai_params(self, native_shared_directory=None):
        """
        Set BatchAI native shared directory param.

        :param native_shared_directory: native shared directory
        :type native_shared_directory: str
        """
        self._batchai_params = {'NativeSharedDirectory': native_shared_directory}
    ...
    def _set_runconfig(self, run_config=None):
        ...
        if isinstance(run_config, RunConfiguration):
            ...
            runconfig_params = {'Framework': run_config.framework,
                                ...
                                'ContainerInstanceRegion': run_config.container_instance.region,
                                'ContainerInstanceCpuCores': run_config.container_instance.cpu_cores,
                                'ContainerInstanceMemoryGb': run_config.container_instance.memory_gb,
                                'NodeCount': run_config.batchai.node_count,
    ...
    def _generate_default_runconfig(target):
        ...
        if target.type == "batchai" or target.type == "remote" or target.type == "BatchAI" \
