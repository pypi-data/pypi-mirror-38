...
setup(){
    ...
    echo "using xanity path: $SCRIPTPATH"

    if [[ ! -d "$SCRIPTPATH/.xanity" ]];then
    ...
    if [[ -f .xanity/setupcomplete ]];then
      ...
      if [[ "$UUID" == "$ouid" ]]; then
        echo "found setupcomplete file containing correct UUID... updating."
        #echo "found setupcomplete file containing correct UUID... quitting."
        # panicandquit
      else
    ...
    if [[ "$( conda env list | grep $UUID )" == "" ]]; then
      ...
      conda env create -f conda_environment.yaml -n $UUID
      ...
      #conda activate $UUID && echo "stepped into conda environment"
      conda env update -f conda_environment.yaml -n $UUID && echo "updated conda environment"
    fi
