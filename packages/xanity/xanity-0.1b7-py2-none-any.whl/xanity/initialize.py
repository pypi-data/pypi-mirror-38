...
#####################################""")
    ...
    if not ( path.isfile(gi_pak) and path.isfile(gi_exist) ):
        print('somethings up with the .gitignore files...')
    
    # check ages
    gi_pak_age = os.stat(gi_pak).st_mtime
    gi_exist_age =  os.stat(gi_exist).st_mtime

    if gi_exist_age <= gi_pak_age:
        # existing is newer... probably modified
        os.remove(gi_pak)
        print('left existing .gitignore in place')
    else:
        # overwrite existing
        os.remove(gi_exist)
        os.rename(gi_pak, gi_exist)
        print('deployed .gitignore')
