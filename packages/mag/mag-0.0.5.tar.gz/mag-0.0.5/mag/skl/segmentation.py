...
def safe_distance(predicted_boundary, target_boundary, distance_fn=subtract, no_prediction_penalty=no_prediction_distance, no_target_penalty=no_target_distance):
    ...
    return distance_fn(predicted_boundary, target_boundary)
...
def aligned_offset_1d(predicted_boundaries, target_boundaries, alignment_scoring_fn=iou_1d, take=max, distance_fn=subtract, aggregate_fn=mean):
    ...
    print(paired)
    distances = [safe_distance(*pair, distance_fn) for pair in paired]
    aggregated = [aggregate_fn(error) for error in zip(*distances)]
    return aggregated


def offset_per_class_1d(predicted_boundaries, target_boundaries, alignment_scoring_fn=iou_1d, take=max, distance_fn=subtract, aggregate_fn=mean):
    '''Returns the aggregated distance of predicted boundings boxes to their aligned bounding box based on alignment_scoring_fn and distance_fn
    ...
    return [
        aligned_offset_1d(predicted_boundaries[class_index], target_boundaries[class_index])
