...
def binarize(data:list, cutoff=0.5:float) -> list:
...
def consecutive(data:list, stepsize=1:float) -> list:
...
from kno.py.math import domain
def runs(array:list) -> list:
    ...
    '''
    return [domain(run) for run in consecutive(array)]
