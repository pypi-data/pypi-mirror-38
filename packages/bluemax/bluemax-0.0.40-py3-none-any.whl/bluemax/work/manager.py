...
class Manager:
    ...
    def __init__(self, procedures, workers=2):
        self.clients = []
        self.ioloop = tornado.ioloop.IOLoop.current()
        self.status = Status(self.ioloop, self.publish_status, timeout=1)
        self.procedures = procedures
        self.proc_names = getattr(procedures,'__all__',[p for p in dir(procedures) if p[0] != "_"])
        self.reflection = self.describe(self.procedures, self.proc_names)
    ...
    def add_client(self, client):
        ...
        client.write_message(json_utils.dumps({
            "signal": "set_reflect",
    ...
    def broadcast(self, signal, message, filter_clients=None):
        ...
        '''
        self.ioloop.spawn_callback(self._broadcast_,
            signal, message, filter_clients)
        ...
        for key in filter(lambda x: x[0] != '_', targets):
            ...
            if inspect.isfunction(f):
                ...
                s = OrderedDict([
                    ...
                    ("returns", cls._annotation_to_str_(sig.return_annotation)
                     if sig.return_annotation is not sig.empty else ""),
