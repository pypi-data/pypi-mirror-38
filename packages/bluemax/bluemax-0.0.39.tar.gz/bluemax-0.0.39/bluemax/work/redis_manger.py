...
class RedisManager(Manager):
    ...
    def put(self, content):
        ...
        procedure = getattr(self.procedures, content["method"])
        pn = f'{procedure.__module__}:{procedure.__name__}'
        content["method"] = pn
        yield self.broadcast_redis.execute('rpush',
            options.redis_work_q, json_utils.dumps(content))
