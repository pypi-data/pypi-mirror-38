...
class Model(object):
    ...
    def download(self, target_dir=".", exists_ok=False):
        ...
        if os.path.exists(target_dir):
            existing_target_dir_nodes = set(os.listdir(target_dir))
        else:
            existing_target_dir_nodes = set()
        ...
        for sas, path in sas_to_relative_download_path.items():
            ...
            if not exists_ok and os.path.exists(target_path):
                raise AzureMLException(
                    "File already exists. To overwrite, set exists_ok to True. {}".format(target_path))
                ...
                if not exists_ok:
                    for tar_file_path in tar.getnames():
                        ...
                        if os.path.exists(candidate_path):
                            raise AzureMLException(
                                "File already exists. To overwrite, set exists_ok to True. {}".format(candidate_path))
                tar.extractall(path=target_dir)
                tar_paths = tar.getnames()
            file_paths = [os.path.commonprefix(tar_paths)]
            if os.path.exists(tar_path):
                os.remove(tar_path)
            else:
                module_logger.warning("tar_path to unpack is already deleted: {}".format(tar_path))
        else:
            file_paths = [path for path in os.listdir(target_dir) if path not in existing_target_dir_nodes]
        if len(file_paths) != 1:
            raise AzureMLException(
                "Illegal state. Unpack={}, Paths in target_dir is {}".format(self.unpack, file_paths))
        return os.path.join(target_dir, file_paths[0])
