...
ATTRIBUTES = ['resource_group', '_distributed_backend', 'worker_count', 'snapshot_project', 'packages', 'group',
              ...
              'databricks', 'configuration', 'path_on_compute', 'data_references', 'region', 'data_store_name',
              'max_run_duration_seconds', '_name', 'base_image_registry', 'conda_dependencies_file',
              'precache_packages', 'docker', '_path', 'amlcompute', 'vm_size', 'vm_priority',
              'source_directory_data_store', '_script_params', '_retain_cluster', '_cluster_max_node_count',
              'retain_cluster', 'maven_libraries', 'pypi_libraries', 'egg_libraries', 'jar_libraries',
              'rcran_libraries', 'coordinates', 'repo', 'exclusions', 'package', 'library']
...
class MavenLibrary(AbstractRunConfigElement):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ATTRIBUTES:
            super(MavenLibrary, self).__setattr__(name, value)
        else:
            raise AttributeError("MavenLibrary has no attribute %s" % name)
...
class PyPiLibrary(AbstractRunConfigElement):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ATTRIBUTES:
            super(PyPiLibrary, self).__setattr__(name, value)
        else:
            raise AttributeError("PyPiLibrary has no attribute %s" % name)
...
class RCranLibrary(AbstractRunConfigElement):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ATTRIBUTES:
            super(RCranLibrary, self).__setattr__(name, value)
        else:
            raise AttributeError("RCranLibrary has no attribute %s" % name)
...
class JarLibrary(AbstractRunConfigElement):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ATTRIBUTES:
            super(JarLibrary, self).__setattr__(name, value)
        else:
            raise AttributeError("JarLibrary has no attribute %s" % name)
...
class EggLibrary(AbstractRunConfigElement):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ATTRIBUTES:
            super(EggLibrary, self).__setattr__(name, value)
        else:
            raise AttributeError("EggLibrary has no attribute %s" % name)
...
class DatabricksEnvironment(AbstractRunConfigElement):
    ...
    def __setattr__(self, name, value):
        ...
        if name in ATTRIBUTES:
            super(DatabricksEnvironment, self).__setattr__(name, value)
        else:
            raise AttributeError("DatabricksEnvironment has no attribute %s" % name)
