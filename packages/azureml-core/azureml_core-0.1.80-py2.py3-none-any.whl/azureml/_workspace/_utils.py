...
from .arm_template_builder import (
    ...
    build_batchai_workspace_resource
)
...
def arm_deploy_template_new_resources(auth, resource_group_name,
    ...
    if batchai_workspace is None:
        batchai_name = get_name_for_dependent_resource(workspace_name, 'batchai_workspace')

        normalized_location = location.lower().replace(" ", "")
        if normalized_location in ['eastus2euap', 'centraluseuap']:
            batchai_workspace_location = 'eastus'
        else:
            batchai_workspace_location = normalized_location

        master_template.add_resource(build_batchai_workspace_resource(batchai_name, batchai_workspace_location))
        workspace_dependencies.append(
            "[resourceId('{}/{}', '{}')]".format('Microsoft.BatchAI', 'Workspaces', batchai_name))
        batchai_workspace = get_arm_resourceId(
            subscription_id,
            resource_group_name,
...
def get_batchai_workspace(auth, subscription_id, resource_group_name, batchai_workspace_name):
    """Gets batchai workspace"""
    rg_scope = "subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}".format(
        subscriptionId=subscription_id, resourceGroupName=resource_group_name)
    batchai_workspace_id = rg_scope + "/providers/Microsoft.BatchAI/workspaces/{name}".format(
        name=batchai_workspace_name)
    host = get_active_cloud().endpoints.resource_manager
    header = auth.get_authentication_header()
    url = host + batchai_workspace_id + "?api-version=2018-05-01"
    return requests.get(url, headers=header)


def delete_batchai_workspace(auth, resource_group_name, batchai_workspace_name, subscription_id):
    """Deletes batchai workspace"""
    rg_scope = "subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}".format(
        subscriptionId=subscription_id, resourceGroupName=resource_group_name)
    batchai_workspace_id = rg_scope + "/providers/Microsoft.BatchAI/workspaces/{name}".format(
        name=batchai_workspace_name)
...
def delete_batchai_workspace_armId(auth, batchai_workspace_armId, throw_exception=False):
    """Deletes batchai workspace"""
    try:
        _check_valid_arm_id(batchai_workspace_armId)
        subcription_id, resource_group, resource_name \
            = _get_subscription_id_resource_group_resource_name_from_arm_id(batchai_workspace_armId)
        return delete_batchai_workspace(auth, resource_group, resource_name, subcription_id)
    except Exception:
        if throw_exception:
            raise
