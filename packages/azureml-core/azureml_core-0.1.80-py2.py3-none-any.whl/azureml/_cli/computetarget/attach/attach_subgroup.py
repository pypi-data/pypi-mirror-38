...
class AttachSubGroup(abstract_subgroup.AbstractSubGroup):
    ...
    def _command_attach_batchai(self):
        function_path = "azureml._base_sdk_common.cli_wrapper.cmd_computetarget_attach#attach_batchai"

        subscription_id = argument.SUBSCRIPTION_ID.get_required_true_copy()
        subscription_id.help = "Specifies the subscription Id for the batch AI cluster."

        resource_group = argument.RESOURCE_GROUP_NAME.get_required_true_copy()
        resource_group.help = "Specifies the resource group for the batch AI cluster."
        resource_group.function_arg_name = "resource_group"

        batchai_workspace_name = argument.Argument(
            "batchai_workspace_name", "--batchai-workspace-name", "",
            help="Specifies the BatchAI workspace name.")

        cluster_name = argument.Argument("cluster_name", "--cluster-name", "",
                                         help="Specifies the batch AI cluster name.", required=True)

        node_count = argument.Argument("node_count", "--node-count", "", required=True,
                                       help="Specifies the maximum number of nodes to use while "
                                            "running an experiment.", default=1)

        return cli_command.CliCommand("batchai", "Attach batchai as a compute target.",
                                      [argument.TARGET_NAME.get_required_true_copy(),
                                       argument.BASE_DOCKER_IMAGE, subscription_id,
                                       resource_group,
                                       batchai_workspace_name,
                                       cluster_name,
                                       node_count,
