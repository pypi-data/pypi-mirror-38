...
from azureml._base_sdk_common.service_discovery import CachedServiceDiscovery
...
class _ServiceKeys(object):
    ...
    EXPERIMENT_GRAPHS_DISCOVERY_KEY = "experimentgraphs"
    ...
    EXPERIMENT_GRAPHS = "experimengraphs"
    ...
    def initialize(cls):
        ...
        cls.register_discovery_url(cls.EXPERIMENT_GRAPHS, cls.EXPERIMENT_GRAPHS_DISCOVERY_KEY)
    ...
    def generate_environment_variable(cls, key):
        ...
        module_logger.debug("Registered dev url override {}".format(env_var))
        return env_var
...
class ServiceContext(ChainedIdentity):
    ...
    def _fetch_endpoints(self):
        ...
        :rtype: dict[str] -> str
        """
        service_discovery = CachedServiceDiscovery(self._authentication)
        scope = self._get_workspace_scope()

        endpoints = dict()
        for discovery_key in _ServiceKeys.get_discovery_keys():
            endpoints[discovery_key] = service_discovery.get_service_url(scope, discovery_key, self._channel)
        return endpoints
    ...
    def _get_endpoint(self, key):
        ...
        if developer_endpoint is not None:
            self._logger.debug("Loaded endpoint {} from environment variable {}.".format(developer_endpoint,
    ...
    def _get_experiment_graphs_url(self):
        """
        Return the url to the experiment graphs service.

        :return: The experiment graphs service endpoint.
        :rtype: str
        """
        return self._get_endpoint(_ServiceKeys.EXPERIMENT_GRAPHS)
