...
from azureml._file_utils import download_file, upload_blob_from_stream
...
class ArtifactsClient(WorkspaceClient):
    ...
    def upload_stream_to_existing_artifact(self, stream, artifact, content_information,
        ...
        blob_name = artifact.artifact_id
        res = upload_blob_from_stream(stream, artifact_uri, blob_name, content_type=content_type, session=session)
        return res
        ...
        for i in range(0, len(names), batch_size):
            with TaskQueue(worker_pool=self._pool, _ident="upload_files", _parent_logger=self._logger) as task_queue:
                ...
                for path, name in zip(batch_paths, batch_names):
                    ...
                    task = task_queue.add(perform_upload, path, artifact, artifact_content_info, session)
                    results.append(task)
