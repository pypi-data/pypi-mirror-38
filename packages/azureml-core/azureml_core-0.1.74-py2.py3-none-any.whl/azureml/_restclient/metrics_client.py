...
def logged_convert(type_func, value):
    """convert metric value to a type"""
    module_logger.debug("Converting metric of {} to {}.".format(type(value), repr(type_func)))
    return type_func(value)


def convert_to_repr(value):
    """Convert metric value to string"""
    #  returns repr(value) if set else str(value)
    module_logger.warning("Converting metric of {} to string, "
                          "original type is unsupported".format(type(value)))
    return repr(value)


def convert_to_float(value):
    """Convert metric value to float"""
    return logged_convert(float, value)


def convert_to_int(value):
    """Convert metric value to int"""
    return logged_convert(int, value)


def convert_to_str(value):
    """Convert metric value to string"""
    return logged_convert(str, value)


def convert_to_bool(value):
    """Convert metric value to boolean"""
    return logged_convert(bool, value)


class MetricsClient(ExperimentClient):
    ...
    try:
        ...
        _type_to_converter[np.bool_] = convert_to_bool

        # Add str type support
        _type_to_converter[np.unicode_] = convert_to_str
        ...
        for numpy_int_type in numpy_ints + numpy_unsigned_ints:
            _type_to_converter[numpy_int_type] = convert_to_int

        # Add float type support
        numpy_floats = [np.float16, np.float32, np.float64]

        for numpy_float_type in numpy_floats:
            _type_to_converter[numpy_float_type] = convert_to_float

        try:
            # Support large float as string
            _type_to_converter[np.float128] = convert_to_repr
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._post_task_queue = TaskQueue(worker_pool=self._pool,
                                          _ident="PostMetrics",
    ...
    def get_converted_value(cls, value):
        ...
        if value_type in cls._type_to_metric_type:
            ...
            converter = cls._type_to_converter.get(value_type, convert_to_repr)
            module_logger.debug("Converter for value is " + converter.__name__)
            return converter(value)

    @classmethod
    def to_cells(cls, keys, values):
        """return metrics cell as list of dictionary"""
        cells = [{key: cls.get_converted_value(value)} for key, value in zip(keys, values)]
        return cells
    ...
    def create_run_metric_dto(cls, run_id, name, value,
                              description="", metric_type="vienna.custom"):
    ...
    def flush(self):
        with self._log_context("FlushingMetricsClient"):
            self._post_task_queue.flush(self.identity)

    def _log(self, run_id, *args, **kwargs):
        run_metric_dto = MetricsClient.create_run_metric_dto(run_id, *args, **kwargs)
        if self.api_route == MetricsClient.OLD_ROUTE:
            func = self._client.legacy_runmetric.post
        else:
            func = self._client.run_metric.post

        async_task = self._execute_with_project_arguments(func,
                                                          run_id,
                                                          run_metric_dto,
                                                          is_async=True)
        self._post_task_queue.add_task(async_task)
        return async_task

    def _log_image(self, run_id, name, value, **kwargs):
        cell = MetricsClient.to_cells([name], [value])
        return self._log(run_id, name, cell,
                         metric_type=AZUREML_IMAGE_METRIC_TYPE, **kwargs)

    def log_list(self, run_id, name, values, description=""):
        ...
        return self._log(run_id, name, cells,
                         description=description,
                         metric_type=AZUREML_LIST_METRIC_TYPE)

    def log_scalar(self, run_id, name, value, description=""):
    ...
    def log_row(self, run_id, name, value, description=""):
    ...
    def log_table(self, run_id, name, value, description=""):
        """log a table metric"""
