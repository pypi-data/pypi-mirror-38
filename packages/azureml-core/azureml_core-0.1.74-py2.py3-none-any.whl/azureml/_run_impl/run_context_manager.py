...
from azureml._history.utils.context_managers import LoggedExitStack, TrackCM, CatchValidExits
from azureml._history.utils.daemon import Daemon
...
class RunHeartBeatContext(BaseRunContext):
    ...
    def __init__(self, run, interval_sec=DEFAULT_INTERVAL_SEC, **kwargs):
        ...
        self._daemon = Daemon("RunHeartBeat",
                              interval_sec=buffered_interval_sec,
                              logger=self._logger,
                              work_func=self.run._heartbeat)
...
class RunContextManager(BaseRunContext):
    def __init__(self, run, outputs=None, logs=None, **kwargs):
