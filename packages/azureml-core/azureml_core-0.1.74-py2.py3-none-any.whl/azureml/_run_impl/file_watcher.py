...
class FileWatcher(Thread):
    ...
    def create_artifacts(self, new_files):
        try:
            ...
            blob_names = [artifact.artifact_id for artifact in artifacts]
            artifact_uris = [res.artifact_content_information[name].content_uri for name in artifact_keys]
            return (blob_names, artifact_uris, True)
        except Exception as ex:
            self.logger.debug("Exception creating artifacts:\n{}".format(ex))
            return ([], [], False)

    def create_blobs(self, blob_names, artifact_uris, files_watched, upload_info, full_paths):
        from azure.storage.blob import AppendBlobService
        try:
            for blob_name, artifact_uri, fullpath in zip(blob_names, artifact_uris, full_paths):
                sas_token, account_name, container_name = get_block_blob_service_credentials(artifact_uri)
    ...
    def walk_files(self, files_watched, files_uploading, upload_info, tq, current_stat):
        ...
        if new_files:
            ...
            (blob_names, artifact_uris, success) = self.create_artifacts(new_files)
            ...
            success = self.create_blobs(blob_names, artifact_uris, files_watched, upload_info, full_paths)
            # Exit on error, stop watcher
            if not success:
