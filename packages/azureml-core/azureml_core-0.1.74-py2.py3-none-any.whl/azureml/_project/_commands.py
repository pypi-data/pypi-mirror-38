...
from azureml._workspace._utils import (
    ...
    delete_batchai_workspace_armId
)
...
def attach_project(auth, resource_group_name, workspace_name, cloud_project_name,
                   subscription_id, project_path=".", location=None):
    """
    ...
    workspace_service_object, _ = _get_or_create_workspace(
        auth, subscription_id, resource_group_name, workspace_name, location)
...
def create_workspace(auth, resource_group_name, workspace_name, subscription_id,
                     ...
                     batchai_workspace=None, exist_ok=False):
    """
    Create workspace
    :param auth:
    :type auth: azureml.core.authentication.AbstractAuthentication
...
def delete_workspace(auth, resource_group_name, workspace_name, subscription_id,
    ...
    try:
        if delete_dependent_resources:
            ...
            delete_batchai_workspace_armId(auth, workspace.batchai_workspace)
