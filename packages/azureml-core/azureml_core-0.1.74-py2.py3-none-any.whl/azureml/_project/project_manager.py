...
_config_folder_name = "aml_config"
...
def create_project(project_id, repo_path, scope, git_client, template_url=None):
    """
    ...
    try:
        ...
        if template_url:
            ...
            git_client.clone(template_url, temp_dir)
            shutil.rmtree(os.path.join(temp_dir, _default_git_folder_name))
            file_utilities.copy_all_files_from_directory(temp_dir, repo_path)
...
def _create_metadata_folders(path):
    ...
    :rtype: None
    """
    file_utilities.create_directory(os.path.join(path, file_utilities.configuration_directory_name))
    file_utilities.create_directory(os.path.join(path, _asset_folder_name))
