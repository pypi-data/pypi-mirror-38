...
def options(opt):
	...
	opt.add_option('--pkg-recipe', dest='pkg_recipe', default='pkg-recipe.json', action='store', help='packaging recipe file')
	...
	opt.add_option('--pkg-maintainer', dest='pkg_maintainer', default='', action='store', help='packaging maintainer')
...
def configure(conf):
	...
	conf.env.PKG_RELEASE = '1'
	conf.env.PKG_LICENSE = 'GPLv2'
	conf.env.PKG_GROUP = 'Development/tools'	
	conf.env.PKG_URL = ''
	conf.env.PKG_RELOCATIONS = ''	
	conf.env.PKG_DESCRIPTION = ''
	conf.env.PKG_REQUIRES = '' # MUST be RPM style dependencies use comma separators
	conf.env.PKG_SECTION = 'misc'
	conf.env.PKG_MAINTAINER = conf.options.pkg_maintainer

	# use recipe file containing packaging meta-information
	pkg_recipe = conf.options.pkg_recipe
	if os.path.exists(pkg_recipe):
		conf.env.PKG_RECIPE = pkg_recipe
		parse_pkg_recipe(conf, pkg_recipe)
