...
def build_deploy_script(project_name, config_name, only_structure=False, remote_dict = None):
    ...
    def host_file(): return os.path.join(yml_file_folder(), debug(get_file_only_name(yml_file_path()), "host file name") + ".host")
    def bash_file(): return os.path.join(yml_file_folder(), get_file_only_name(yml_file_path()) + ".sh")
    def get_roles_data():return roles_load(debug(get_config_path(project_name, config_name), "config_path"))
    ...
    roles_link(get_roles_data(), \
               debug(yml_file_folder(), "link_root") \
    )
