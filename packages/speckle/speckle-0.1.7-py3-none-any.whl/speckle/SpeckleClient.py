...
class SpeckleClient(object):
    ...
    def check_response_status_code(self, r):
        ...
        if self.verbose:
            print()
            self.log(json.dumps(r.json(), indent=4, sort_keys=True))
    ...
    def AddObjects(self, obj, streamId):
        ...
        #payload = {"objects":[obj]}
        #self.log (json.dumps(payload, indent=4, sort_keys=True))
    ...
    def GetObjectBulk(self, objectIds, query):
        url = self.baseUrl + "/objects/getbulk?%s" % query
        r = requests.post(url, data=json.dumps(objectIds), headers=self.CreateHeader())
