...
def get_positions(exchange=None, currency=None):
    ...
    for position in positions:
        ...
        if 1 == position['subject']:
            if currency.strip() and currency.strip() != t_crc.upper():
                continue
...
def get_symbol_of_coins(market, currency_a, currency_b, df=False):
    ...
    preq.market = market
    preq.coin_a = currency_a
    preq.coin_b = currency_b
...
def get_symbol_for_future(exchange, base, quote, period):

    if not exchange or not base or not quote or not period:
