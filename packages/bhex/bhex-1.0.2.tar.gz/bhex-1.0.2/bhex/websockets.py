...
class BhexSocketManager(threading.Thread):
    ...
    STREAM_URL = 'wss://ws.bhex.com/openapi/'
    ...
    def _start_socket(self, id_, path, payload, callback):
        ...
        factory_url = self.STREAM_URL + path
    ...
    def _start_auth_socket(self, id_, payload, callback):
        client = BhexClient(api_key=self._api_key, secret=self._secret)
        listen_key = client.stream_get_listen_key()
        if not listen_key:
            raise BhexRequestException('Get listen key failure.')
        path = 'ws/' + listen_key.get('listenKey', '')
