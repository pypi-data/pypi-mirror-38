...
def kraus_generator(n, non_ideal_qubits=0, ideal_qubits=0,classical_error=False, leave_alone=False, opers=[], prob_error=[]):
    """
    :param n:
    ...
    id_oper = op.superkron(ideal_qubits_oper, val=1, string='0'*ideal_qubits)
    num_operators = len(opers)
    kraus_operators = {str(i): opers[i] for i in range(len(opers))}
    labels = op.createlabel(n, num_operators)
    if classical_error is False and leave_alone is False:
        for i in range(len(labels)):
            kraus = 1
            for digit in labels[i]:
                kraus = np.kron(kraus, kraus_operators[digit])
            yield kraus
    elif classical_error is False and leave_alone:
        assert non_ideal_qubits + ideal_qubits == n, 'Number of ideal and non ideal qubits should add to n'
        for i in range(len(labels)):
            kraus = 1
            for digit in labels[i]:
                kraus = np.kron(kraus, kraus_operators[digit])
            yield op.superkron(kraus, id_oper)


def apply_kraus(K, rho_s, n):
    """
    :param K: List of kraus operators
    :param rho: density matrix of system
    :param n: Number of qubits
...
if __name__ == '__main__':
    ...
    kraus = generic_kraus(1, classical_error=False, opers=E)
    m = [op.superkron(kraus[i]) for i in range(len(kraus))]

    for i in range(4):
        print(i)
        generator_kraus = kraus_generator(1, classical_error=False, opers=E)
        state = apply_kraus(generator_kraus, state, 1)
        print(state)

    for i in range(4):
        state_1 = serial_decohere(m, state_1, 1)
        print(state_1)