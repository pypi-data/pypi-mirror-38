...
class Qubit(object):
    ...
    def qubits_decohere(self, n_s):
        ...
        :return:
        """
        K = ne.kraus_generator(n_s, classical_error=False, opers=self.single_kraus)
        self.state = ne.apply_kraus(K, self.state, n_s)
...
if __name__ == '__main__':
    ...
    init = '11'
    oper_dict = {'0': g.z()}
    E = [np.array([[1, 0], [0, np.cos(np.pi/3)]]), np.array([[0, np.sin(np.pi/3)], [0, 0]])]
    ...
    q2 = Qubit(init, 2, no_measurment_qubits=2)
    q2.single_kraus = E
    q2.dt = 0.01
    q2.T = 1
    ...
    q2.hamiltonian = 0.5*op.superkron(oper_dict, val=1, string='00')
    q2.evolve(basis='ZZ', noise=True)
