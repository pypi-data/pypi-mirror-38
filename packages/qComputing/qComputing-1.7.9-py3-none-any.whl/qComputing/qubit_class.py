...
if __name__ == '__main__':
    ...
    init = '11111'
    oper_dict = {'0': g.z()}
    E = [np.array([[1, 0], [0, np.cos(np.pi/25)]]), np.array([[0, np.sin(np.pi/25)], [0, 0]])]
    k = ne.generic_kraus(3, classical_error=False, opers=E)
    m = [op.superkron(k[i], g.id(), g.id()) for i in range(len(k))]
    q2 = Qubit(init, 5, no_measurment_qubits=5)
    q2.dt = 0.01
    q2.T = 1
    ...
    q2.hamiltonian = 0.5*op.superkron(oper_dict, val=1, string='00000')
    q2.evolve(basis='ZZZZZ', noise=True)
    q2.expectation_values({'XXXXX':op.superkron(g.z(), g.z(), g.z(), g.z(), g.z()), 'YYYYY':op.superkron(g.y(), g.y(), g.y(), g.y(), g.y())})
    q2.graph('11111')
    q2.graph('00111')
    q2.graph('01111')
    q2.graph('00011')
