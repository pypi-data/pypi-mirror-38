...
def read_image(path, dtype='float32', ROI = None):
...
def read_stack2(path, dtype, ROI=None, asarray=True):
    """
    ...
    out = (Parallel(n_jobs=num_cores)(delayed(read_image)(path+'/'+i, dtype, ROI) for i in imlist))
    if asarray:
        out = np.asarray(out)
    return out
