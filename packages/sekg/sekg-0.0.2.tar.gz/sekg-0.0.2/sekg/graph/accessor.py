...
class GraphAccessor:
    ...
    def merge_relation(self, start_node, relation_str, end_node):
        ...
        r = Relationship(start_node, relation_str, end_node)
        self.graph.merge(r)
        modify_time = int(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
        if "_create_time" not in r:
            r["_create_time"] = modify_time
            r["_modify_version"] = 1
        if "_update_time" not in r:
            r["_update_time"] = modify_time
            r["_modify_version"] = r["_modify_version"] + 1

        print("merge id=%d relation version:%d data:%s", r.identity,
              r["_modify_version"], str(r))
        self.graph.push(r)
        return r

    def merge_node(self, node, primary_label, primary_property_key):
        """
        merge a node to graph, and update the create. And will create log and update some metadata.
        :param primary_property_key: the merge primary key
        ...
        if not isinstance(node, Node):
            ...
            return None
        self.graph.merge(node, primary_label, primary_property_key)
        modify_time = int(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
        ...
        print("merge id=%d relation version:%d data:%s", node.identity,
    ...
    def push_node(self, node):
        ...
        """
        if node is None:
            print("fail push node for start node is None")
            return None
        if not isinstance(node, Node):
            print("fail the node is not a node object")

        if not self.is_remote(node):
            print("fail push node for start node is not a remote node %s", str(node))
            return
        modify_time = int(time.strftime('%Y%m%d%H%M%S', time.localtime(time.time())))
        if "_create_time" not in node:
            node["_create_time"] = modify_time
        ...
        print("push id=%d node version:%d data:%s", node.identity,
              node["_modify_version"], str(node))
        self.graph.push(node)
        return node
