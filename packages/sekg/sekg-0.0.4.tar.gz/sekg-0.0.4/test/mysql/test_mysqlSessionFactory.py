...
class TestMysqlSessionFactory(TestCase):
    def test_create_mysql_engine_by_server_name(self):
        ...
        engine = factory.create_mysql_engine_by_server_name("SEKETestServer")
    ...
    def test_create_mysql_session_by_server_name(self):
        ...
        session = factory.create_mysql_session_by_server_name("SEKETestServer")
        self.assertIsNotNone(session)

        session = factory.create_mysql_session_by_server_name("DysD3Wrong")
        self.assertIsNone(session)

    def test_create_mysql_session_by_server_id(self):
        factory = MysqlSessionFactory("mysql_config.json")
        session = factory.create_mysql_session_by_server_id(1)
        self.assertIsNotNone(session)

        engine3 = factory.create_mysql_session_by_server_id(3)
        self.assertIsNone(engine3)
