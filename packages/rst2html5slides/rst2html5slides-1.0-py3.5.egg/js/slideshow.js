...
function getAnimations(index, slide) {
    var curAnimation = null;
    var animation = $(slide).data('animation');
    if (animation) {
        var func = window[animation];
        if (func) {
            curAnimation = func(slide);  // TimelineMax expected
        }
    }
    animations.push(curAnimation);
    return
}
