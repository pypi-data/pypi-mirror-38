...
class SchieberEnv(gym.Env):
    ...
    # index 9 to 31: the cards in the stack which have been turned over in the order of appearance
    # index 32 to 35: the cards currently on the table: 32 --> 1st card, 33 --> 2nd card, 34 --> 3rd card
    observation_space = spaces.Box(low=0, high=36, shape=(35,), dtype=int)
    ...
    def reset(self):
        ...
        if self.observation == {}:
            return asyncio.get_event_loop().run_until_complete(self.get_initial_observation())
        return self.observation_dict_to_index(self.observation)
    ...
    def observation_dict_to_index(self, observation):
        ...
        for i in range(len(observation["table"])):
            table[i] = from_card_to_index(observation["table"][i]["card"])
