...
from schieber.card import Card, from_tuple_to_card, from_card_to_tuple
...
class SchieberEnv(gym.Env):
    ...
    suit_space = spaces.Discrete(4)
    value_space = spaces.Discrete(9)
    ...
    def step(self, action):
        ...
        observation = self.observation_dict_to_tuple(self.observation)
    ...
    async def send_action(self):
        async with websockets.connect(f'ws://localhost:{self.player_port}') as websocket:
            ...
            self.action = from_tuple_to_card(self.action)
