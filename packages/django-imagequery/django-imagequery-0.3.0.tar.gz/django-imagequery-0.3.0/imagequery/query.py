...
class QueryItem(object):
    ...
    def __unicode__(self):
        return u', '.join([str(x.operation) for x in self])
...
class RawImageQuery(object):
    ...
    def _format_extension(self, format):
        try:
            ...
            for ext, _format in list(Image.EXTENSION.items()):
    ...
    def image_format(self, value):
        ...
        if not Image.EXTENSION:
            Image.init()
        if not value in list(Image.EXTENSION.values()):
    ...
    def textimg(text, font, size=None, fill=None, padding=0, mode='RGBA', bg=None, storage=default_storage):
        ...
        if bg is None:
            ...
            if fill:
                for i in range(0, min(len(fill), 3)):
                    bg[i] = fill[i]
        if padding:
