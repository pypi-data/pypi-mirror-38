...
class AgentCom(object):
    ...
    def _start_new_connnection(self):
        ...
        try:
            ...
            if e.code() in (StatusCode.CANCELLED, StatusCode.NOT_FOUND,
        ...
        self._retry_thread = threading.Thread(target=self._command_thread, args=(self._command_request, ),
    ...
    def _command_loop(self, commands_request):
        ...
        # TODO:: verify there is no race here
        self._output.stop_sending_messages()
    ...
    def _start_retry_thread(self):
        logger.debug("Retry thread started")
        if not self._retry_thread:
            self._retry_thread = threading.Thread(target=self._retry_thread,
                                      name=AgentComConfiguration.COMMAND_THREAD_NAME)
            self._retry_thread.daemon = True
            self._retry_thread.start()
