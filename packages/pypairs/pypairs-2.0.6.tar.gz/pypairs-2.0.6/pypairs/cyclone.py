...
def filter_matrix(matrix, gene_names, sample_names, subset_genes, subset_samples, rm_zeros=True):
    ...
    helper.print_info("O", "Filtered out {num} (of {num_old}) genes: {subset} subsetted, {notde} not expressed".format(
    ...
    helper.print_info("O", "Filtered out {subset} (of {num_old}) samples".format(
        subset=len(subset_samples_mask) - sum(subset_samples_mask), num_old = len(subset_samples_mask)), "cyclone")
...
def cyclone(matrix, marker_pairs, gene_names, sample_names, subset_genes=None, subset_samples=None, iterations=1000,
    ...
    scores_df['Prediction'] = np.where(scores_df.max(axis=1) >= 0.5, scores_df.idxmax(axis=1), 'NA (' + str(scores_df.idxmax(axis=1)) + ')')
