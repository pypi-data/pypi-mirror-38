...
def rmp_norm(matrix):
    a = matrix.sum(axis=0) / 1000000
    ret = np.divide(matrix, a, out=np.zeros_like(matrix), where=a != 0)
    return ret
...
def print_init_sandbag(matrix, category_names, categories, fraction, processes):
    print("############################################################")
    print("#", end=" ")
    ...
    print("#", end=" ")
    print_info("I", "\tNo of Categories: {}".format(len(category_names)), "sandbag")
    print("#", end=" ")
    print_info("I", "\tSize of Categories:", "sandbag")
    for c,n in enumerate(category_names):
        print("#", end=" ")
        print_info("I", "\t\t{cat}: {size}".format(cat=n, size=sum(categories[c])), "sandbag")
    print("#", end=" ")
