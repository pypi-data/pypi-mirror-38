...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_17a_update_data_mart(self):
        TestAIOpenScaleClient.ai_client.data_mart.update(db_credentials=self.postgres_credentials, schema=get_schema_name2())
        guids = TestAIOpenScaleClient.ai_client.data_mart.subscriptions.get_uids()
        self.assertTrue(len(guids) > 0)

    def test_17b_data_mart_get_details(self):
        details = TestAIOpenScaleClient.ai_client.data_mart.get_details()
        print(details)
        self.assertTrue(len(json.dumps(details)) > 10)
