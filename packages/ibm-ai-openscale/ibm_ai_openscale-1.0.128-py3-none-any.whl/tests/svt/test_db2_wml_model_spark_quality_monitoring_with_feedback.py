...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.db2_datamart_credentials = get_db2_datamart_credentials()
        self.spark_credentials = get_spark_reference()
        self.schema_name = get_db2_schema_name()

        clean_db2_schema(self.db2_datamart_credentials, self.schema_name)

    def test_01_create_client(self):
        TestAIOpenScaleClient.ai_client = APIClient(self.aios_credentials)

    def test_02_setup_data_mart(self):
        TestAIOpenScaleClient.ai_client.data_mart.setup(db_credentials=self.db2_datamart_credentials, schema=self.schema_name)
    ...
    def test_04_get_wml_client(self):
        ...
        TestAIOpenScaleClient.wml_client = TestAIOpenScaleClient.ai_client.data_mart.bindings.get_native_engine_client(TestAIOpenScaleClient.binding_uid)
    ...
    def test_05_prepare_model(self):
        ...
        print(TestAIOpenScaleClient.deployment_uid)
    ...
    def test_11a_stats_on_feedback_table_from_db2(self):
        tables = list_db2_tables(self.db2_datamart_credentials, self.schema_name)
        print(tables)
        feedback_table = None
        for table in tables:
        ...
        table_content = execute_sql_query(""" SELECT * FROM "{}" """.format(feedback_table), db2_credentials=self.db2_datamart_credentials)
        print(table_content)

        self.assertEqual(len(table_content), 10)

    def test_11b_stats_on_quality_monitoring_table(self):
    ...
    def test_11c_stats_on_feedback_logging_table(self):
