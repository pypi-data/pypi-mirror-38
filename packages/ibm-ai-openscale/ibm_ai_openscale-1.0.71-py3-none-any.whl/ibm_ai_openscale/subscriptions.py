...
from ibm_ai_openscale.engines.custom_machine_learning import CustomMachineLearningAsset
from ibm_ai_openscale.engines.custom_machine_learning import CustomMachineLearningDeployment
...
class Subscriptions:
    ...
    def add(self, asset, add_all_deployments=True, deployment_uids=None):
        ...
        if asset.binding_uid is not None:
            ...
            client = clients[key]
        elif type(asset) == GenericAsset or type(asset)==CustomMachineLearningDeployment:
            clients = self._ai_client._clients_manager.get_all()
        ...
        # TODO it should be replaced by the client method returning such props
        # TODO asset_properties should be set for all engines
        asset_properties = {}

        if type(asset) == CustomMachineLearningDeployment:
            asset_properties = {
                                "scoring_endpoint": {
                                                        "url": asset.scoring_url,
                                                        "request_headers": asset.scoring_header
                                                    }
                                }
