...
@unittest.skip("Skipped due to issue: https://github.ibm.com/NGP-TWC/ml-planning/issues/6399")
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    def test_08_setup_quality_monitoring(self):
        TestAIOpenScaleClient.subscription.quality_monitoring.enable(evaluation_method='multiclass', threshold=0.8, min_records=5, spark_credentials=self.spark_credentials, interval_in_minutes=1)
    ...
    def test_11_stats_on_quality_monitoring_table(self):
        ...
        TestAIOpenScaleClient.runs_no = len(quality_metrics['values'])

    def test_11b_wait_and_check(self):
        import time
        time.sleep(5*60)
        TestAIOpenScaleClient.subscription.quality_monitoring.show_table()
        quality_metrics = TestAIOpenScaleClient.subscription.quality_monitoring.get_table_content(format='python')
        self.assertTrue(len(quality_metrics['values']) >= 4 + TestAIOpenScaleClient.runs_no)
        self.assertTrue(6 + TestAIOpenScaleClient.runs_no >= len(quality_metrics['values']))

    def test_11c_debug(self):
        import requests

        response = requests.get(
            TestAIOpenScaleClient.ai_client._service_credentials['url'] + '/v1prv/data_marts/' + TestAIOpenScaleClient.ai_client._service_credentials['data_mart_id'] + '/subscriptions/' + TestAIOpenScaleClient.subscription.uid + '/triggers/quality/track/events',
            headers=TestAIOpenScaleClient.ai_client._get_headers()
        )

        print(len(response.json()["events"]) > 0)
