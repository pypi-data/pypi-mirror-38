...
class SimpleShading(TimeSeriesComponent):
    def __init__(self, time, solar_angles, surface):
        self.overhang_length = 3
        self.overhang_distance_from_window = 0
        self.overhang_height_above_window = 0
        self.window_height = 4
        self.major_shaded_fraction = None
        self.minor_sunlit_fraction = None
        self.total_shaded_fraction = None
    ...
    def update_calculated_values(self):
        self.calculate_major_shaded_fraction()
        self.calculate_minor_sunlit_fraction()
        self.calculate_total_shaded_fraction()
        print('')

    def calculate_major_shaded_fraction(self):
        overhang_length = self.overhang_length
        overhang_distance_from_window = self.overhang_distance_from_window
        overhang_height_above_window = self.overhang_height_above_window
        window_height = self.window_height
        solar_altitude = np.deg2rad(self.solar_angles.solar_altitude)
        sun_surface_azimuth = np.deg2rad(self.surface.surface_solar_angles.sun_surface_azimuth)
        major_shaded_fraction = ((overhang_length-overhang_distance_from_window)*np.tan(solar_altitude)/np.cos(sun_surface_azimuth)-overhang_height_above_window)/window_height
        major_shaded_fraction[major_shaded_fraction>1] = 1
        major_shaded_fraction[major_shaded_fraction<0] = 0
        self.major_shaded_fraction = pd.Series(major_shaded_fraction, index=time.datetime_range)

    def calculate_minor_sunlit_fraction(self):
        overhang_length = self.overhang_length
        overhang_distance_from_window = self.overhang_distance_from_window
        overhang_height_above_window = self.overhang_height_above_window
        window_height = self.window_height
        solar_altitude = np.deg2rad(self.solar_angles.solar_altitude)
        sun_surface_azimuth = np.deg2rad(self.surface.surface_solar_angles.sun_surface_azimuth)
        minor_sunlit_fraction = (overhang_distance_from_window*np.tan(solar_altitude)/np.cos(sun_surface_azimuth)-overhang_height_above_window)/window_height
        minor_sunlit_fraction[major_shaded_fraction>1] = 1
        minor_sunlit_fraction[major_shaded_fraction<0] = 0
        self.minor_sunlit_fraction = pd.Series(minor_sunlit_fraction, index=time.datetime_range)

    def calculate_total_shaded_fraction(self):
        sun_on_surface = self.surface.surface_solar_angles.sun_on_surface
        major_shaded_fraction = self.major_shaded_fraction
        minor_sunlit_fraction = self.minor_sunlit_fraction
        total_shaded_fraction = sun_on_surface*(major_shaded_fraction-minor_sunlit_fraction)
        total_shaded_fraction[total_shaded_fraction>1] = 1
        total_shaded_fraction[total_shaded_fraction<0] = 0
        self.total_shaded_fraction = pd.Series(total_shaded_fraction, index=time.datetime_range)
