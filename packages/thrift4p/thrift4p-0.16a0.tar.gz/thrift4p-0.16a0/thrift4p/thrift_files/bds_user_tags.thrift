...
service UserTagsManageService {    	 
	 ...
	 //添加数据
	 bool add(1:i32 id, 2:string tagName, 3:i16 weight)
	 //添加临时标签数据,ttl单位毫秒
	 bool addWithTTL(1:i32 id, 2:string tagName, 3:i16 weight, 4:i64 ttl)
     //添加数据
	 bool addData(1:User data)
	 //批量插入
	 bool bulkData(1:list<User> data)
	 //批量插入临时标签数据,ttl单位毫秒
	 bool bulkDataWithTTL(1:list<User> data, 2:i64 ttl)
	 //更新数据，支持批量更新
	 ...
	 list<User> queryDataByTagName(1:string tag,2:i32 size,3:i32 page,4:bool flag)
	 //同时含有这几个标签的用户
	 binary queryDataByTagNames(1:list<string> tags)
	 //根据用户id查询数据
	 list<User> queryDataByUserId(1:list<i32> data)
	 //查询某个用户是否贴有某些标签(同时贴有这些标签)
	 bool isOwnTag(1:i32 user_id ,2:list<string> tagName)
	 //查询某个用户是否贴有某些标签,返回贴有的标签
	 list<string> getUserOwnTags(1:i32 user_id, 2:list<string> tagName)
	 ...
	 bool registerTags(1:map<string,string> tags)
	 bool registerTag(1:string key,2:string value)
	 //删除标签，支持批量删除，同时删除含有该标签的数据，
	 bool deleteTag(1:list<string> tags)
