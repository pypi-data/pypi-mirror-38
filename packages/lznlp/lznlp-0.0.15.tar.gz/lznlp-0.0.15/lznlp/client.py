...
class LZNLP(object):
    ...
    def register_model(self, name, domain_id, type, model_script):
        """LiangZhiNLP 模型注册接口

        :param name: 模型名称
        :type name: string

        :param domain_id: 项目id（需要从项目配置表中查找domain_config）
        :type domain_id: int

        :param type: 模型类型，可以为"主题标引"，"实体抽取"，"事件抽取"
        :type type: string
        ...
        >>> import lznlp
        >>> nlp = lznlp.LZNLP()
        >>> nlp.register_model('xx分类模型', 101, '主题标引', '/path/to/model/script')

        """
        if type in ["主题标引", "实体抽取", "事件抽取"]:
            data_file = {'script':open(model_script, "rb")}
            data_param = {'name': name, 'domainId': str(domain_id), 'type': type}
...
class DeepLearningBaseline(LZNLP):
    ...
    def train(self,
        ...
        params = {"name": name,
                  ...
                  "dropout": str(dropout)}
        if method_name in ["cnn", "lstm", "bilstm"]:
            print("正在训练并评估 " + method_name + " 模型...")
        else:
            return json.loads('{"result":"","status":"模型类型不支持"}')
