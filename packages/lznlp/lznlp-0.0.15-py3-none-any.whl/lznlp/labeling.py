...
class LabelTool(object):
    ...
    def label_ner(self,
        ...
        for item in self.collection.find({}, col_filter)[skip:]:
            ...
            print("Doc " + str(count) + "：\n")
            print(text)
            entity_str = input("输入你从文中发现的实体，以逗号分隔：")
            entities = str(entity_str).split(',')
            offsets = []
            for entity in entities:
                for occur in re.finditer(entity, text):
                    offsets.append(occur.span())
            training_set = ""
            pos = 0
            for char in text:
                flag = 0
                for span in offsets:
                    if span[1] > pos >= span[0]:
                        if pos == span[0]:
                            training_set += char + sep + tag_begin + '\n'
                            flag = 1
                        else:
                            training_set += char + sep + tag_inside + '\n'
                            flag = 1
                        break
                    else:
                        continue
                if flag == 0:
                    training_set += char + sep + tag_outside + '\n'
                pos += 1
