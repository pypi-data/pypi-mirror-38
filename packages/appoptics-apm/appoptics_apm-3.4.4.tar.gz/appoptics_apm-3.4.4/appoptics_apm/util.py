...
def _apm_str(ns):
    """convers non-string type to string if possible
       otherwise retrun is as intact if not unicode.
       * this typically happens py2 unicode string
    """
    try:
        return str(ns)
    except UnicodeEncodeError as ue:
        # 'utf-8 encoding is more safe if there is regex processing etc. for consistence not ns
