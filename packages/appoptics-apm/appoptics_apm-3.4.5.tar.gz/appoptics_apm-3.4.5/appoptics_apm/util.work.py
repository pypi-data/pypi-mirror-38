...
def _get_logger():
    ...
    if ('APPOPTICS_APM_PYTHON_DISABLE_LOGGER' in os.environ and
            os.environ['APPOPTICS_APM_PYTHON_DISABLE_LOGGER'].lower() in ['1', 'true', 'yes']):
...
def appoptics_apm_init():
    ...
    if sys_is_traceable():
        ...
        ret = SwigContext.init(os.environ.get('APPOPTICS_SERVICE_KEY'),
                               os.environ.get('APPOPTICS_HOSTNAME_ALIAS', ''), 2)
        if ret in (0, None):
            _ready = True
        else:
            logger.error(
...
class Context(object):
    ...
    def is_sampled(self):
        ...
        for test mode, it is always FALSE, but it need be Ture under TEST_MODE
        """
