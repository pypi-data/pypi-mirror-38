...
class Galaxy(object):
    ...
    def __init__(self, redshift=None, pixelization=None, regularization=None, hyper_galaxy=None, **kwargs):
        """
        Represents a real model_galaxy. This could be a lens model_galaxy or source model_galaxy. Note that a lens model_galaxy must have mass
        profiles

        Parameters
        ----------
        redshift: float
            The redshift of this model_galaxy
        light_profiles: [LightProfile]
            A list of light profiles describing the light profiles of this model_galaxy
        mass_profiles: [MassProfile]
            A list of mass profiles describing the mass profiles of this model_galaxy
        """
        ...
        if self.has_pixelization and not self.has_regularization:
            raise exc.GalaxyException('If the model_galaxy has a pixelization, it must also have a regularization.')
        if not self.has_pixelization and self.has_regularization:
            raise exc.GalaxyException('If the model_galaxy has a regularization, it must also have a pixelization.')
    ...
    def has_pixelization(self):
        ...
        Returns
        -------
        True iff this model_galaxy has an associated inversion
        """
    ...
    def has_hyper_galaxy(self):
        ...
        Returns
        -------
        True iff this model_galaxy has an associated hyper model_galaxy
        """
    ...
    def has_light_profile(self):
        ...
        Returns
        -------
        True iff there is one or more mass or light profiles associated with this model_galaxy
        """
    ...
    def luminosity_within_circle(self, radius):
        """
        Compute the total luminosity of the model_galaxy's light profiles within a circle of specified radius.
    ...
    def luminosity_within_ellipse(self, major_axis):
        """
        Compute the total luminosity of the model_galaxy's light profiles, within an ellipse of specified major axis. This 
    ...
    def luminosity_within_ellipse_individual(self, major_axis):
        """
        Compute the individual total luminosity of each light profile in the model_galaxy, within an ellipse of 
    ...
    def surface_density_from_grid(self, grid):
        ...
        Compute the summed surface density of the model_galaxy's mass profiles at a given set of image_grid.
    ...
    def potential_from_grid(self, grid):
        """
        Compute the summed gravitational potential of the model_galaxy's mass profiles at a given set of image_grid.
    ...
    def dimensionless_mass_within_circles(self, radius):
        """
        Compute the total dimensionless mass of the model_galaxy's mass profiles within a circle of specified radius.
    ...
    def dimensionless_mass_within_ellipses(self, major_axis):
        """
        Compute the total dimensionless mass of the model_galaxy's mass profiles within an ellipse of specified major_axis.
...
# TODO : Should model_galaxy masked_image and minimum value be in the constructor (they aren't free parameters)?
...
class HyperGalaxy(object):
    ...
    def __init__(self, contribution_factor=0.0, noise_factor=0.0, noise_power=1.0):
        ...
        contribution_factor : float
            Factor that adjusts how much of the model_galaxy's light is attributed to the contribution mappers.
        noise_factor : float
            Factor by which the noise_map_ is increased in the regions of the model_galaxy's contribution mappers.
        noise_power : float
    ...
    def contributions_from_hyper_images(self, hyper_model_image, hyper_galaxy_image, minimum_value):
        """Compute the contribution mappers of a model_galaxy, which represents the fraction of flux in each pixel that \
        model_galaxy can be attributed to contain.

        This is computed by dividing that model_galaxy's flux by the total flux in that pixel, and then scaling by the \
        ...
        hyper_galaxy_image : ndarray
            A model masked_image of the model_galaxy (e.g the lens light profile or source reconstructed_image) computed from
            a previous lensing.
        minimum_value : float
    ...
    def scaled_noise_from_contributions(self, noise, contributions):
        """Compute a scaled model_galaxy noise_map_ mappers from a baseline nosie mappers.

        This uses the model_galaxy contribution mappers with their noise_map_ scaling hyper-parameters.
        ...
        contributions : ndarray
            The model_galaxy contribution mappers.
        """
