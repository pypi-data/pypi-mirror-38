...
class LensingProfileFit(fitting.AbstractProfileFit, AbstractLensingFit):
    ...
    def __init__(self, lensing_image, tracer, padded_tracer=None):
        ...
        super(LensingProfileFit, self).__init__(fitting_images=lensing_image, images_=tracer.image_plane_images_,
    ...
    def fast_likelihood(cls, lensing_image, tracer):
        ...
        _model_image = convolve_image(tracer.image_plane_images_, tracer.image_plane_blurring_images_)
        _residuals = fitting.residuals_from_datas_and_model_datas(lensing_image[:], _model_image)
        _chi_squareds = fitting.chi_squareds_from_residuals_and_noise_maps(_residuals, lensing_image.noise_maps)
        chi_squared_term = fitting.chi_squared_terms_from_chi_squareds(_chi_squareds)
        noise_term = fitting.noise_terms_from_noise_maps(lensing_image.image.noise_maps)
    ...
    def model_images_of_planes(self):
        ...
        _model_images_of_planes = list(map(lambda plane:
                                  self.convolver_image.convolve_image(plane.image_plane_images_,
                                                                      plane.image_plane_blurring_images_),
                                           self.tracer.all_planes))

        return list(map(lambda image : None if not image.any() else self.map_to_scaled_arrays(image),
    ...
    def unmasked_model_profile_image(self):
        if self.padded_tracer is None:
            ...
            return fitting.unmasked_model_images_from_fitting_images(self.lensing_image,
                                                                     self.padded_tracer.image_plane_images_)
...
class HyperLensingProfileFit(LensingProfileFit, fitting.AbstractHyperFit):
    ...
    def __init__(self, lensing_hyper_image, tracer, padded_tracer=None):
        ...
        fitting.AbstractHyperFit.__init__(self=self, fitting_hyper_images=lensing_hyper_image,
                                          hyper_galaxies=tracer.hyper_galaxies)
        super(HyperLensingProfileFit, self).__init__(lensing_hyper_image, tracer, padded_tracer)

        self._scaled_chi_squareds = fitting.chi_squareds_from_residuals_and_noise_maps(self.residuals_,
    ...
    def fast_scaled_likelihood(cls, lensing_hyper_image, tracer):
        ...
        _scaled_noise_map = fitting.scaled_noise_from_hyper_galaxies_and_contributions(_contributions, tracer.hyper_galaxies,
                                                                                       lensing_hyper_image.noise_maps)

        convolve_image = lensing_hyper_image.convolver_image.convolve_image
        _model_image = convolve_image(tracer.image_plane_images_, tracer.image_plane_blurring_images_)
        _residuals = fitting.residuals_from_datas_and_model_datas(lensing_hyper_image[:], _model_image)
...
class LensingInversionFit(fitting.AbstractInversionFit, AbstractLensingFit):
    ...
    def __init__(self, lensing_image, tracer):
        ...
        super(LensingInversionFit, self).__init__(fitting_images=lensing_image, mapper=mapper,
    ...
    def fast_evidence(cls, lensing_image, tracer):
        mapper = tracer.mappers_of_planes[0]
        regularization = tracer.regularizations_of_planes[0]
        inversion = inversions.inversion_from_lensing_image_mapper_and_regularization(lensing_image[:], lensing_image.noise_maps,
        ...
        _chi_squareds = fitting.chi_squareds_from_residuals_and_noise_maps(_residuals, lensing_image.noise_maps)
        chi_squared_term = fitting.chi_squared_terms_from_chi_squareds(_chi_squareds)
        noise_term = fitting.noise_terms_from_noise_maps(lensing_image.noise_maps)
    ...
    @property
    def model_images_of_planes(self):
        return [None, self.map_to_scaled_arrays(self.model_datas_)]
...
class HyperLensingInversionFit(LensingInversionFit, HyperLensingInversion):
    ...
    def __init__(self, lensing_hyper_image, tracer):
        ...
        self.scaled_inversion = inversions.inversion_from_lensing_image_mapper_and_regularization(
            lensing_hyper_image[:], self.scaled_noise_maps_, lensing_hyper_image.convolver_mapping_matrix,
        ...
        self._scaled_residuals = fitting.residuals_from_datas_and_model_datas(self.datas_, self._scaled_model_image)
    ...
    def fast_scaled_evidence(cls, lensing_image, tracer):
        ...
        _scaled_residuals = fitting.residuals_from_datas_and_model_datas(lensing_image[:], scaled_inversion.reconstructed_data_vector)
...
class LensingProfileInversionFit(fitting.AbstractProfileInversionFit, AbstractLensingFit):
    ...
    def __init__(self, lensing_image, tracer, padded_tracer=None):
        ...
        AbstractLensingFit.__init__(self=self, lensing_image=lensing_image, tracer=tracer, padded_tracer=padded_tracer)
        super(LensingProfileInversionFit, self).__init__(fitting_images=lensing_image, images_=tracer.image_plane_images_,
                                                         blurring_images_=tracer.image_plane_blurring_images_,
    ...
    def fast_evidence(cls, lensing_image, tracer):
        convolve_image = lensing_image.convolver_image.convolve_image
        _profile_model_image = convolve_image(tracer.image_plane_images_, tracer.image_plane_blurring_images_)
...
class HyperLensingProfileInversionFit(LensingProfileInversionFit, HyperLensingInversion):
    ...
    def __init__(self, lensing_hyper_image, tracer, padded_tracer=None):
        ...
        self.scaled_inversion = inversions.inversion_from_lensing_image_mapper_and_regularization(
            self.profile_subtracted_images_, self.scaled_noise_maps_, lensing_hyper_image.convolver_mapping_matrix,
            self.inversion.mapper, self.inversion.regularization)

        self._scaled_model_image = self.profile_model_images_ + self.scaled_inversion.reconstructed_data_vector
        self._scaled_residuals = fitting.residuals_from_datas_and_model_datas(self.datas_, self._scaled_model_image)
        self._scaled_chi_squareds = fitting.chi_squareds_from_residuals_and_noise_maps(self._scaled_residuals,
                                                                                       self.scaled_noise_maps_)
    ...
    def fast_scaled_evidence(cls, lensing_image, tracer):
        ...
        _scaled_residuals = fitting.residuals_from_datas_and_model_datas(lensing_image[:], _scaled_model_image)
        _scaled_chi_squareds = fitting.chi_squareds_from_residuals_and_noise_maps(_scaled_residuals, _scaled_noise_map)
        scaled_chi_squared_term = fitting.chi_squared_terms_from_chi_squareds(_scaled_chi_squareds)
        scaled_noise_term = fitting.noise_terms_from_noise_maps(_scaled_noise_map)
        return fitting.evidences_from_reconstruction_terms(scaled_chi_squared_term, scaled_inversion.regularization_term,
...
def unmasked_model_images_of_galaxies_from_lensing_image_and_tracer(lensing_image, tracer):
    ...
    for plane_index, plane in enumerate(tracer.all_planes):
        for galaxy_index in range(len(plane.galaxies)):
            ...
            _galaxy_image_plane_image = plane.image_plane_images_of_galaxies_[galaxy_index]

            galaxy_model_image = fitting.unmasked_model_images_from_fitting_images(fitting_images=lensing_image,
