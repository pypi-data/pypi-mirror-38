...
# Every operation AutoLens performs to incorrect_fit a lens takes time. Every light profile intensity. Every mass profile
...
# incorrect_fit, we have to compute the light-profile intensities, the mass-profile deflection angles and convolve it with
...
# Of course, there are ways that we can reduce the number of image-pixels we incorrect_fit. That's what masking does. If we masks
# out more of the image, we'll incorrect_fit fewer pixels and AutoLens will run faster. Alternatively, we could 'bin-up' the
...
#    lens model_galaxy parameters. Can you setup phases that incorrect_fit each model_galaxy individiually, before fitting them jointly?
