...
# Every operation AutoLens performs to fit_normal a lens takes time. Every light profile intensity. Every mass profile
...
# fit_normal, we have to compute the light-profile intensities, the mass-profile deflection angles and convolve it with
...
# Of course, there are ways that we can reduce the number of image-pixels we fit_normal. That's what masking does. If we masks
# out more of the image, we'll fit_normal fewer pixels and AutoLens will run faster. Alternatively, we could 'bin-up' the
...
#    lens model_galaxy parameters. Can you setup phases that fit_normal each model_galaxy individiually, before fitting them jointly?
