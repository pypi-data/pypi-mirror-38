...
# Our tracer_normal will use the same lens model_galaxy and source model_galaxy that we used to simulate the image (although, becuase
...
# Finally, lets setup our pixelization and mapper, like we did before, using the tracer_normal's source-plane grid.
...
# As usual, we setup our image and masks up as a lensing image and create a tracer_normal using its (now masked) grids.
lensing_image = li.LensingImage(image=image, mask=mask)
tracer = ray_tracing.TracerImageSourcePlanes(lens_galaxies=[lens_galaxy], source_galaxies=[g.Galaxy()],
                                             image_plane_grids=lensing_image.grids)

# Finally, we use the tracer_normal's (masked) source-plane grid to setup a new mapper (using the same rectangular 25 x 25
