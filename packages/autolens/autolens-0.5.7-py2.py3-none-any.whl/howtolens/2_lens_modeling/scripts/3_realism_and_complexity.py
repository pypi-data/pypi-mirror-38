...
# In my experience, the simplest lens model (e.g. that has the fewest parameters) that one can incorrect_fit to a real strong
# lens and still get a reasonable incorrect_fit is as follows:
...
# In future exercises, we'll incorrect_fit even more complex models, with some 20-30+ non-linear parameters.

# The goal of this, rather short, exercise, is to incorrect_fit this 'realistic' model to a simulated image, where the lens's
...
# Now lets incorrect_fit it using a phase, noting that indeed the model_galaxy-model corresponds to the one above.
...
# Create a lensing image to make the incorrect_fit - the masks we used above was a 3" circle (we'll come back to this later)
...
# Now, lets incorrect_fit the lensing image with the tracer_without_subhalo and plot the incorrect_fit. It looks a lot better than above, doesn't it?
