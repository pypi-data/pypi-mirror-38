...
# In this example, we'll incorrect_fit the image we simulated in the previous exercise. We'll do this using model images generated
# via a tracer, and by compairing to the simulated image we'll get diagostics about the quality of the incorrect_fit.
...
# To incorrect_fit an image, we first specify a mask. A mask describes the sections of the image that we incorrect_fit.
...
# A lensing image is a 'package' of all parts of the the image data we need in order to incorrect_fit it:
...
# To incorrect_fit an image, we need to create an image-plane image using a tracer.
# Lets use the same tracer we simulated the image with (thus, our incorrect_fit should be 'perfect').
...
# To incorrect_fit the image, we pass the lensing image and tracer to the fitting module. This performs the following:
...
# 2) Computes the difference between this model_image and the observed image-data, creating the incorrect_fit's 'residuals'.

# 3) Divides the residuals by the noise-map and squaring each value, creating the fits 'chi-squareds'.

# 4) Sums up these chi-squared values and converts them to a 'likelihood', which quantities how good the tracer's incorrect_fit
#    to the data was (higher likelihood = better incorrect_fit).
...
# We used the same tracer to create and incorrect_fit the image. Therefore, our incorrect_fit to the image was excellent.
...
print(fit.likelihood)
# It decreases! This model was a worse incorrect_fit to the data.
...
# Clearly, the model provides a terrible incorrect_fit, and this tracer is not a plausible representation of
