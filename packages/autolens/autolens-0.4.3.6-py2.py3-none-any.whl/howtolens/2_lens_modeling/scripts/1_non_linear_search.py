...
# random set of parameters. We could use this lens model to create a tracer and incorrect_fit the image-data, and quantify how
# good the incorrect_fit was using its likelihood (we inspected this in previous tutorial). If we kept guessing lens
# models, eventually we'd find one that provides a good incorrect_fit (i.e. high likelihood) to the data!
...
# good incorrect_fit to the data, another set with similar values probably will too.
...
# To run the phase, we simply pass it the image data we want to incorrect_fit, and the non-linear search begins! As the phase
