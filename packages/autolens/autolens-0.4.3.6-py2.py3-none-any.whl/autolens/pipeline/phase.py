...
class Phase(object):
    ...
    def __init__(self, optimizer_class=non_linear.MultiNest, phase_name=None):
        """
        A phase in an lensing pipeline. Uses the set non_linear optimizer to try to incorrect_fit models and images passed to it.
    ...
    class Analysis(object):
        ...
        def try_output(self, instance):
            ...
            Returns
            -------
            incorrect_fit: fitting.Fit
                How incorrect_fit the model is and the model
            """
            if self.should_log:
                self.log(instance)
...
class PhasePositions(Phase):
    ...
    def run(self, positions, pixel_scale, previous_results=None):
        ...
        result: non_linear.Result
            A result object comprising the best incorrect_fit model and other data.
        """
    ...
    def make_analysis(self, positions, pixel_scale, previous_results=None):
        ...
        -------
        lensing: Analysis
            An lensing object that the non-linear optimizer calls to determine the incorrect_fit of a set of values
        """
    ...
    class Analysis(Phase.Analysis):
        ...
        def fit(self, instance):
            """
            Determine the incorrect_fit of a lens galaxy and source galaxy to the lensing_image in this lensing.
            ...
            -------
            incorrect_fit: Fit
                A fractional value indicating how well this model incorrect_fit and the model lensing_image itself
            """
...
class LensSourcePlanePhase(PhaseImaging):
    ...
    class Result(PhaseImaging.Result):
        ...
        def __init__(self, constant, likelihood, variable, analysis):
            ...
            # self.padded_model_image = self.incorrect_fit.padded_model_image

            # self.lens_galaxy_padded_model_images = self.incorrect_fit.padded_model_images_of_galaxies[0]
            # self.lens_subtracted_padded_image = analysis.lensing_image.image - self.padded_model_image
            #
            # # TODO : Need to split lens and source galaxy model images somehow
            # self.padded_model_image = self.incorrect_fit.padded_model_image
            # self.source_galaxy_padded_model_images = self.incorrect_fit.padded_model_images_of_galaxies_for_tracer
