...
# 2) It should also give us a pretty good incorrect_fit to the lensed source model_galaxy. This means we'll already know where in
#    source-plane its is located and what its intensity and effective are.


# As per usual, lets load the datas - we'll use the same datas as before, given that we're familiar with it now.
...
# And indeed, we get a reasonably good model and incorrect_fit to the datas - in a much shorter space of time!
