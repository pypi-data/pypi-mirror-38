...
# In this example, we're going to take an image and find a lens model that provides a good incorrect_fit to it and we're going
...
# I'll let you into a secret - this is the same lens model that I used to simulate the image we'll incorrect_fit (but I'm not
...
# random set of parameters. We could use this lens model to create a tracer_without_subhalo and incorrect_fit the image-datas, and quantify how
# good the incorrect_fit was using its likelihood (we inspected this in previous tutorial). If we kept guessing lens
# models, eventually we'd find one that provides a good incorrect_fit (i.e. high likelihood) to the datas!
...
# good incorrect_fit to the datas, another set with similar values probably will too.
...
# So, lets get on with it! In the file 'simulations', we've simulated the image we'll incorrect_fit in this example.
...
image = im.load_imaging_from_path(image_path=path + '/datas/1_non_linear_search_image.fits',
                                  noise_map_path=path+'/datas/1_non_linear_search_noise_map.fits',
                                  psf_path=path + '/datas/1_non_linear_search_psf.fits', pixel_scale=0.1)
...
# To run the phase, we simply pass it the image datas we want to incorrect_fit, and the non-linear search begins! As the phase
# runs, a logger will show you the parameters of the best-incorrect_fit model.
...
# We can print the results to see the best-incorrect_fit model parameters
print(results) # NOTE - this isn't working yet, need to sort out.

# The best-incorrect_fit solution (i.e. the highest likelihood) is stored in the 'results', which we can plot as per usual.
lensing_fitting_plotters.plot_fitting_subplot(fit=results.fit)

# The incorrect_fit looks good, and we've therefore found a model pretty close to the one we simulated the image with (you can
