...
# Let's go back to our complex source pipeline, but this time, as you've probably guessed, fit_normal it with an inversion.
...
# Lets simulate the image we'll fit_normal, which is the same complex source as the 3_pipelines.py tutorial.
image = simulate()
...
def make_pipeline():
    ...
    # This is te same phase 1 as the complex_source pipeline, which we saw will give a good fit_normal to the overall
    ...
    # Now, in phase 2, lets pass the lens mass model to phase 2, and use it to fit_normal the source with an inversion.
