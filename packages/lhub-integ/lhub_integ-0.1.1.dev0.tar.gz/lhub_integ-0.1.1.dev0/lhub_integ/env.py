    ...
    def read(self):
        ...
        if value is None and not self.optional:
            exit_with_instantiation_errors(
                1, [{"message": f"Environment variable {self.name} must be defined"}]
            )
        else:
            return value
...
class PrefixedEnvVar(__EnvVar, metaclass=ABCMeta):
    ...
    Environment variables that map input columns to parameter names are prefixed with `___`
    """

    def __init__(self, name):
        super().__init__(self.prefix() + name)
