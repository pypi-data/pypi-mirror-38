...
logging.basicConfig(level=logging.getLevelName(os.environ.get('LOG_LEVEL', 'ERROR').upper()))
...
class Node(object, metaclass=abc_mod.ABCMeta):
    """
...
class ComplexNode(Node):
    ...
    def __str__(self):
        if not self.complete:
            raise NodeCompleteError(
                "Can't turn an incomplete complex node into a string.")
...
class ComplexDirective(Directive):
    ...
    def complete(self):
        ...
        if self.tailmatch and not self.body.complete:
            raise NodeCompleteError(
                "Tail is matched but body is not complete.")
    ...
    def add_line(self, line, depth=0):
        ...
        if not self.header.complete:
            ...
            if header and "<" in header:
                raise InvalidLineError(
                    "Angle brackets not allowed in complex directive header.  Received: %s" % line)
            if header:
                try:
        ...
        elif re_mod.match(r"\s*</%s>\s*$" % self.name, line):
    ...
    def __str__(self):
        if not self.lines:
            raise NodeCompleteError(
                "Can't turn an uninitialized complex directive into a string.")
        if not self.complete:
            raise NodeCompleteError(
                "Can't turn an incomplete complex directive into a string.")
        return "%s\n%s%s%s" % (
            "\n".join(self.lines),
            self.body,
    ...
    def pprint(self, depth=0):
        if not self.complete:
            raise NodeCompleteError(
                "Can't print an incomplete complex directive.")
        return "%s<%s>\n%s%s%s</%s>" % (
            INDENT*depth,
            self.header.pprint(),
