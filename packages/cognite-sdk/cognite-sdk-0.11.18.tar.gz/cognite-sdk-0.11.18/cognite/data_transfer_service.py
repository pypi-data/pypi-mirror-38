...
class TimeSeries:
    ...
    def __init__(self, id, aggregates=None, missing_data_strategy=None, label=None):
        self.id = id
...
class DataTransferService:
    ...
    def __init__(self, data_spec, project=None, api_key=None, cookies=None, num_of_processes=10):
    ...
    def get_dataframes(self, drop_agg_suffix=True):
    ...
    def get_dataframe(self, label="default", drop_agg_suffix=True):
        if self.ts_data_specs is None:
            raise InputError("Data spec does not contain any TimeSeriesDataSpecs")
        tsds = None
    ...
    def __convert_ts_names_to_labels(self, df, tsds, drop_agg_suffix):
        name_to_label = {}
        for ts in tsds.time_series:
