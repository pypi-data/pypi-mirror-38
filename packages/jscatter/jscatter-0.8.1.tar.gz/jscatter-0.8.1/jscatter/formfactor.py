...
def cuboid(q, a, b=None, c=None, SLD=1, solventSLD=0, N=30):
    ...
    .. math:: I(q)=\rho^2V_{cube}^2 \int_{0}^{2\pi}\int_{0}^{\pi} \lvert sinc(aq_x ) sinc(bq_y) sinc(cq_z)\rvert^2 \sin\theta d\theta d\phi
...
def multilayer(q, shelld, SLD, solventSLD=0, ds=0):
    """
    ...
    """
    shelld=np.atleast_1d(shelld)
    # generate symmetric thicknesses
...
def multilamellarVesicles(Q, R, N, phi, displace=0, dR=0, dN=0,shellthickness=0,ds=0, SLD=1, solventSLD=0,nGauss=100):
    ...
    if ds>0 and shelld[0] > 0 :
        ...
        fq=dL([multilayer(q=Q, shelld=shelld, SLD=sld, solventSLD=0, ds=xx) for xx, ww in zip(x, w)])
        Fq=(fq.Y.array*w[:,None]).sum(axis=0)/w.sum()
    else:
        # no thickness fluctuation
        Fq = multilayer(Q, shelld, sld, 0).Y
