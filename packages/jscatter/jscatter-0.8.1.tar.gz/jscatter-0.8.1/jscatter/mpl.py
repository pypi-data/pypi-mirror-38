...
def contourImage(x,y=None,z=None,levels=8,fontsize=10,colorMap='jet',lineMap=None,title=None,axis=None):
    """
    ...
    if np.ndim(x)<2 or hasattr(x,'_isdataArray'):
        ...
        if np.unique(x).shape[0]*np.unique(y)[0]==np.shape(z)[0]:
            ...
            im = ax.imshow(zz, cmap=cmap, extent=extend, origin='lower')
            im.cset = pyplot.contour(zz,levels=levels,linewidths=1,cmap=lmap,extent=extend,origin='lower')
            im.labels=pyplot.clabel(im.cset,inline=True,fmt='%1.1f',fontsize=fontsize)
            ...
            im=ax.tricontour(x, y, z, levels=levels, linewidths=1, cmap=lmap,extent=extend,origin='lower')
            im2=ax.tricontourf(x, y, z, levels=levels, cmap=cmap,extent=extend,origin='lower')
        ...
        try:
            ...
            extend=None
        im = ax.imshow(z, cmap=cmap, extent=extend, origin='lower')
        im.cset = pyplot.contour(z,levels=levels,linewidths=1,cmap=lmap,extent=extend,origin='lower')
        im.labels=pyplot.clabel(im.cset,inline=True,fmt='%1.1f',fontsize=fontsize)
        fig.colorbar(im) # note that colorbar is a method of the figure, not the axes
    # adding the Contour lines with labels  old
    #try:
    #    im.cset = pyplot.contour(z,levels,linewidths=1,cmap=lmap,extent=extend,origin='lower')
    #    im.labels=pyplot.clabel(im.cset,inline=True,fmt='%1.1f',fontsize=10)
    #except:
    #    im.cset = pyplot.contour(z,levels, linewidths=1,extent=extend,origin='lower')
