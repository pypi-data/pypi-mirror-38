...
shape=[%(ll)s] %(length)s     """
    ...
    def filter(self,filterfunction=None,**kwargs):
        ...
        If no filterfunction is given the filterfunction is generated
        to compare keys with values from kwargs.
        ...
        if filterfunction is None:
            ...
            filterfunction = lambda a:np.alltrue([getattr(a,k) == v for k,v in kwargs.items()])
    ...
    def fit(self,model,freepar={},fixpar={},mapNames={},method='leastsq',xslice=slice(None),condition=None,output=True,**kw):
        ...
        - A least square fit of the .Y values dependent on X (,Z,W) and attributes (multidimensional fitting).
        ...
        method : default 'leastsq', 'differential_evolution', ‘BFGS’, ‘Nelder-Mead’  or from scipy.optimize.minimize
            Type of solver for minimization, for options see scipy.optimize. See last example below for a
            comparison and usage.
             ...
             - 'differential_evolution' uses automatic bounds as (x0/10**0.5,x0*10**0.5)
               if no explicit bounds are set for a freepar. x0 is start value from freepar.
        ...
        **How to make a model**:
        ...
        More explicit example can be found in

        :ref:`How to build simple models`

        :ref:`How to build a more complex model`

        :ref:`Some Sinusoidal fits with different kinds to use data atrributes`
        ...
        - Comparison of fit methods ::
           ...
           # 498 evaluations, error YES ; needs >20000 evaluations using D=[0.2]
        ...
        * Limits for parameters can be set prior to the fit as .setlimit(D=[1,4,0,10]).
          ...
          (hard set to these values if outside interval and increasing chi2).
        * If errors exist (.eY) and are not zero, the error weighted chi**2 is minimized.
          Without error or with single errors equal zero an unweighted chi**2 is minimized (equal weights).
        * Errors are 1-sigma errors as determined from the covariance matrix (see [1]_).
          This holds under the assumption that we have a well-behaved likelihood function which is asymptotically
          Gaussian near its maximum.
          ...
          Thus for equal weights error weighted chi**2 and unweighted chi**2 the result is a 1- sigma error.
