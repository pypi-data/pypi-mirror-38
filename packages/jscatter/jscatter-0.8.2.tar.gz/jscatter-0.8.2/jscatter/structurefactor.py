...
def _Z0q(qhkl,qpeaks,f2peaks,peakWidthSigma,rotvector,angle=0):
    ...
    if rotvector is not None and angle!=0:
        ...
        qqhkl=np.einsum('ij,kj->ki',R,qhkl)
    else:
        qqhkl=qhkl.copy()
    # calc Z0q
    if useFortran:
        Z0q = np.c_[[f2 *  fscatter.cloud.lhkl(qqhkl, q, peakWidthSigma)
                                        for q, f2 in zip(qpeaks, f2peaks) if la.norm(q)>0]].sum(axis=0)
    else:
        Z0q = np.c_[[f2 * _Lhkl(qqhkl, q, peakWidthSigma)
                                        for q, f2 in zip(qpeaks, f2peaks) if la.norm(q)>0]].sum(axis=0)
    return Z0q

def orientedLatticeStructureFactor(qxyz, lattice, orientation=None, rotation=None, domainsize=1000,
    ...
    ----------
    qxyz : array 3xN
        Wavevector array representing a surface in q space (3D), units 1/A or 1/nm.
    ...
    orientation : 3 float as [h,k,l], None
        Orientation of the crystal relative to  qxyz surface.
         - [h,k,l] rotates the crystal that the lattice hkl direction points to the center of qxyz (mean position).
           e.g. points to qxyz plane center.
         - If None the orientation is not changed.
    rotation : 5 float as [mean,sigma,h,k,l], None
        Average over rotation of the crystal around axis hkl
        with Gaussian distribution around mean and width sigma (units rad).
    ...
    - To get the scattering of a specific particle shape the formfactor has to be included.
      The above is valid for isotropic scatterers (symetric or uncorrelated to the crystal orientation)
    ...
    Comparison fcc and sc to demonstrate selection rules ::
     ...
     ffe=js.sf.orientedLatticeStructureFactor(qxyz,fcclattice,orientation=[1,1,1],domainsize=ds,rmsd=0.1,hklmax=4)
     fig=js.mpl.surface(ffe.X,ffe.Y,ffe[3].array)
     sclattice= js.lattice.scLattice(3.1, 5)
     ffs=js.sf.orientedLatticeStructureFactor(qxyz,sclattice,orientation=[1,1,1],domainsize=ds,rmsd=0.1,hklmax=4)
     fig=js.mpl.surface(ffs.X,ffs.Y,ffs[3].array)
     # or
     fig2=js.mpl.contourImage(ffs[[0,1,3]])
    ...
    if isinstance(domainsize,(float,int)):
        ...
        if np.ndim(domainsize)==1:
            ...
            peakWidthSigma=np.sum([ s*lV/la.norm(lV) for lV,s in zip(lattice.latticeVectors,sigma)],axis=1)
        else:
            ...
            peakWidthSigma = np.sum([s * lV / la.norm(lV) for lV, s in zip(hkldirection, sigma)], axis=1)
    else:
        raise TypeError('domainsize cannot be interpreted.')

    if orientation is not None:
        qhkl=lattice.rotate2hkl(qxyz,orientation[0],orientation[1],orientation[2])

    if rotation is not None:
        # rotation direction
        rotvector=rotation[2]*lattice.latticeVectors[0]+\
                  rotation[3]*lattice.latticeVectors[1]+\
                  rotation[4]*lattice.latticeVectors[2]
        R=formel.rotationMatrix(rotvector,rotation[0])
        qhkl=np.einsum('ij,kj->ki',R,qhkl)
    else:
        rotvector=None

    # Debye Waller factor
    qr=(qxyz**2).sum(axis=1)**0.5
    ...
    if rotation is not None and abs(rotation[1])>0:
        # gauss distribution
        Z0q=formel.parDistributedAverage(_Z0q, abs(rotation[1]), parname='angle', nGauss=nGauss,
                                         qhkl=qhkl,qpeaks=qpeaks,f2peaks=f2peaks,
                                         peakWidthSigma=peakWidthSigma,rotvector=rotvector,angle=0,ncpu=0 )
    else:
        # single orientation
        Z0q=_Z0q(qhkl,qpeaks,f2peaks,peakWidthSigma,rotvector,0)
...
def radialorientedLSF(*args,**kwargs):
    ...
    ::
     ...
     ffe=js.sf.radialorientedLSF(qxyz=qxyz,lattice=fcclattice,orientation=[1,1,1],domainsize=ds,rmsd=0.05,hklmax=4)
     p=js.grace()
     p.plot(ffe)
