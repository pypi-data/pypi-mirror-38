...
def sphereCoreShellGaussianCorona(q, Rc, Rs, Rg, Ncoil, thicknessCoils, coilSLD=0.64e-4, coreSLD=4.186e-4, shellSLD=2e-4, solventSLD= 6.335e-4, d=1):
    ...
    ::
     ...
     p=js.grace()
     p.plot(js.ff.sphereCoreShellGaussianCorona(q,8,8,6,20,1.5),sy=0,li=1)
     p.yaxis(scale='l')
     p.xaxis(scale='l')
...
def multilamellarVesicles(Q, R, N, phi, displace=0, dR=0, dN=0, layers=0, gaussw=None, ds=0, SLD=1, solventSLD=0, nGauss=100):
    ...
    - Comparing a unilamellar vesicle (N=1) with multiShellSphere shows that
      R is located in the center of the shell::
        ...
        mV=ffmV(Q=Q, R=100., displace=0, dR=0,N=1,dN=0, phi=1,layerthickness=6, SLD=1e-4)
        p[0].plot(mV)
        p[0].plot(js.ff.multiShellSphere(Q,[97,6],[0,1e-4]),li=[1,1,3],sy=0)
        # triple layer
        mV1=ffmV(Q=Q, R=100., displace=0, dR=0,N=1,dN=0, phi=1,layerthickness=[1,4,1], SLD=[0.07e-3,0.6e-3,0.07e-3])
