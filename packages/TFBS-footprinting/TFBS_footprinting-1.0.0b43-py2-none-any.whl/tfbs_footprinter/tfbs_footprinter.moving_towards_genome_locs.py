...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent("""\
            ...
            Example Usage:
                ...
                all arguments:
                tfbs_footprinter -t PATH_TO/sample_ensembl_ids.txt -tfs PATH_TO/sample_jaspar_tf_ids.txt -s homo_sapiens -sg mammals -e low -pb 900 -pa 100 -tx 10 -update
    ...
    parser.add_argument('--genomic_locs', '-g', metavar='', type=str, choices = [True, False], default = False,
                        help='Optional: Indicates whether the target location file contains genomic locations instead of Ensembl transcript ids. \
                        If using "genomic" option, each line/row of the file should include species name, chromosome, strand, start, end. \
                        (see sample file tf_ids.txt at https://github.com/thirtysix/TFBS_footprinting) [default: all Jaspar TFs]')

    parser.add_argument('--tf_ids_file', '-tfs', metavar='', type=str, default = None,
                        help='Optional: Location of a file containing a limited list of Jaspar TFs to use in scoring alignment \
                        (see sample file tf_ids.txt at https://github.com/thirtysix/TFBS_footprinting) [default: all Jaspar TFs]')
...
def generate_args_lists(args):
    ...
    if transcript_ids_filename:
        ...
        if file_extension == ".csv" or file_extension == ".tsv":
            ...
            use_genomic_loc = sargs.genomic_locs
            if use_genomic_loc:
