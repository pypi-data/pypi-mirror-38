...
template <class T>
void AtomicSAGA<T>::dense_thread_solve(int n_epochs, size_t n_thread) {
  ulong n_samples = model->get_n_samples();
  T n_samples_inverse = (T)1 / (T) n_samples;
  ...
  T grad_i_factor = 0, grad_i_factor_old = 0, gradients_average_j = 0;
  ...
  for (int epoch = 1; epoch < (n_epochs + 1); ++epoch) {
    for (ulong t = 0; t < thread_epoch_size; ++t) {
      ...
      for (ulong j = 0; j < n_features; ++j) {
        ...
        // gradients_average[j] += grad_factor_diff * x_ij * n_samples_inverse;
      ...
      if (use_intercept) {
        ...
        gradients_average_j = gradients_average[n_features];
        while (!gradients_average[n_features].compare_exchange_weak(
            gradients_average_j,
            gradients_average_j + (grad_factor_diff * n_samples_inverse))) {
        }
      }
...
template <class T>
void AtomicSAGA<T>::sparse_thread_solve(int n_epochs, size_t n_thread) {
  ulong n_samples = model->get_n_samples();
  T n_samples_inverse = (T)1 / (T) n_samples;
...
void AtomicSAGA<T>::solve(int n_epochs) {
  ...
  if (!model->is_sparse())
    for (size_t i = 0; i < un_threads; i++)
      threads.emplace_back(&AtomicSAGA<T>::dense_thread_solve, this, n_epochs, i);
  else
    for (size_t i = 0; i < un_threads; i++)
      threads.emplace_back(&AtomicSAGA<T>::sparse_thread_solve, this, n_epochs, i);
