...
def parse_req_tcp(ba: (bytes, bytearray), offset: int = 0, nb: (None, int) = None) -> (None, collections.namedtuple):
    assert isinstance(ba, (bytes, bytearray)), ba
    assert isinstance(offset, int), offset
    ...
    if fn_nb == MB_FN_READ_OUT_BITS_1 or fn_nb == MB_FN_READ_IN_BITS_2:
        ...
        if nb_bits == 0 or nb_bits > MAX_READ_BITS_NB or (addr + nb_bits - 1) > 0xffff:
            raise MbError(ErrorCode.BadData)
        ...
        if nb_words == 0 or nb_words > MAX_READ_WORDS_NB or (addr + nb_words - 1) > 0xffff:
            raise MbError(ErrorCode.BadData)
