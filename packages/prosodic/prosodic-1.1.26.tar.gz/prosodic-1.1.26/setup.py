...
README_CONFIG="""
# PROSODIC HOME DIRECTORY
This is your home directory for Prosodic: https://github.com/quadrismegistus/prosodic

* In order to configure Prosodic, copy or rename 'config_default.py' to 'config.py', and edit that file according to its instructions. [Note: 'config_default.py' will be overwritten if you update Prosodic, but 'config.py' will not be.]
* To edit or create your own meter, copy or rename 'meters/meter_default.py' to 'meters/your_meter_name.py', and edit that file according to its instructions. Then consider changing the default 'meter' setting in your config.py to 'your_meter_name'. You can also select 'your_meter_name' from within Prosodic. [Note: 'meters/meter_default.py' will be overwritten if you update Prosodic, but 'meters/your_meter_name.py' will not be.]
* To run Prosodic against your own tagged sample, create a file like 'tagged_samples/tagged-sample-litlab-2016.txt', which has at least a column for the line (e.g. "From fairest creatures we desire increase") and a column for the parse (e.g. "wswswswsws"). [Note: 'tagged_samples/tagged-sample-litlab-2016.txt' will be overwritten if you update Prosodic, but your own files will not be.]
...
def configure_home_dir():
	...
	if not os.path.exists(opath_samples): os.makedirs(opath_samples)
