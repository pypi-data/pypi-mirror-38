        ...
        while not self.is_stopped():
            ...
            for mgr in self.config_mgrs.values():
                ...
                if os.path.getmtime(mgr.config_file) > mgr.acktime:
                    ...
                    LOG.info("Config already ACK'ed.")

            # 3rd do data_out
            if inbound and inbound.resource == 'data_out':
                LOG.critical("ACKing data_out: {}".format(inbound))
                self.device.tell(
                    resource=inbound.resource,
                    timestamp=time.time(),
                    payload=inbound.payload
                )
                LOG.critical("Got data_out: {}".format(inbound.payload))
                for name, value in payload.items():
                    LOG.critical("Searching for channel: {}".format(name))
